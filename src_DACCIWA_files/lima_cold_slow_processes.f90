!MNH_LIC Copyright 2013-2018 CNRS, Meteo-France and Universite Paul Sabatier
!MNH_LIC This is part of the Meso-NH software governed by the CeCILL-C licence
!MNH_LIC version 1. See LICENSE, CeCILL-C_V1-en.txt and CeCILL-C_V1-fr.txt  
!MNH_LIC for details. version 1.
!-----------------------------------------------------------------
!      #####################
       MODULE MODI_LIMA_COLD_SLOW_PROCESSES
!      #####################
!
INTERFACE
      SUBROUTINE LIMA_COLD_SLOW_PROCESSES (PTSTEP, KMI, PZZ, PRHODJ,                 &
                                           PRHODREF, PEXNREF, PPABST,                &
                                           PTHT, PRVT, PRCT, PRRT, PRIT, PRST, PRGT, &
                                           PTHS, PRVS, PRIS, PRSS,                   &
                                           PCIT, PCIS, PCIS_SHAPE, PCIT_SHAPE)
!
REAL,                     INTENT(IN)    :: PTSTEP  ! Time step          
INTEGER,                  INTENT(IN)    :: KMI     ! Model index 
!
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PZZ     ! Height (z)
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRHODJ  ! Dry density * Jacobian
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRHODREF! Reference density
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PEXNREF ! Reference Exner function
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PPABST  ! abs. pressure at time t
!
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PTHT    ! Theta at time t
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRVT    ! Water vapor m.r. at t 
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRCT    ! Cloud water m.r. at t 
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRRT    ! Rain water m.r. at t 
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRIT    ! Cloud ice m.r. at t 
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRST    ! Snow/aggregate m.r. at t 
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRGT    ! Graupel m.r. at t 
!
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PTHS    ! Theta source
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PRVS    ! Water vapor m.r. source
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PRIS    ! Pristine ice m.r. source
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PRSS    ! Snow/aggregate m.r. source
!
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PCIT    ! Ice crystal C. at t
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PCIS    ! Ice crystal C. source
REAL, DIMENSION(:,:,:,:), INTENT(INOUT) :: PCIS_SHAPE  ! 
REAL, DIMENSION(:,:,:,:), INTENT(IN)    :: PCIT_SHAPE  ! 
!
END SUBROUTINE LIMA_COLD_SLOW_PROCESSES
END INTERFACE
END MODULE MODI_LIMA_COLD_SLOW_PROCESSES
!
!     ################################################################################
      SUBROUTINE LIMA_COLD_SLOW_PROCESSES (PTSTEP, KMI, PZZ, PRHODJ,                 &
                                           PRHODREF, PEXNREF, PPABST,                &
                                           PTHT, PRVT, PRCT, PRRT, PRIT, PRST, PRGT, &
                                           PTHS, PRVS, PRIS, PRSS,                   &
                                           PCIT, PCIS, PCIS_SHAPE, PCIT_SHAPE)
!     ################################################################################
!
!!    PURPOSE
!!    -------
!!      The purpose of this routine is to compute the microphysical sources
!!    for slow cold processes :
!!      - conversion of snow to ice
!!      - deposition of vapor on snow
!!      - conversion of ice to snow (Harrington 1995)
!!      - aggregation of ice on snow
!!
!!
!!    REFERENCE
!!    ---------
!!
!!
!!    AUTHOR
!!    ------
!!      J.-M. Cohard     * Laboratoire d'Aerologie*
!!      J.-P. Pinty      * Laboratoire d'Aerologie*
!!      S.    Berthet    * Laboratoire d'Aerologie*
!!      B.    Vié        * Laboratoire d'Aerologie*
!!
!!    MODIFICATIONS
!!    -------------
!!      Original             ??/??/13 
!!      C. Barthe  * LACy *  jan. 2014   add budgets
!!  Philippe Wautelet: 05/2016-04/2018: new data structures and calls for I/O
!!      M. Claeys  * LACy *  mar. 2019   add ice crystal shapes
!!
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
!
USE MODD_PARAMETERS,      ONLY : JPHEXT, JPVEXT
USE MODD_CST,             ONLY : XP00, XRD, XRV, XMV, XMD, XCPD, XCPV,        &
                                 XCL, XCI, XTT, XLSTT, XALPI, XBETAI, XGAMI
USE MODD_PARAM_LIMA,      ONLY : LSNOW, XRTMIN, XCTMIN, XALPHAI, XALPHAS,     &
                                 XNUI, LCRYSTAL_SHAPE, NB_CRYSTAL_SHAPE
USE MODD_PARAM_LIMA_COLD, ONLY : XLBI, XLBEXI, XLBS, XLBEXS, XBI, XCXS, XCCS,    &
                                 XLBDAS_MAX, XDSCNVI_LIM, XLBDASCNVI_MAX,        &
                                 XC0DEPSI, XC1DEPSI, XR0DEPSI, XR1DEPSI,         &
                                 XSCFAC, X1DEPS, X0DEPS, XEX1DEPS, XEX0DEPS,     &
                                 XDICNVS_LIM, XLBDAICNVS_LIM,                    &
                                 XC0DEPIS, XC1DEPIS, XR0DEPIS, XR1DEPIS,         &
                                 XCOLEXIS, XAGGS_CLARGE1, XAGGS_CLARGE2,         &
                                 XAGGS_RLARGE1, XAGGS_RLARGE2,                   &
                                 XLBI_SHAPE, XLBEXI_SHAPE, XBI_SHAPE,            &
                                 XC0DEPIS_SHAPE, XC1DEPIS_SHAPE, XR0DEPIS_SHAPE, &
                                 XR1DEPIS_SHAPE,                                 &
                                 XAGGS_RLARGE1_SHAPE, XAGGS_RLARGE2_SHAPE
USE MODD_BUDGET
USE MODD_NSV, ONLY : NSV_LIMA_NI
!
USE MODI_LIMA_FUNCTIONS,  ONLY : COUNTJV
USE MODI_COMPUTE_LBDA_SHAPE
USE MODI_BUDGET

!
IMPLICIT NONE
!
!*       0.1   Declarations of dummy arguments :
!
REAL,                     INTENT(IN)    :: PTSTEP  ! Time step          
INTEGER,                  INTENT(IN)    :: KMI     ! Model index 
!
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PZZ     ! Height (z)
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRHODJ  ! Dry density * Jacobian
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRHODREF! Reference density
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PEXNREF ! Reference Exner function
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PPABST  ! abs. pressure at time t
!
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PTHT    ! Theta at time t
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRVT    ! Water vapor m.r. at t 
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRCT    ! Cloud water m.r. at t 
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRRT    ! Rain water m.r. at t 
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRIT    ! Cloud ice m.r. at t 
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRST    ! Snow/aggregate m.r. at t 
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PRGT    ! Graupel m.r. at t 
!
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PTHS    ! Theta source
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PRVS    ! Water vapor m.r. source
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PRIS    ! Pristine ice m.r. source
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PRSS    ! Snow/aggregate m.r. source
!
REAL, DIMENSION(:,:,:),   INTENT(IN)    :: PCIT    ! Ice crystal C. at t
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PCIS    ! Ice crystal C. source
REAL, DIMENSION(:,:,:,:), INTENT(INOUT) :: PCIS_SHAPE  ! Ice crystal C. source
REAL, DIMENSION(:,:,:,:), INTENT(IN)    :: PCIT_SHAPE  ! Ice crystal C. source
!
!
!*       0.2   Declarations of local variables :
!
LOGICAL, DIMENSION(SIZE(PRHODREF,1),SIZE(PRHODREF,2),SIZE(PRHODREF,3)) &
			  :: GMICRO ! Computations only where necessary
INTEGER :: IMICRO
INTEGER , DIMENSION(SIZE(GMICRO)) :: I1,I2,I3 ! Used to replace PACK
INTEGER                           :: JL       ! and PACK intrinsics
INTEGER                           :: JSH      ! Index for number of ice crystal shapes
!
REAL, DIMENSION(:), ALLOCATABLE :: ZRVT    ! Water vapor m.r. at t
REAL, DIMENSION(:), ALLOCATABLE :: ZRCT    ! Cloud water m.r. at t
REAL, DIMENSION(:), ALLOCATABLE :: ZRRT    ! Rain water m.r. at t
REAL, DIMENSION(:), ALLOCATABLE :: ZRIT    ! Pristine ice m.r. at t
REAL, DIMENSION(:), ALLOCATABLE :: ZRST    ! Snow/aggregate m.r. at t
REAL, DIMENSION(:), ALLOCATABLE :: ZRGT    ! Graupel/hail m.r. at t
!
REAL, DIMENSION(:), ALLOCATABLE :: ZRVS    ! Water vapor m.r. source
REAL, DIMENSION(:), ALLOCATABLE :: ZRIS    ! Pristine ice m.r. source
REAL, DIMENSION(:), ALLOCATABLE :: ZRSS    ! Snow/aggregate m.r. source
!
REAL, DIMENSION(:),   ALLOCATABLE :: ZTHS    ! Theta source
!
REAL,    DIMENSION(:),   ALLOCATABLE :: ZCIT    ! Pristine ice conc. at t
REAL,    DIMENSION(:),   ALLOCATABLE :: ZCIS    ! Pristine ice conc. source
REAL,    DIMENSION(:,:), ALLOCATABLE :: ZCIS_SHAPE  ! Pristine ice conc. source
REAL,    DIMENSION(:,:), ALLOCATABLE :: ZCIT_SHAPE  ! Pristine ice conc. at t
REAL,    DIMENSION(:,:), ALLOCATABLE :: ZCSHAPE_T   ! ratio of conc. per source
REAL,    DIMENSION(:,:), ALLOCATABLE :: ZCSHAPE_S   ! ratio of conc. per source
REAL,    DIMENSION(:,:), ALLOCATABLE :: ZLBDAI_SHAPE_T
REAL,    DIMENSION(:,:), ALLOCATABLE :: ZLBDAI_SHAPE_S
REAL,    DIMENSION(:,:), ALLOCATABLE :: ZRIS_SHAPE, ZRIT_SHAPE
INTEGER, DIMENSION(:),   ALLOCATABLE :: ISHAPE_MAX_S  ! index of the dominant shape concentration 
INTEGER, DIMENSION(:),   ALLOCATABLE :: ISHAPE_MAX_T  ! index of the dominant shape concentration 
REAL,    DIMENSION(:),   ALLOCATABLE :: ZONEOVER_VARS, ZONEOVER_VART  ! for optimization
!
REAL, DIMENSION(:), ALLOCATABLE &
                   :: ZRHODREF, & ! RHO Dry REFerence
                      ZRHODJ,   & ! RHO times Jacobian
                      ZZT,      & ! Temperature
                      ZPRES,    & ! Pressure
                      ZEXNREF,  & ! EXNer Pressure REFerence
                      ZZW,      & ! Work array
                      ZZX,      & ! Work array
                      ZLSFACT,  & ! L_s/(Pi_ref*C_ph)
                      ZSSI,     & ! Supersaturation over ice
                      ZLBDAI,   & ! Slope parameter of the ice crystal distr.
                      ZLBDAS,   & ! Slope parameter of the aggregate distr.
                      ZAI,      & ! Thermodynamical function
                      ZCJ,      & ! used to compute the ventilation coefficient
                      ZKA,      & ! Thermal conductivity of the air
                      ZDV,      & ! Diffusivity of water vapor in the air
                      ZVISCA      ! Viscosity of air
!
REAL, DIMENSION(:,:), ALLOCATABLE :: ZZW1      ! Work arrays
!
REAL, DIMENSION(SIZE(PRHODREF,1),SIZE(PRHODREF,2),SIZE(PRHODREF,3))   &
                                  :: ZT, ZW ! Temperature
REAL, DIMENSION(:,:,:,:), ALLOCATABLE :: ZW_4D
REAL, DIMENSION(:,:),     ALLOCATABLE :: ZZW_2D, ZZX_2D, ZZW1_2D
REAL, DIMENSION(:,:),     ALLOCATABLE :: ZZW_2D_MR
!
INTEGER :: IIB, IIE, IJB, IJE, IKB, IKE        ! Physical domain
!
REAL,    DIMENSION(:),   ALLOCATABLE :: ZRTMIN, ZCTMIN
!
!-------------------------------------------------------------------------------
!
! Physical domain
!
IIB=1+JPHEXT
IIE=SIZE(PZZ,1) - JPHEXT
IJB=1+JPHEXT
IJE=SIZE(PZZ,2) - JPHEXT
IKB=1+JPVEXT
IKE=SIZE(PZZ,3) - JPVEXT
!
! Physical limitations
!
ALLOCATE(ZRTMIN(SIZE(XRTMIN)))
ALLOCATE(ZCTMIN(SIZE(XCTMIN)))
ZRTMIN(:) = XRTMIN(:) / PTSTEP
ZCTMIN(:) = XCTMIN(:) / PTSTEP
!
! Temperature
ZT(:,:,:)  = PTHT(:,:,:) * ( PPABST(:,:,:)/XP00 ) ** (XRD/XCPD)
!
! Looking for regions where computations are necessary
!
GMICRO(:,:,:) = .FALSE.
GMICRO(IIB:IIE,IJB:IJE,IKB:IKE) =                            &
     PRIT(IIB:IIE,IJB:IJE,IKB:IKE)>XRTMIN(4) .OR. &
     PRST(IIB:IIE,IJB:IJE,IKB:IKE)>XRTMIN(5)
!
IMICRO = COUNTJV( GMICRO(:,:,:),I1(:),I2(:),I3(:))
!
!++je++ 19/02/20 test //
!IF( IMICRO >= 1 ) THEN
IF ( IMICRO >= 0 ) THEN
!--je--
!
!------------------------------------------------------------------------------
!
!
!*       1.    Optimization : packing variables
!              --------------------------------
!
!
!
  ALLOCATE(ZRVT(IMICRO)) 
  ALLOCATE(ZRCT(IMICRO)) 
  ALLOCATE(ZRRT(IMICRO)) 
  ALLOCATE(ZRIT(IMICRO)) 
  ALLOCATE(ZRST(IMICRO)) 
  ALLOCATE(ZRGT(IMICRO)) 
!
  ALLOCATE(ZRVS(IMICRO))  
  ALLOCATE(ZRIS(IMICRO))
  ALLOCATE(ZRSS(IMICRO))
!
  ALLOCATE(ZTHS(IMICRO))
! 
  ALLOCATE(ZRHODREF(IMICRO)) 
  ALLOCATE(ZZT(IMICRO)) 
  ALLOCATE(ZPRES(IMICRO)) 
  ALLOCATE(ZEXNREF(IMICRO))
!
  ALLOCATE(ZCIT(IMICRO)) 
  ALLOCATE(ZCIS(IMICRO))
!
  IF (LCRYSTAL_SHAPE) THEN
    ALLOCATE(ZCIT_SHAPE(IMICRO,NB_CRYSTAL_SHAPE ))
    ALLOCATE(ZCIS_SHAPE(IMICRO,NB_CRYSTAL_SHAPE ))
    ALLOCATE(ZRIS_SHAPE(IMICRO,NB_CRYSTAL_SHAPE))
    ALLOCATE(ZRIT_SHAPE(IMICRO,NB_CRYSTAL_SHAPE))
    ZRIS_SHAPE(:,:) = 0.0
    ZRIT_SHAPE(:,:) = 0.0
  END IF
!
  DO JL=1,IMICRO   
    ZRVT(JL) = PRVT(I1(JL),I2(JL),I3(JL))
    ZRCT(JL) = PRCT(I1(JL),I2(JL),I3(JL))
    ZRRT(JL) = PRRT(I1(JL),I2(JL),I3(JL))
    ZRIT(JL) = PRIT(I1(JL),I2(JL),I3(JL))
    ZRST(JL) = PRST(I1(JL),I2(JL),I3(JL))
    ZRGT(JL) = PRGT(I1(JL),I2(JL),I3(JL))
!
    ZCIT(JL) = PCIT(I1(JL),I2(JL),I3(JL))
    ZCIS(JL) = PCIS(I1(JL),I2(JL),I3(JL))
    IF (LCRYSTAL_SHAPE) THEN
      DO JSH = 1, NB_CRYSTAL_SHAPE
        ZCIT_SHAPE(JL,JSH) = PCIT_SHAPE(I1(JL),I2(JL),I3(JL),JSH)
        ZCIS_SHAPE(JL,JSH) = PCIS_SHAPE(I1(JL),I2(JL),I3(JL),JSH)
      END DO
    END IF
!
    ZRVS(JL) = PRVS(I1(JL),I2(JL),I3(JL))
    ZRIS(JL) = PRIS(I1(JL),I2(JL),I3(JL))
    ZRSS(JL) = PRSS(I1(JL),I2(JL),I3(JL))
!
    ZTHS(JL) = PTHS(I1(JL),I2(JL),I3(JL))
!
    ZRHODREF(JL) = PRHODREF(I1(JL),I2(JL),I3(JL))
    ZZT(JL)      = ZT(I1(JL),I2(JL),I3(JL))
    ZPRES(JL)    = PPABST(I1(JL),I2(JL),I3(JL))
    ZEXNREF(JL)  = PEXNREF(I1(JL),I2(JL),I3(JL))
  ENDDO
!
  IF (NBUMOD==KMI .AND. LBU_ENABLE) THEN
    ALLOCATE(ZRHODJ(IMICRO))
    ZRHODJ(:) = PACK( PRHODJ(:,:,:),MASK=GMICRO(:,:,:) )
  END IF
!
!------------------------------------------------------------------------------
!
!*       2.    Microphysical computations
!              --------------------------
! 
  ALLOCATE(ZZW(IMICRO))
  ALLOCATE(ZZX(IMICRO))
  ALLOCATE(ZLSFACT(IMICRO))
  ALLOCATE(ZSSI(IMICRO))
  ALLOCATE(ZLBDAI(IMICRO))
  ZLBDAI(:) = 1.E10
  IF (LCRYSTAL_SHAPE) THEN
    ALLOCATE(ZLBDAI_SHAPE_T(IMICRO,NB_CRYSTAL_SHAPE))
    ALLOCATE(ZLBDAI_SHAPE_S(IMICRO,NB_CRYSTAL_SHAPE))
    ZLBDAI_SHAPE_T(:,:) = 1.E10
    ZLBDAI_SHAPE_S(:,:) = 1.E10
  END IF
  ALLOCATE(ZLBDAS(IMICRO))
  ALLOCATE(ZAI(IMICRO))
  ALLOCATE(ZCJ(IMICRO))
  ALLOCATE(ZKA(IMICRO))
  ALLOCATE(ZDV(IMICRO))
  ALLOCATE(ZZW1(IMICRO,7))
!
! Preliminary computations
!
  ZZW(:)  = ZEXNREF(:)*( XCPD+XCPV*ZRVT(:)+XCL*(ZRCT(:)+ZRRT(:)) &
                                  +XCI*(ZRIT(:)+ZRST(:)+ZRGT(:)) )
!
  ZLSFACT(:) = (XLSTT+(XCPV-XCI)*(ZZT(:)-XTT))/ZZW(:) ! L_s/(Pi_ref*C_ph)
!
  ZZW(:) = EXP( XALPI - XBETAI/ZZT(:) - XGAMI*ALOG(ZZT(:) ) )
  ZSSI(:) = ZRVT(:)*( ZPRES(:)-ZZW(:) ) / ( (XMV/XMD) * ZZW(:) ) - 1.0
                                                       ! Supersaturation over ice
! Distribution parameters for ice and snow
  IF (.NOT. LCRYSTAL_SHAPE) THEN
    ZLBDAI(:) = 1.E10
    WHERE (ZRIT(:) .GT. XRTMIN(4) .AND. ZCIT(:) .GT. XCTMIN(4))
      ZLBDAI(:) = (XLBI * ZCIT(:) / ZRIT(:))**XLBEXI
    END WHERE
  ELSE
    ALLOCATE(ISHAPE_MAX_T(IMICRO)) 
    ALLOCATE(ISHAPE_MAX_S(IMICRO)) 
    ALLOCATE(ZCSHAPE_T(IMICRO,NB_CRYSTAL_SHAPE)) 
    ALLOCATE(ZCSHAPE_S(IMICRO,NB_CRYSTAL_SHAPE))
    ALLOCATE(ZONEOVER_VARS(IMICRO))
    ALLOCATE(ZONEOVER_VART(IMICRO))
!
    ZCSHAPE_T(:,:) = 0.
    ZCSHAPE_S(:,:) = 0.
    ISHAPE_MAX_T(:)    = 0.
    ISHAPE_MAX_S(:)    = 0.
!
! rapport concentration par forme / concentration totale pour chaque forme
    WHERE (ZCIT(:) .GT. 0.0) ZONEOVER_VART(:) = 1.0 / ZCIT(:)
    WHERE (ZCIS(:) .GT. 0.0) ZONEOVER_VARS(:) = 1.0 / ZCIS(:)
    DO JSH = 1, NB_CRYSTAL_SHAPE
      WHERE ((ZCIT(:) .GT. 0.0) .AND. (ZCIT_SHAPE(:,JSH) .GT. 0.0))
        ZCSHAPE_T(:,JSH) = ZCIT_SHAPE(:,JSH) * ZONEOVER_VART(:) 
      END WHERE
      WHERE ((ZCIS(:) .GT. 0.0) .AND. (ZCIS_SHAPE(:,JSH) .GT. 0.0))
        ZCSHAPE_S(:,JSH) = ZCIS_SHAPE(:,JSH) * ZONEOVER_VARS(:)
      END WHERE
    END DO
!
! find the dominant habit per grid mesh
    ISHAPE_MAX_T(:) = MAXLOC(ZCSHAPE_T,DIM=2)
    ISHAPE_MAX_S(:) = MAXLOC(ZCSHAPE_S,DIM=2)
    CALL COMPUTE_LBDA_SHAPE(ISHAPE_MAX_S, IMICRO, PTSTEP,     &
                            ZRIT, ZCIT, ZLBDAI_SHAPE_S, &
                            ZRIS, ZCIS_SHAPE, ZRIS_SHAPE)
    CALL COMPUTE_LBDA_SHAPE(ISHAPE_MAX_T, IMICRO, PTSTEP,     &
                            ZRIT, ZCIT, ZLBDAI_SHAPE_T, &
                            ZRIS, ZCIT_SHAPE, ZRIT_SHAPE)
!
    DEALLOCATE(ZONEOVER_VARS)
    DEALLOCATE(ZONEOVER_VART)
  END IF !  LCRYSTAL_SHAPE
!
  ZLBDAS(:) = 1.E10
  WHERE (ZRST(:) > XRTMIN(5) )
    ZLBDAS(:) = XLBS * (ZRHODREF(:) * ZRST(:))**XLBEXS
  END WHERE
!
  ZKA(:) = 2.38E-2 + 0.0071E-2 * ( ZZT(:) - XTT )          ! k_a
  ZDV(:) = 0.211E-4 * (ZZT(:)/XTT)**1.94 * (XP00/ZPRES(:)) ! D_v
!
! Thermodynamical function ZAI = A_i(T,P)
  ZAI(:) = ( XLSTT + (XCPV-XCI)*(ZZT(:)-XTT) )**2 / (ZKA(:)*XRV*ZZT(:)**2) &
                                        + ( XRV*ZZT(:) ) / (ZDV(:)*ZZW(:))
! ZCJ = c^prime_j/c_i (in the ventilation factor) ( c_i from v(D)=c_i*D^(d_i) )
  ZCJ(:) = XSCFAC * ZRHODREF(:)**0.3 / SQRT( 1.718E-5+0.0049E-5*(ZZT(:)-XTT) )
!
!
!*       2.1    Conversion of snow to r_i: RSCNVI
!        ----------------------------------------
!
  WHERE ( ZRST(:)>XRTMIN(5) )
    ZLBDAS(:)  = MIN( XLBDAS_MAX,                                           &
                      XLBS*( ZRHODREF(:)*MAX( ZRST(:),XRTMIN(5) ) )**XLBEXS )
  END WHERE
  ZZW(:) = 0.0
  IF (.NOT. LCRYSTAL_SHAPE) THEN
    WHERE (ZLBDAS(:)<XLBDASCNVI_MAX .AND. (ZRST(:) > XRTMIN(5)) &
                                    .AND. (ZSSI(:) < 0.0)       &
                                    .AND. (ZRSS(:) > 0.0))
      ZZW(:) = (ZLBDAS(:)*XDSCNVI_LIM)**(XALPHAS)
      ZZX(:) = ( -ZSSI(:)/ZAI(:) ) * (XCCS*ZLBDAS(:)**XCXS) * (ZZW(:)**XNUI) &
                                                            * EXP(-ZZW(:))
!
! Correction BVIE RHODREF
!      ZZW(:) = MIN( ( XR0DEPSI+XR1DEPSI*ZCJ(:) )*ZZX(:)/ZRHODREF(:),ZRSS(:) )
      ZZW(:) = MIN( ( XR0DEPSI+XR1DEPSI*ZCJ(:) )*ZZX(:),ZRSS(:) )
      ZRIS(:) = ZRIS(:) + ZZW(:)
      ZRSS(:) = ZRSS(:) - ZZW(:)
!
      ZZW(:) = ZZW(:)*( XC0DEPSI+XC1DEPSI*ZCJ(:) )/( XR0DEPSI+XR1DEPSI*ZCJ(:) )
      ZCIS(:) = ZCIS(:) + ZZW(:)
    END WHERE
  ELSE
    WHERE (ZLBDAS(:)<XLBDASCNVI_MAX .AND. (ZRST(:) > XRTMIN(5)) &
                                    .AND. (ZSSI(:) < 0.0)       &
                                    .AND. (ZRSS(:) > 0.0))
      ZZW(:) = (ZLBDAS(:)*XDSCNVI_LIM)**(XALPHAS)       ! calcul intermediaire
      ZZX(:) = ( -ZSSI(:)/ZAI(:) ) * (XCCS*ZLBDAS(:)**XCXS) * (ZZW(:)**XNUI) &
                                                            * EXP(-ZZW(:))
!
! Correction BVIE RHODREF
!      ZZW(:) = MIN( ( XR0DEPSI+XR1DEPSI*ZCJ(:) )*ZZX(:)/ZRHODREF(:),ZRSS(:) )
      ZZW(:) = MIN( ( XR0DEPSI+XR1DEPSI*ZCJ(:) )*ZZX(:),ZRSS(:) )
      ZRIS(:) = ZRIS(:) + ZZW(:)
      ZRSS(:) = ZRSS(:) - ZZW(:)
!
      ZZW(:) = ZZW(:) * (XC0DEPSI + XC1DEPSI * ZCJ(:)) / &
              (XR0DEPSI + XR1DEPSI * ZCJ(:))
      ZCIS_SHAPE(:,1) = ZCIS_SHAPE(:,1) + ZZW(:)
    END WHERE
    ZCIS(:) = SUM(ZCIS_SHAPE, DIM=2)
  END IF
!
! Budget storage
  IF (NBUMOD==KMI .AND. LBU_ENABLE) THEN
    IF (LBUDGET_RI) CALL BUDGET (                                          &
               UNPACK(ZRIS(:),MASK=GMICRO(:,:,:),FIELD=PRIS)*PRHODJ(:,:,:),&
                                                            9,'CNVI_BU_RRI')
    IF (LBUDGET_RS) CALL BUDGET (                                          &
               UNPACK(ZRSS(:),MASK=GMICRO(:,:,:),FIELD=PRSS)*PRHODJ(:,:,:),&
                                                            10,'CNVI_BU_RRS')
    IF (LBUDGET_SV) THEN
      IF (.NOT. LCRYSTAL_SHAPE) THEN
        CALL BUDGET (UNPACK(ZCIS(:),MASK=GMICRO(:,:,:),FIELD=PCIS)*PRHODJ(:,:,:), &
                                              12+NSV_LIMA_NI,'CNVI_BU_RSV')
      ELSE
        DO JSH = 1, NB_CRYSTAL_SHAPE
          CALL BUDGET (UNPACK(ZCIS_SHAPE(:,JSH),MASK=GMICRO(:,:,:),&
                       FIELD=PCIS_SHAPE(:,:,:,JSH))*PRHODJ(:,:,:), &
                       12+NSV_LIMA_NI+JSH-1,'CNVI_BU_RSV')
        END DO
      END IF
    END IF
  END IF
!
!
!*       2.2    Deposition of water vapor on r_s: RVDEPS
!        -----------------------------------------------
!
!
  ZZW(:) = 0.0
  WHERE ( (ZRST(:)>XRTMIN(5)) .AND. (ZRSS(:)>ZRTMIN(5)) )
!Correction BVIE rhodref
!         ZZW(:) = ( ZSSI(:)/(ZRHODREF(:)*ZAI(:)) ) *                               &
    ZZW(:) = ( ZSSI(:)/(ZAI(:)) ) *                               &
             ( X0DEPS*ZLBDAS(:)**XEX0DEPS + X1DEPS*ZCJ(:)*ZLBDAS(:)**XEX1DEPS )
    ZZW(:) =    MIN( ZRVS(:),ZZW(:)      )*(0.5+SIGN(0.5,ZZW(:))) &
              - MIN( ZRSS(:),ABS(ZZW(:)) )*(0.5-SIGN(0.5,ZZW(:)))
    ZRSS(:) = ZRSS(:) + ZZW(:)
    ZRVS(:) = ZRVS(:) - ZZW(:)
    ZTHS(:) = ZTHS(:) + ZZW(:)*ZLSFACT(:)
  END WHERE
!
! Budget storage
  IF (NBUMOD==KMI .AND. LBU_ENABLE) THEN
    IF (LBUDGET_TH) CALL BUDGET (                                                 &
               UNPACK(ZTHS(:),MASK=GMICRO(:,:,:),FIELD=PTHS)*PRHODJ(:,:,:),&
                                                            4,'DEPS_BU_RTH')
    IF (LBUDGET_RV) CALL BUDGET (                                                 &
               UNPACK(ZRVS(:),MASK=GMICRO(:,:,:),FIELD=PRVS)*PRHODJ(:,:,:),&
                                                            6,'DEPS_BU_RRV')
    IF (LBUDGET_RS) CALL BUDGET (                                                 &
               UNPACK(ZRSS(:),MASK=GMICRO(:,:,:),FIELD=PRSS)*PRHODJ(:,:,:),&
                                                           10,'DEPS_BU_RRS')
  END IF
!
!
!*       2.3    Conversion of pristine ice to r_s: RICNVS
!        ------------------------------------------------
!
!
  ZZW(:) = 0.0
! note : Souvent constitue de plates et dendrites : conditions de sat proche
! de l'eau. Raremennt colonnes et plates epaisses : conditions de sat de la
! glace, inferieure a celle de l'eau. A rajouter en condition.
! Determiner les valeurs de sursat entre plates et columns, pour rajouter dans
! le code.
  IF (.NOT. LCRYSTAL_SHAPE) THEN
    WHERE ((ZLBDAI(:) < XLBDAICNVS_LIM) .AND. (ZCIT(:) > XCTMIN(4)) &
                                        .AND. (ZSSI(:) > 0.0) &
                                        .AND. (ZRIS(:) > 0.0) &
                                        .AND. (ZCIS(:) > 0.0))
      ZZW(:) = (ZLBDAI(:) * XDICNVS_LIM)**(XALPHAI)
      ZZX(:) = (ZSSI(:) / ZAI(:)) * ZCIT(:) * (ZZW(:)**XNUI) * EXP(-ZZW(:))
!
! Correction BVIE
!     ZZW(:) = MAX( MIN( (XR0DEPIS + XR1DEPIS*ZCJ(:))*ZZX(:)/ZRHODREF(:) &
      ZZW(:) = MAX( MIN( (XR0DEPIS + XR1DEPIS*ZCJ(:))*ZZX(:),ZRIS(:) ) + &
                    ZRTMIN(5), ZRTMIN(5) ) - ZRTMIN(5)
      ZRIS(:) = ZRIS(:) - ZZW(:)
      ZRSS(:) = ZRSS(:) + ZZW(:)
!
      ZZW(:) = MIN( ZZW(:)*(( XC0DEPIS+XC1DEPIS*ZCJ(:) )                   &
                           /( XR0DEPIS+XR1DEPIS*ZCJ(:) )),ZCIS(:) )
      ZCIS(:) = ZCIS(:) - ZZW(:)
    END WHERE
  ELSE
    ALLOCATE(ZZW_2D(IMICRO,NB_CRYSTAL_SHAPE))
    ALLOCATE(ZZW_2D_MR(IMICRO,NB_CRYSTAL_SHAPE))
    ALLOCATE(ZZX_2D(IMICRO,NB_CRYSTAL_SHAPE))
    ZZW_2D(:,:) = 0.
    ZZW_2D_MR(:,:) = 0.
    ZZX_2D(:,:) = 0.
    DO JSH = 1, NB_CRYSTAL_SHAPE
      ZZW(:) = 0.
! question : Avec les calculs suivants, chaque forme s'aggrege avec elle
! meme mais pas d'aggregation entre les differentes formes
      WHERE ((ZLBDAI_SHAPE_T(:,JSH) < XLBDAICNVS_LIM) .AND. (ZCIT_SHAPE(:,JSH) > XCTMIN(4)) &
                                                      .AND. (ZSSI(:) > 0.0)                   &
                                                      .AND. (ZRIS_SHAPE(:,JSH) > 0.0)       &
                                                      .AND. (ZCIS_SHAPE(:,JSH) > 0.0 ))
        ZZW(:) = (ZLBDAI_SHAPE_T(:,JSH) * XDICNVS_LIM)**(XALPHAI)
        ZZX_2D(:,JSH) = (ZSSI(:) / ZAI(:)) * ZCIT_SHAPE(:,JSH) * (ZZW(:)**XNUI) * EXP(-ZZW(:))
        ZZW_2D_MR(:,JSH) = MAX(MIN((XR0DEPIS_SHAPE(JSH) + XR1DEPIS_SHAPE(JSH)*ZCJ(:))*ZZX_2D(:,JSH) &
                        ,ZRIS_SHAPE(:,JSH)) + ZRTMIN(5), ZRTMIN(5)) - ZRTMIN(5)
!
!
        ZZW_2D(:,JSH) = MIN(ZZW_2D_MR(:,JSH)*((XC0DEPIS_SHAPE(JSH)+XC1DEPIS_SHAPE(JSH)*ZCJ(:)) &
                                          /(XR0DEPIS_SHAPE(JSH)+XR1DEPIS_SHAPE(JSH)*ZCJ(:))),&
                            ZCIS_SHAPE(:,JSH))
        ZCIS_SHAPE(:,JSH) = ZCIS_SHAPE(:,JSH) - ZZW_2D(:,JSH)
      END WHERE
    END DO   ! JSH
    ZRIS(:) = ZRIS(:) - SUM(ZZW_2D_MR, DIM=2)
    ZRSS(:) = ZRSS(:) + SUM(ZZW_2D_MR, DIM=2)
    ZCIS(:) = SUM(ZCIS_SHAPE,DIM = 2)
  END IF
!
! Budget storage
  IF (NBUMOD==KMI .AND. LBU_ENABLE) THEN
    IF (LBUDGET_RI) CALL BUDGET (                                          &
               UNPACK(ZRIS(:),MASK=GMICRO(:,:,:),FIELD=PRIS)*PRHODJ(:,:,:),&
                                                            9,'CNVS_BU_RRI')
    IF (LBUDGET_RS) CALL BUDGET (                                          &
               UNPACK(ZRSS(:),MASK=GMICRO(:,:,:),FIELD=PRSS)*PRHODJ(:,:,:),&
                                                           10,'CNVS_BU_RRS')
    IF (LBUDGET_SV) THEN 
      IF (.NOT. LCRYSTAL_SHAPE) THEN
        CALL BUDGET (UNPACK(ZCIS(:),MASK=GMICRO(:,:,:),FIELD=PCIS)*PRHODJ(:,:,:), &
                                12+NSV_LIMA_NI,'CNVS_BU_RSV')
      ELSE IF (LCRYSTAL_SHAPE) THEN
        DO JSH = 1, NB_CRYSTAL_SHAPE
          CALL BUDGET (UNPACK(ZCIS_SHAPE(:,JSH),MASK=GMICRO(:,:,:),&
                         FIELD=PCIS_SHAPE(:,:,:,JSH))*PRHODJ(:,:,:), &
                                12+NSV_LIMA_NI+JSH-1,'CNVS_BU_RSV')
        END DO
      END IF
    END IF
  END IF
!
!
!*       2.4    Aggregation of r_i on r_s: CIAGGS and RIAGGS
!        ---------------------------------------------------
!
  IF (.NOT. LCRYSTAL_SHAPE) THEN
    WHERE ( (ZRIT(:)>XRTMIN(4)) .AND. (ZRST(:)>XRTMIN(5)) .AND. (ZRIS(:)>ZRTMIN(4)) & 
                                                          .AND. (ZCIS(:)>ZCTMIN(4)) )
!
      ZZW1(:,3) = (ZLBDAI(:) / ZLBDAS(:))**3
      ZZW1(:,1) = (ZCIT(:)*(XCCS*ZLBDAS(:)**XCXS)*EXP( XCOLEXIS*(ZZT(:)-XTT) )) &
                                                  / (ZLBDAI(:)**3)
      ZZW1(:,2) = MIN( ZZW1(:,1)*(XAGGS_CLARGE1+XAGGS_CLARGE2*ZZW1(:,3)),ZCIS(:) )
      ZCIS(:) = ZCIS(:) - ZZW1(:,2)
!
      ZZW1(:,1) = ZZW1(:,1) / ZLBDAI(:)**XBI
      ZZW1(:,2) = MIN( ZZW1(:,1)*(XAGGS_RLARGE1+XAGGS_RLARGE2*ZZW1(:,3)),ZRIS(:) )
      ZRIS(:) = ZRIS(:) - ZZW1(:,2)
      ZRSS(:) = ZRSS(:) + ZZW1(:,2)
    END WHERE
  ELSE
    ALLOCATE(ZZW1_2D(IMICRO,NB_CRYSTAL_SHAPE))
    ZZW1_2D(:,:) = 0.0
    DO JSH = 1, NB_CRYSTAL_SHAPE
      WHERE ((ZRIT(:) > XRTMIN(4))       .AND. (ZRST(:) > XRTMIN(5)) .AND. &
             (ZRIS_SHAPE(:,JSH) > 0.0) .AND. (ZCIS_SHAPE(:,JSH) > 0.0))
!
        ZZW1(:,3) = (ZLBDAI_SHAPE_T(:,JSH) / ZLBDAS(:))**3
        ZZW1(:,1) = (ZCIT_SHAPE(:,JSH)*(XCCS*ZLBDAS(:)**XCXS)*EXP( XCOLEXIS*(ZZT(:)-XTT) )) &
                                                     / (ZLBDAI_SHAPE_T(:,JSH)**3)
        ZZW1(:,2) = MIN( ZZW1(:,1)*(XAGGS_CLARGE1+XAGGS_CLARGE2*ZZW1(:,3)),ZCIS_SHAPE(:,JSH) )
        ZCIS_SHAPE(:,JSH) = ZCIS_SHAPE(:,JSH) - ZZW1(:,2)
!
        ZZW1(:,1) = ZZW1(:,1) / ZLBDAI_SHAPE_T(:,JSH)**XBI_SHAPE(JSH)
        ZZW1_2D(:,JSH) = MIN( ZZW1(:,1)*(XAGGS_RLARGE1_SHAPE(JSH)+XAGGS_RLARGE2_SHAPE(JSH)*ZZW1(:,3)),ZRIS_SHAPE(:,JSH) )
      END WHERE
    END DO
    ZZW1(:,2) = 0.
!++cb++ 31/03/20
    ZZW1(:,2) = MIN(SUM(ZZW1_2D,DIM=2), ZRIS(:))
    WHERE ((ZRIT(:) > XRTMIN(4)) .AND. (ZRST(:) > XRTMIN(5)) .AND. &
           (ZRIS(:) > 0.0)       .AND. (ZCIS(:) > 0.0))
      ZRIS(:) = ZRIS(:) - ZZW1(:,2) !SUM(ZZW1_2D,DIM=2)
      ZRSS(:) = ZRSS(:) + ZZW1(:,2) !SUM(ZZW1_2D,DIM=2)
!--cb--
      ZCIS(:) = SUM(ZCIS_SHAPE,DIM = 2)
    END WHERE
    DEALLOCATE(ZZW1_2D)
  END IF  ! LCRYSTAL_SHAPE
!
! Budget storage
  IF (NBUMOD==KMI .AND. LBU_ENABLE) THEN
    IF (LBUDGET_RI) CALL BUDGET (                                               &
                   UNPACK(ZRIS(:),MASK=GMICRO(:,:,:),FIELD=PRIS)*PRHODJ(:,:,:), &
                                                            9,'AGGS_BU_RRI')
    IF (LBUDGET_RS) CALL BUDGET (                                               &
                   UNPACK(ZRSS(:),MASK=GMICRO(:,:,:),FIELD=PRSS)*PRHODJ(:,:,:), &
                                                           10,'AGGS_BU_RRS')
    IF (LBUDGET_SV) THEN
      IF (.NOT. LCRYSTAL_SHAPE) THEN
        CALL BUDGET (UNPACK(ZCIS(:),MASK=GMICRO(:,:,:), &
                            FIELD=PCIS)*PRHODJ(:,:,:), &
                            12+NSV_LIMA_NI,'AGGS_BU_RSV')
      ELSE
        DO JSH = 1, NB_CRYSTAL_SHAPE
          CALL BUDGET (UNPACK(ZCIS_SHAPE(:,JSH),MASK=GMICRO(:,:,:),&
                       FIELD=PCIS_SHAPE(:,:,:,JSH))*PRHODJ(:,:,:), &
                       12+NSV_LIMA_NI+JSH-1,'AGGS_BU_RSV')
        END DO
      END IF
    END IF
  END IF
!
!
!------------------------------------------------------------------------------
!
!
!*       3.    Unpacking & Deallocating
!              ------------------------
!
! 
!
  ZW(:,:,:) = PRVS(:,:,:)
  PRVS(:,:,:) = UNPACK( ZRVS(:),MASK=GMICRO(:,:,:),FIELD=ZW(:,:,:) )
  ZW(:,:,:) = PRIS(:,:,:)
  PRIS(:,:,:) = UNPACK( ZRIS(:),MASK=GMICRO(:,:,:),FIELD=ZW(:,:,:) )
  ZW(:,:,:) = PRSS(:,:,:)
  PRSS(:,:,:) = UNPACK( ZRSS(:),MASK=GMICRO(:,:,:),FIELD=ZW(:,:,:) )
!
  IF (.NOT. LCRYSTAL_SHAPE) THEN
    ZW(:,:,:) = PCIS(:,:,:)
    PCIS(:,:,:) = UNPACK( ZCIS(:),MASK=GMICRO(:,:,:),FIELD=ZW(:,:,:) )
  ELSE
    ALLOCATE(ZW_4D(SIZE(PRHODREF,1),SIZE(PRHODREF,2),SIZE(PRHODREF,3),NB_CRYSTAL_SHAPE))
    DO JSH = 1, NB_CRYSTAL_SHAPE
      ZW_4D(:,:,:,JSH) = PCIS_SHAPE(:,:,:,JSH)
      PCIS_SHAPE(:,:,:,JSH) = UNPACK( ZCIS_SHAPE(:,JSH),MASK=GMICRO(:,:,:),FIELD=ZW_4D(:,:,:,JSH) )
    END DO
    PCIS(:,:,:) = SUM(PCIS_SHAPE, DIM=4)
  END IF
!
  ZW(:,:,:) = PTHS(:,:,:)
  PTHS(:,:,:) = UNPACK( ZTHS(:),MASK=GMICRO(:,:,:),FIELD=ZW(:,:,:) )
!
  DEALLOCATE(ZRVT) 
  DEALLOCATE(ZRCT) 
  DEALLOCATE(ZRRT) 
  DEALLOCATE(ZRIT) 
  DEALLOCATE(ZRST) 
  DEALLOCATE(ZRGT) 
  DEALLOCATE(ZRVS)  
  DEALLOCATE(ZRIS)
  DEALLOCATE(ZRSS)
  DEALLOCATE(ZTHS)
  DEALLOCATE(ZCIS)
  DEALLOCATE(ZCIT)
  IF (LCRYSTAL_SHAPE) THEN
    DEALLOCATE(ZCIS_SHAPE)
    DEALLOCATE(ZCIT_SHAPE)
    DEALLOCATE(ZCSHAPE_T)
    DEALLOCATE(ZCSHAPE_S)
    DEALLOCATE(ZW_4D) 
    DEALLOCATE(ISHAPE_MAX_S) 
    DEALLOCATE(ISHAPE_MAX_T) 
    DEALLOCATE(ZZW_2D)
    DEALLOCATE(ZZW_2D_MR)
    DEALLOCATE(ZZX_2D)
    DEALLOCATE(ZLBDAI_SHAPE_T)
    DEALLOCATE(ZLBDAI_SHAPE_S)
    DEALLOCATE(ZRIT_SHAPE)
    DEALLOCATE(ZRIS_SHAPE)
  END IF
  DEALLOCATE(ZRHODREF) 
  DEALLOCATE(ZZT) 
  DEALLOCATE(ZPRES) 
  DEALLOCATE(ZEXNREF)
  DEALLOCATE(ZZW)
  DEALLOCATE(ZZX)
  DEALLOCATE(ZLSFACT)
  DEALLOCATE(ZSSI)
  DEALLOCATE(ZLBDAI) 
  DEALLOCATE(ZLBDAS)
  DEALLOCATE(ZAI)
  DEALLOCATE(ZCJ)
  DEALLOCATE(ZKA)
  DEALLOCATE(ZDV)
  DEALLOCATE(ZZW1)
  IF (NBUMOD==KMI .AND. LBU_ENABLE) DEALLOCATE(ZRHODJ)
!
ELSE
!
! Advance the budget calls
!
  IF (NBUMOD==KMI .AND. LBU_ENABLE) THEN
    IF (LBUDGET_TH) THEN
      ZW(:,:,:) = PTHS(:,:,:)*PRHODJ(:,:,:)
      CALL BUDGET (ZW,4,'DEPS_BU_RTH')
    ENDIF
    IF (LBUDGET_RV) THEN
      ZW(:,:,:) = PRVS(:,:,:)*PRHODJ(:,:,:)
      CALL BUDGET (ZW,6,'DEPS_BU_RRV')
    ENDIF
    IF (LBUDGET_RI) THEN
      ZW(:,:,:) = PRIS(:,:,:)*PRHODJ(:,:,:)
      CALL BUDGET (ZW,9,'CNVI_BU_RRI')
      CALL BUDGET (ZW,9,'CNVS_BU_RRI')
      CALL BUDGET (ZW,9,'AGGS_BU_RRI')
    ENDIF
    IF (LBUDGET_RS) THEN
      ZW(:,:,:) = PRSS(:,:,:)*PRHODJ(:,:,:)
      CALL BUDGET (ZW,10,'CNVI_BU_RRS')
      CALL BUDGET (ZW,10,'DEPS_BU_RRS')
      CALL BUDGET (ZW,10,'CNVS_BU_RRS')
      CALL BUDGET (ZW,10,'AGGS_BU_RRS')
    ENDIF
    IF (LBUDGET_SV) THEN
      IF (.NOT. LCRYSTAL_SHAPE) THEN
        ZW(:,:,:) = PCIS(:,:,:)*PRHODJ(:,:,:)
        CALL BUDGET (ZW,12+NSV_LIMA_NI,'CNVI_BU_RSV')
        CALL BUDGET (ZW,12+NSV_LIMA_NI,'CNVS_BU_RSV')
        CALL BUDGET (ZW,12+NSV_LIMA_NI,'AGGS_BU_RSV')
      ELSE IF (LCRYSTAL_SHAPE) THEN
         DO JSH=1, NB_CRYSTAL_SHAPE
            ZW(:,:,:) = PCIS_SHAPE(:,:,:,JSH)*PRHODJ(:,:,:)
            CALL BUDGET (ZW,12+NSV_LIMA_NI+JSH-1,'CNVI_BU_RSV')
            CALL BUDGET (ZW,12+NSV_LIMA_NI+JSH-1,'CNVS_BU_RSV')
            CALL BUDGET (ZW,12+NSV_LIMA_NI+JSH-1,'AGGS_BU_RSV')
          END DO
       END IF
    ENDIF  !lbudget_sv
  ENDIF    !nbumod...
END IF
!
DEALLOCATE(ZRTMIN)
DEALLOCATE(ZCTMIN)
!
END SUBROUTINE LIMA_COLD_SLOW_PROCESSES
