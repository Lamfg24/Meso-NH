...
!LOCAL VARIABLES
REAL,DIMENSION(:,:), ALLOCATABLE  :: ZSFSLT_MDE ! sea salt flux from modes
...
END IF
!
IF (LEMISSALT2LIMA) THEN  ! case of sea salt emssion direct to lima CCN free

  IF (CEMISPARAM_SLT .eq. "Ova14") THEN ! Rajouter Ova14 dans fichier initialisation
  PSFSLT(:,KSLT)  = XFRAC2LIMA1* ZSFSLT_MDE(:,1) +&
                 XFRAC2LIMA2* ZSFSLT_MDE(:,2) +&
                 XFRAC2LIMA3* ZSFSLT_MDE(:,3) +&
                 XFRAC2LIMA4* ZSFSLT_MDE(:,4) +&
                 XFRAC2LIMA5* ZSFSLT_MDE(:,5) 
  ELSEIF ((CEMISPARAM_SLT .eq. "Vig01").OR.(CEMISPARAM_SLT .eq. "Sch04")) THEN
  PSFSLT(:,KSLT)  = XFRAC2LIMA3* ZSFSLT_MDE(:,1) +&
                 XFRAC2LIMA4* ZSFSLT_MDE(:,2) +&
                 XFRAC2LIMA5* ZSFSLT_MDE(:,3) 
  END IF

ELSE ! case of orilam use
