...
USE MODD_CH_M9_SCHEME,    ONLY : JP_SO2
...
USE MODD_PARAM_LIMA,      ONLY : NMOD_CCN, NMOD_IFN, NMOD_IMM, NINDICE_CCN_IMM,&
                                 ...
                                 XR_MEAN_CCN, XLOGSIG_CCN ,NB_CRYSTAL_SHAPE, &
                                 LCRYSTAL_SHAPE
USE MODD_PARAM_LIMA_WARM, ONLY : CLIMA_WARM_CONC, CAERO_MASS
USE MODD_PARAM_LIMA_COLD, ONLY : CLIMA_COLD_CONC, CLIMA_SHAPE_CONC
...
REAL,DIMENSION(SIZE(XTHT,1),SIZE(XTHT,2))               :: ZTOPSO2
...
REAL,DIMENSION(:,:,:),   ALLOCATABLE                    :: ZCIT_TOT
...
!
      IF (LUSECI) THEN
        ...
        IF (.NOT. LCRYSTAL_SHAPE) THEN
          CALL IO_WRITE_FIELD(TPFILE,'CIT',XSVT(:,:,:,NSV_LIMA_NI))
        ELSE
          DO JSH = 1, NB_CRYSTAL_SHAPE
            WRITE(INDICE,'(I2.2)')(JSH)
            TZFIELD%CMNHNAME   = TRIM('CIT'//INDICE)
            ZWORK31(:,:,:) = XSVT(:,:,:,NSV_LIMA_NI+JSH-1)
            CALL IO_WRITE_FIELD(TPFILE,TZFIELD,ZWORK31)
          END DO
        END IF
...
! Ni
    ...
    ELSE
      IF (JSV .GE. NSV_LIMA_NI .AND. JSV .LT. NSV_LIMA_NI+NB_CRYSTAL_SHAPE) THEN
        WRITE(INDICE,'(I2.2)')(JSV - NSV_LIMA_NI + 1)
        TZFIELD%CMNHNAME = TRIM(CLIMA_SHAPE_CONC(JSV-NSV_LIMA_NI+1))//INDICE//'T'
      END IF
    END IF
! N IFN free
...
IF (LCHEMDIAG) THEN
  DO JSV = NSV_CHGSBEG,NSV_CHGSEND
    ...
    ZWORK31(:,:,:)= XSVT(:,:,:,JSV) *&
                   (XSREALMASSMOLVAL(JSV-NSV_CHGSBEG+1)/XMD)*&
                   1E-3*XRHODREF(:,:,:) ! kg / m3
    ...
    DO JK=IKB,IKE
      DO JJ=IJB,IJE
        DO JI=IIB,IIE
           ZWORK21(JI,JJ)=ZWORK21(JI,JJ)+ZWORK31(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO

    TZFIELD%CMNHNAME='BURN'//TRIM(UPCASE(CNAMES(JSV-NSV_CHGSBEG+1)))
    TZFIELD%CUNITS     = 'kg/m2'
    WRITE(TZFIELD%CCOMMENT,'(A6,A4,I3.3)')'X_Y_Z_','BURN',JSV
    CALL IO_WRITE_FIELD(TPFILE,TZFIELD,ZWORK21)

   ! ZWORK21(:,:) = ZWORK21(:,:) * 6.022E23 / (XSREALMASSMOLVAL(JSV)*1E-3*2.69E20)
    ZWORK21(:,:) = ZWORK21(:,:) * 1E3  / 0.0285

    TZFIELD%CMNHNAME='DOBS'//TRIM(UPCASE(CNAMES(JSV-NSV_CHGSBEG+1)))
    TZFIELD%CUNITS     = 'Dobson Unit'
    WRITE(TZFIELD%CCOMMENT,'(A6,A4,I3.3)')'X_Y_Z_','DOBS',JSV
    CALL IO_WRITE_FIELD(TPFILE,TZFIELD,ZWORK21)
  END DO
    ZTOPSO2(:,:) = 0.
    DO JK=IKB,IKE
      DO JJ=IJB,IJE
        DO JI=IIB,IIE
           IF (XSVT(JI,JJ,JK,NSV_CHGSBEG+JP_SO2-1) .GT. 20E-9) ZTOPSO2(JI,JJ)= XZZ(JI,JJ,JK)
        ENDDO
      ENDDO
    ENDDO
    TZFIELD%CMNHNAME='TOPSO2'
    WRITE(TZFIELD%CCOMMENT,'(A6,A6)')'X_Y_Z_','TOPSO2'
    TZFIELD%CUNITS     = 'm'
    CALL IO_WRITE_FIELD(TPFILE,TZFIELD,ZTOPSO2)
...
! CASE  PREP_REAL_CASE after arome
  ...
  ELSE IF (CCLOUD=='LIMA') THEN
    IF (.NOT. LCRYSTAL_SHAPE) THEN
      ...
      CALL INI_RADAR('PLAT')
    ELSE
      ALLOCATE(ZCIT_SHAPE(IIU,IJU,IKU, NB_CRYSTAL_SHAPE))
      ALLOCATE(XCIT(IIU,IJU,IKU))
      DO JSH= 1, NB_CRYSTAL_SHAPE
        ZCIT_SHAPE(:,:,:,JSH) = XSVT(:,:,:,NSV_LIMA_NI+JSH-1)
      END DO
      XCIT(:,:,:) = SUM(ZCIT_SHAPE,DIM=4)
      CALL INI_RADAR('PLAT')
    END IF
  END IF
...
! original version of radar diagnostics 
    ...
    IF (CCLOUD=='LIMA') THEN
      IF (LCRYSTAL_SHAPE) THEN
        CALL RADAR_RAIN_ICE (XRT, ZCIT_SHAPE, XRHODREF, ZTEMP, ZWORK31, ZWORK32, &
                             ZWORK33, ZWORK34,XSVT(:,:,:,NSV_LIMA_NR) )
      END IF
      IF (ALLOCATED(ZCIT_SHAPE)) DEALLOCATE(ZCIT_SHAPE)
    ELSE
      ALLOCATE(ZAUX(SIZE(XCIT,1),SIZE(XCIT,2),SIZE(XCIT,3),1))
      ZAUX(:,:,:,1) = XCIT(:,:,:)
      CALL RADAR_RAIN_ICE (XRT, ZAUX, XRHODREF, ZTEMP, ZWORK31, ZWORK32, &
                                                       ZWORK33, ZWORK34 )
      DEALLOCATE(ZAUX)
  ENDIF
!
