...
! concentration décroissante selon z
    DO JSV = 1, NMOD_CCN
        !PSVT(:,:,:,NSV_LIMA_CCN_FREE+JSV-1) = XCCN_CONC(JSV)*1.0E6 &
         !        / PRHODREF(:,:,:) * EXP(-LOG(XCCN_CONC(JSV))*PZZ(:,:,:)/7000.)
      WHERE (PZZ(:,:,IKB:IKE) .LE. 1000.) 
         PSVT(:,:,IKB:IKE,NSV_LIMA_CCN_FREE+JSV-1) = XCCN_CONC(JSV)*1.0E6 / PRHODREF(:,:,IKB:IKE)
      ELSEWHERE (PZZ(:,:,IKB:IKE) .LE. 10000.)
         PSVT(:,:,IKB:IKE,NSV_LIMA_CCN_FREE+JSV-1) = XCCN_CONC(JSV)*1.0E6 &
                 / PRHODREF(:,:,IKB:IKE) * EXP(-LOG(XCCN_CONC(JSV)/0.01)*PZZ(:,:,IKB:IKE)/10000.)
      ELSEWHERE
         PSVT(:,:,IKB:IKE,NSV_LIMA_CCN_FREE+JSV-1) = 0.01*1.0E6 / PRHODREF(:,:,IKB:IKE)
      ENDWHERE
    END DO
  ENDIF
