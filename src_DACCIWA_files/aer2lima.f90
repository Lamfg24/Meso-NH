...
!
!
!!****  *AER2LIMA* allocates lima microphysics scheme in case of orilam aerosols
!!
!!    PURPOSE
!!    -------
!!
...
!
ZSUM(:,:,:) = PSVT(:,:,:,NSV_LIMA_CCN_FREE)+PSVT(:,:,:,NSV_LIMA_CCN_FREE+1)+PSVT(:,:,:,NSV_LIMA_CCN_FREE+2)
...
!
IF (LORILAM) THEN
!  IF (NMOD_CCN .GE. 2) ZCCN_SUM(:,:,:,2) = 0.
!  IF (NMOD_CCN .GE. 3) ZCCN_SUM(:,:,:,3) = 0.
!  IF (.NOT.(LDUST))    ZIFN_SUM(:,:,:,1) = 0.
!  IF (NMOD_IFN .GE. 2) ZIFN_SUM(:,:,:,2) = 0.
...
!DO JK=1,SIZE(PSVT,3)
!print*,'ZRH ',JK,' =',MINVAL(ZRH(:,:,JK)), MAXVAL(ZRH(:,:,JK))
!ENDDO
...
! conversion ppp to µg/m3
  ZAER(:,:,:,2)=PSVT(:,:,:,NSV_CHEMBEG-1+JP_CH_NH3)*XNH3*1E-3*PRHODREF(:,:,:)/XMD
! conversion ppp to µg/m3
  ZAER(:,:,:,3)=PSVT(:,:,:,NSV_CHEMBEG-1+JP_CH_HNO3)*XHNO3*1E-3*PRHODREF(:,:,:)/XMD
  ZAER(:,:,:,4)=ZTOT(:,:,:,JP_AER_H2O)
  ZAER(:,:,:,5)=ZTOT(:,:,:,JP_AER_NO3)
  ZAER(:,:,:,6)=ZTOT(:,:,:,JP_AER_NH3)
  ZAER(:,:,:,:)=MAX(ZAER(:,:,:,:),0.)
!  DO JSV=1,6
!  print*,'entree ZAER',JSV,' =',MINVAL(ZAER(:,:,:,JSV)), MAXVAL(ZAER(:,:,:,JSV))
!  END DO
...
!  DO JSV=1,6
!  print*,'sortie ZAER',JSV,' =',MINVAL(ZAER(:,:,:,JSV)), MAXVAL(ZAER(:,:,:,JSV))
!  END DO
...
! Balance the mass according to size
  ...
  DO JSV=1,JPMODE
    ...
    END DO
  END DO
  PSVT(:,:,:,NSV_AERBEG-1+JP_CH_NH3i) = ZCTOTA(:,:,:,JP_AER_NH3,1) *1E-9 * XMD / &
                                        (PRHODREF(:,:,:)*ZMI(JP_AER_NH3)*1E-3)
  PSVT(:,:,:,NSV_AERBEG-1+JP_CH_NH3j) = ZCTOTA(:,:,:,JP_AER_NH3,2) *1E-9 * XMD / &
                                        (PRHODREF(:,:,:)*ZMI(JP_AER_NH3)*1E-3)
  PSVT(:,:,:,NSV_AERBEG-1+JP_CH_NO3i) = ZCTOTA(:,:,:,JP_AER_NO3,1) *1E-9 * XMD / &
                                        (PRHODREF(:,:,:)*ZMI(JP_AER_NO3)*1E-3)
  PSVT(:,:,:,NSV_AERBEG-1+JP_CH_NO3j) = ZCTOTA(:,:,:,JP_AER_NO3,2) *1E-9 * XMD / &
                                        (PRHODREF(:,:,:)*ZMI(JP_AER_NO3)*1E-3)
  PSVT(:,:,:,NSV_AERBEG-1+JP_CH_SO4i) = ZCTOTA(:,:,:,JP_AER_SO4,1) *1E-9 * XMD / &
                                        (PRHODREF(:,:,:)*ZMI(JP_AER_SO4)*1E-3)
  PSVT(:,:,:,NSV_AERBEG-1+JP_CH_SO4j) = ZCTOTA(:,:,:,JP_AER_SO4,2) *1E-9 * XMD / &
                                        (PRHODREF(:,:,:)*ZMI(JP_AER_SO4)*1E-3)
  PSVT(:,:,:,NSV_AERBEG-1+JP_CH_H2Oi) = ZCTOTA(:,:,:,JP_AER_H2O,1) *1E-9 * XMD / &
                                        (PRHODREF(:,:,:)*ZMI(JP_AER_H2O)*1E-3)
  PSVT(:,:,:,NSV_AERBEG-1+JP_CH_H2Oj) = ZCTOTA(:,:,:,JP_AER_H2O,2) *1E-9 * XMD / &
                                        (PRHODREF(:,:,:)*ZMI(JP_AER_H2O)*1E-3)
...
! aerosol radius greater than 0.1 µm to be considers as CCN 
! water mass greater than 20%
...
ELSE
  IF (TRIM(CDUMMY1) == "DACCIWA") THEN
     ZCCN_SUM(:,:,:,JSV) = ZCCN_SUM(:,:,:,JSV) + ZN0_AER(:,:,:,JSV) 
  ELSE
  ! Sulfates 
...
! IFN_FREE initialization
  WHERE (ZRATH2O(:,:,:) .LT. ZRATMASSH2O) ! fraction of dust if low water
     ZIFN_SUM(:,:,:,1) = ZIFN_SUM(:,:,:,1) + ZN0_AER(:,:,:,JSV) * ZRATDST(:,:,:)
  END WHERE

! hydrophobic aerosols water mass less than 20%
  IF (NMOD_IFN .GE. 2) THEN
   WHERE (ZRATH2O(:,:,:) .LT. ZRATMASSH2O) ! hydrophobic aerosols can act as IFN
...
IF (LSALT) THEN
!
  IF (((CPROGRAM=="REAL  ").AND.(LSLTINIT).AND.(.NOT.LSLTMACC)).OR.(CPROGRAM=="IDEAL ")) THEN
...
!
  DO JSV=1,NMODE_SLT
  !print*,'SLT ',JSV,' =',MINVAL(PSVT(:,:,:,NSV_SLTBEG:NSV_SLTEND)), MAXVAL(PSVT(:,:,:,NSV_SLTBEG:NSV_SLTEND))
...
IF (TRIM(CDUMMY1) /= "DACCIWA") ZCCN_SUM(:,:,:,1) = PSVT(:,:,:,NSV_LIMA_CCN_FREE) + PSVT(:,:,:,NSV_LIMA_CCN_ACTI)
...
IF (LDUST) THEN
  ...
  IF (((CPROGRAM=="REAL  ").AND.(LDSTINIT).AND.(.NOT.LDSTMACC)).OR.(CPROGRAM=="IDEAL ")) THEN
