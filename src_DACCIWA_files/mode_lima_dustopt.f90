...
!-----------------------------------------------------------------
! $Source: /home/cvsroot/MNH-VX-Y-Z/src/MNH/mode_dustopt.f90,v $ $Revision: 1.1.2.2.2.1.2.1 $
! masdev4_7 BUG1 2007/06/29 12:06:27
!-----------------------------------------------------------------
!        ###################
         MODULE MODE_LIMA_DUSTOPT
!        ###################
!
!!
!!    PURPOSE
...
!!     July 2011    C.Lac : Bug on ZEXT_COEFF_ vertical levels 
!!
...
!  
! *********************************************************************
  SUBROUTINE LIMA_DUSTOPT_GET(PSVT,PZZ,PRHODREF,                      &
...
! Arguments
!
REAL,    DIMENSION(:,:,:,:), INTENT(IN)    :: PSVT        ! [moments/molec_{air}] transported 
                                                          ! moments aerosol
REAL,    DIMENSION(:,:,:),   INTENT(IN)    :: PZZ         ! [m] height of layers
REAL,    DIMENSION(:,:,:),   INTENT(IN)    :: PRHODREF    ! [kg/m3] density of air
REAL,    DIMENSION(:,:,:,:), INTENT(INOUT) :: PPIZA_WVL   ! [-] single scattering albedo aerosol 
                                                          ! layer for all SW wavelengths
REAL,    DIMENSION(:,:,:,:), INTENT(INOUT) :: PCGA_WVL    ! [-] assymetry factor faerosol layer 
                                                          ! for all SW wavelengths
REAL,    DIMENSION(:,:,:,:), INTENT(INOUT) :: PTAUREL_WVL ! [-] opt.depth/opt.depth(550) faerosol 
                                                          ! layer for all SW wvl 
...
REAL, ALLOCATABLE, DIMENSION(:,:,:,:)   :: ZRADIUS       ![um] number median radius of aerosol mode
...
!
!     If CCN type "M", -> sea salt
!     If CCN type "C", -> mix of Su, BC, OC 
!     IF CCN from MACC analyses, mode1=sea-salt, mode2=Su, mode3=BC/OC
!
!     Eventually, two modes of dust are considered (IFN 1 and 2)
!
NMODE_AER=2

! Allocate arrays which size depend on number of modes
!
ALLOCATE(ZMASS  (SIZE(PSVT,1),SIZE(PSVT,2),SIZE(PSVT,3),NMODE_AER))
ALLOCATE(ZRADIUS(SIZE(PSVT,1),SIZE(PSVT,2),SIZE(PSVT,3),NMODE_AER))
ALLOCATE(ZSIGMA (SIZE(PSVT,1),SIZE(PSVT,2),SIZE(PSVT,3),NMODE_AER))
...
! ZSVT
!
ALLOCATE(ZSVT(SIZE(PSVT,1),SIZE(PSVT,2),SIZE(PSVT,3),NMODE_AER))
ZSVT(:,:,:,:) = 0.
DO JMDE=1, NMOD_IFN
   ZSVT(:,:,:,1) = ZSVT(:,:,:,1) + PSVT(:,:,:,NSV_LIMA_IFN_FREE+JMDE-1)*XFRAC(1,JMDE)
   ZSVT(:,:,:,2) = ZSVT(:,:,:,2) + PSVT(:,:,:,NSV_LIMA_IFN_FREE+JMDE-1)*XFRAC(2,JMDE)
END DO
! Conversion ZSVT en #/m3
!
DO JMDE = 1, NMODE_AER
   ZSVT(:,:,:,JMDE) = ZSVT(:,:,:,JMDE) * PRHODREF(:,:,:)
END DO
!
! Prepare r and sigma arrays (DM1, DM2)
!
DO JMDE = 1, 2
   ZSIGMA(:,:,:,JMDE)=XSIGMA_IFN(JMDE)
   ZRADIUS(:,:,:,JMDE)=XMDIAM_IFN(JMDE)*1.E6/2.
END DO
!
! Compute IFN aerosol mass mixing ratio (kg/m3)
!
! Dust 1
ZMASS(:,:,:,1) = 3.1415/6. * XRHO_IFN(1) * ZSVT(:,:,:,1) &
     * EXP( 3.*LOG(XMDIAM_IFN(1)) + 4.5*LOG(XSIGMA_IFN(1))**2 )
! Dust 2
ZMASS(:,:,:,2) = 3.1415/6. * XRHO_IFN(2) * ZSVT(:,:,:,2) &
     * EXP( 3.*LOG(XMDIAM_IFN(2)) + 4.5*LOG(XSIGMA_IFN(2))**2 )
!
DO JMDE=1,NMODE_AER
...
DEALLOCATE(ZSVT)
!    
END SUBROUTINE LIMA_DUSTOPT_GET
!
!-----------------------------------------------------------------------------
!
! *****************************************************************
...
!LOCALS
REAL, DIMENSION(SIZE(PTAU550,1),SIZE(PTAU550,2),SIZE(PTAU550,3),KSWB) :: ZEXT_COEFF_WVL    
                                                          ![m2/kg] Extinction coefficient at wvl
REAL, DIMENSION(SIZE(PTAU550,1),SIZE(PTAU550,2),SIZE(PTAU550,3)     ) :: ZEXT_COEFF_550    
                                                          ![m2/kg] Extinction coefficient at 550nm
