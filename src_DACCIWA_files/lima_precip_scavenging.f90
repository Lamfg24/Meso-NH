...
!!  C. Barthe  03/2020   add a condition on xfracp to reduce the cost
!-------------------------------------------------------------------------------
!
!*                  0.DECLARATIONS          
!                   --------------
...
REAL :: XFRACP
!
REAL, DIMENSION(SIZE(PRHODREF,1),SIZE(PRHODREF,2),SIZE(PRHODREF,3)) :: ZAUX1, ZAUX2, ZAUX3
REAL, DIMENSION(:), ALLOCATABLE :: ZAUX4, ZAUX5, ZAUX6
REAL, DIMENSION(:,:), ALLOCATABLE :: ZVOLDR_AUX, ZRE_AUX, ZFVELR_AUX, ZST_STAR_AUX
REAL :: ZEXP
!
!
!------------------------------------------------------------------------------
...
END WHERE
!
!++cb++18/03/20 optimization : on sort des boucles les calculs sans indice de boucle
ZEXP = 1. / XALPHAR
WHERE ( GRAIN(:,:,:) )
   ZAUX1(:,:,:) = 1. / ZLAMBDAR_3D(:,:,:)
   ZAUX2(:,:,:) = XCR * (XRHO00 / PRHODREF(:,:,:))**(0.4)
   ZAUX3(:,:,:) = PRHODREF(:,:,:) / (2.0 * ZVISCA_3D(:,:,:))
END WHERE
!
DO J2=1,NDIAMR
   WHERE ( GRAIN(:,:,:) )
...
!      ZVOLDR_3D(:,:,:,J2) = ZABSCISSR(J2)**(1./XALPHAR) / ZLAMBDAR_3D(:,:,:)
      ZVOLDR_3D(:,:,:,J2) = ZABSCISSR(J2)**ZEXP * ZAUX1(:,:,:)
...
!      ZFVELR_3D(:,:,:,J2) = XCR * (ZVOLDR_3D(:,:,:,J2)**XDR) * (XRHO00 / PRHODREF(:,:,:))**(0.4)
      ZFVELR_3D(:,:,:,J2) = ZVOLDR_3D(:,:,:,J2)**XDR * ZAUX2(:,:,:)
      ! Reynolds number
!      ZRE_3D(:,:,:,J2) = ZVOLDR_3D(:,:,:,J2) * ZFVELR_3D(:,:,:,J2)          &
!                                             * PRHODREF(:,:,:) / (2.0*ZVISCA_3D(:,:,:))   
      ZRE_3D(:,:,:,J2) = ZVOLDR_3D(:,:,:,J2) * ZFVELR_3D(:,:,:,J2) * ZAUX3(:,:,:)
...
!
      ALLOCATE(ZAUX4(ISCAV))
      ALLOCATE(ZAUX5(ISCAV))
      ALLOCATE(ZAUX6(ISCAV))
!
...
!
      ...
      ZRE_INV(:,:) = 1./ZRE(:,:)
!
!++cb++ 18/03/20 optimisation des boucles ?
      ZAUX4(:) = XBOLTZ * ZT(:) / (3. * XPI * ZVISCA(:))
      ZAUX5(:) = ZVISCA(:) / ZRHODREF(:)
      ZAUX6(:) = XRHOP / (18. * ZVISCA(:))
!--cb--
...
!
      DO JM = 1, NM  ! species (DM1,DM2,BC,O) for IFN
         ...
         END IF
!
!++cb++ 18/03/20 on ne fait le calcul que si xfracp non nul
         IF (XFRACP .GT. 0.) THEN
!--cb--
!++cb++ 18/03/20 on sort ce calcul de la boucle car pas d'indice de boucle
         ! Density number
...
!              ZDIFF(:,J1) = XBOLTZ*ZT(:)*ZCUNSLIP(:,J1)/(3.*XPI*ZVISCA(:)*ZVOLDP(J1))
              ZDIFF(:,J1) = ZAUX4(:) * ZCUNSLIP(:,J1) / ZVOLDP(J1)
              ! Schmidt number
!              ZSC(:,J1)       = ZVISCA(:)/(ZRHODREF(:)*ZDIFF(:,J1))  
              ZSC(:,J1)       = ZAUX5(:) / ZDIFF(:,J1)
              ZSC_INV(:,J1)   = 1. / ZSC(:,J1)
...
!              ZRELT(:,J1) = (ZVOLDP(J1)**2)*ZCUNSLIP(:,J1)*XRHOP/(18.*ZVISCA(:))
              ZRELT(:,J1) = (ZVOLDP(J1)**2) * ZCUNSLIP(:,J1) * ZAUX6(:)
!              ! Density number
!              ZDENS_RATIO = XRHOP/XRHOLW
!              ZDENS_RATIO_SQRT = SQRT(ZDENS_RATIO)
              ! Initialisation
           ...
           END DO
        ! End of the loop over the aerosol particles diameters
        !--------------------------------------------------------------------------
...
!
! arguments
  INTEGER :: K1, K2  ! indexes for aerosol and drop diameters
...
!
  REAL, DIMENSION(SIZE(PRE,1))      :: PCOL_EF   !result : collision efficiency
!
! local variables
  INTEGER :: I
!
!-------------------------------------------------------------------------------
!
  PCOL_EF(:) = (4.*PSC_INV(:,K1)*PRE_INV(:,K2)*(1.+0.4*PRE_SQRT(:,K2)              &
                  *PSC_3SQRT(:,K1)+0.16*PRE_SQRT(:,K2)*PSC_SQRT(:,K1)))      &
              +(4.*PSIZE_RATIO(:,K1,K2)*(PVISC_RATIO(:)                    & 
              +(1.+2.*PRE_SQRT(:,K2))*PSIZE_RATIO(:,K1,K2)))  
  DO I=1,ISCAV
    IF (PST(I,K1,K2)>PST_STAR(I,K2)) THEN            
            PCOL_EF(I) = PCOL_EF(I)                                           &
                    +(PDENS_RATIO_SQRT*((PST(I,K1,K2)-PST_STAR(I,K2))        &
                    /(PST(I,K1,K2)-PST_STAR(I,K2)+2./3.))**(3./2.))
    ENDIF
  ENDDO
