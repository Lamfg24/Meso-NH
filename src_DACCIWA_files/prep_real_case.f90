...
USE MODD_CH_AEROSOL,       ONLY: LORILAM, LINITPM, XINIRADIUSI, XINIRADIUSJ, &
                                 XINISIGI, XINISIGJ, XN0IMIN, XN0JMIN, CRGUNIT, JP_AER_H2O
USE MODD_CH_MNHC_n,        ONLY: LUSECHAQ_n=>LUSECHAQ,LUSECHIC_n=>LUSECHIC, LUSECHEM_n=>LUSECHEM
USE MODD_CONF
USE MODD_CONF_n
USE MODD_CST
USE MODD_DIM_n
USE MODD_DUST,             ONLY:  LDUST, NMODE_DST, CRGUNITD, XINISIG, XINIRADIUS, XN0MIN, LDSTMACC
...
USE MODI_READ_MACC_DATA_NETCDF_CASE
...
USE MODI_VER_PREP_MACC_NETCDF_CASE
...
CHARACTER(LEN=28)              :: YMACCFILE    ! name of the input MACC file
CHARACTER(LEN=6)               :: YMACCFILETYPE! type of the input MACC file
...
REAL, DIMENSION(:,:,:,:), ALLOCATABLE :: ZSIG_AER, ZRG_AER, ZN0_AER
...
NAMELIST/NAM_AERO_CONF/ LORILAM, LINITPM, LDUST, XINIRADIUSI, XINIRADIUSJ,&
                        ...
                        LDSTMACC, LSLTMACC, CCLOUD, CACTCCN, NMOD_IFN, NMOD_CCN
NAMELIST/NAM_CH_CONF/ LUSECHAQ,LUSECHIC,LUSECHEM
! name of dad of input FM file
INTEGER                :: II, IJ, JSV, IGRID, ILENGTH
...
END IF
!
!*       5.2   reading the input MACC data
!
IF(LEN_TRIM(YMACCFILE)>0)THEN
  IF(YMACCFILETYPE=='NETCDF') THEN
    CALL READ_MACC_DATA_NETCDF_CASE(TZPRE_REAL1FILE,YMACCFILE,TPGDFILE,ZHORI,NVERB,LDUMMY_REAL)
  ELSE
    WRITE(ILUOUT0,FMT=*) 
    !callabortstop
...
END IF
!
IF (LEN_TRIM(YMACCFILE)>0 .AND. YMACCFILETYPE=='NETCDF') THEN
  CALL VER_PREP_MACC_NETCDF_CASE(ZDG)
END IF
!
CALL SECOND_MNH(ZTIME2)
...
CALL BOUNDARIES (                                                 &
          ...
          XLBYUM,XLBYVM,XLBYWM,XLBYTHM,XLBYTKEM,XLBYRM,XLBYSVM,   &
          XRHODJ,                                                 &
          XUT, XVT, XWT, XTHT, XTKET, XRT, XSVT, XSRCT, XRHODREF  )
!
CALL SECOND_MNH(ZTIME2)
ZMISC = ZMISC + ZTIME2 - ZTIME1
!
