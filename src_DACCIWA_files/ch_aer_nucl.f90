...
!
!print*,'Ch_aer_nucl: ZSULF',ZSULF
!
!ZGR(:) = ZSULF(:) / (661.1 * (PRH(:) * 100)**2 - 1.129E5 * (PRH(:)*100) + 1.549E7)
!
!print*,'Ch_aer_nucl: ZGR',ZGR
!
...
ELSE IF (CNUCLEATION == 'MAATTANEN_IONIND') THEN
  ...
  !ZRCI2(:) = 2. * ZRCI(:) * 1.E9
  !
  !print*,'Ch_aer_nucl: ZRCI',ZRCI
  !print*,'Ch_aer_nucl: ZRCI2',ZRCI2
  !print*,'Ch_aer_nucl: ZGAMMA(before)',ZGAMMA
  !
  !ZGAMMA(:) = max( 0.0, 1.0 / (ZMAV+1) * ((ZTSIZE /(ZRCI2(:)))**(ZMAV+1) -1) )
  !
  !print*,'Ch_aer_nucl: ZGAMMA(after)',ZGAMMA
  !
