...
!
  IF (CCLOUD /= 'REVE' ) THEN
    ...
    END IF
!
! Lessivage des CCN et IFN nucleables par Slinn
!
...
XTIME_LES_BU_PROCESS = 0.
!
IF (CELEC /= 'NONE' .AND. (CCLOUD(1:3) == 'ICE' .OR. CCLOUD == 'LIMA')) THEN
!  ZWT_ACT_NUC(:,:,:) = 0.
  IF (CCLOUD == 'LIMA') THEN
    IF ( LFORCING ) THEN
      ZWT_ACT_NUC(:,:,:) = XWT(:,:,:) + XWTFRC(:,:,:)
...
!
  ...
  IF (CSURF=='EXTE') THEN
    ...
    ZTOWN(:,:)= 0.
    CALL MNHGET_SURF_PARAM_n (PSEA=ZSEA(:,:),PTOWN=ZTOWN(:,:))
    CALL RESOLVED_ELEC_n (CCLOUD, CACTCCN, CSCONV, CMF_CLOUD,            &
                          ...
                          ZWT_ACT_NUC, KACTIT, KSEDC, KSEDI, KRAIN,      &
                          KHHONI, LHAIL, NSPLITG, XPABSM, XTHM, XRCM,    &
                          ZSPEEDC, ZSPEEDR, ZSPEEDS, ZSPEEDG, ZSPEEDH,   &
                          XINDEP, XRAINFR, ZSEA, ZTOWN                   )
    DEALLOCATE(ZTOWN)
  ELSE
    CALL RESOLVED_ELEC_n (CCLOUD, CACTCCN, CSCONV, CMF_CLOUD,            &
                          NRR, NSPLITR, IMI, KTCOUNT, OEXIT,             &
...
!
  XACPRR = XACPRR + XINPRR * XTSTEP
  IF ((CCLOUD(1:3) == 'ICE'  .AND. LSEDIC) .OR. &
      (CCLOUD      == 'LIMA' .AND. KSEDC))      &
    XACPRC = XACPRC + XINPRC * XTSTEP
  IF (CCLOUD(1:3) == 'ICE' .OR. &
      (CCLOUD      == 'LIMA' .AND. LCOLD)) THEN
    XACPRS = XACPRS + XINPRS * XTSTEP
    XACPRG = XACPRG + XINPRG * XTSTEP
    IF ( CCLOUD == 'ICE4' .OR. &
        (CCLOUD == 'LIMA' .AND. LHAIL)) &
      XACPRH = XACPRH + XINPRH * XTSTEP
  END IF
END IF
!
CALL SECOND_MNH2(ZTIME2)
