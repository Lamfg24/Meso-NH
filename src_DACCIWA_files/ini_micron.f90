...
USE MODD_BLOWSNOW
USE MODD_BLOWSNOW_n
USE MODD_DUST, ONLY : LDUST, LDEPOS_DST
USE MODD_SALT, ONLY : LSALT, LDEPOS_SLT
!
...
!              ------------------------------
IF (LACTIT .OR. MACTIT .OR. LCOLD) THEN
  ...
  IF (CGETCLOUD /= 'READ') THEN
    CALL INIT_AEROSOL_CONCENTRATION(XRHODREF,                              &
                                    XSVT(:,:,:,NSV_LIMA_BEG:NSV_LIMA_END), &
                                                                XZZ(:,:,:) )
  END IF
! pour lessivage aerosols 
  IF ((LSALT .AND. LDEPOS_SLT(IMI)) .OR. (LDUST .AND. LDEPOS_DST(IMI))) THEN
    CALL INI_RAIN_ICE(KLUOUT,XTSTEP,ZDZMIN,NSPLITR,CCLOUD)
  END IF
