...
USE MODD_PARAM_LIMA_COLD, ONLY: CLIMA_COLD_NAMES, CLIMA_SHAPE_NAMES
...
REAL,DIMENSION(:,:,:,:), ALLOCATABLE :: ZCIT_SHAPE  ! Nb conc. for each ice crystal shape
!
...
! Ni
    IF (.NOT. LCRYSTAL_SHAPE) THEN
      ...
      END IF
    ELSE
      IF (JSV .GE. NSV_LIMA_NI .AND. JSV .LT. NSV_LIMA_NI+NB_CRYSTAL_SHAPE) THEN
        TZFIELD%CMNHNAME   = TRIM(CLIMA_SHAPE_NAMES(JSV-NSV_LIMA_NI+1))//'T'
      END IF
    END IF
! N IFN free
...
! Blowing snow variables
  ...
  IF (LORILAM) THEN
  ALLOCATE(ZRHODREF(SIZE(XRHODREF,1),SIZE(XRHODREF,2),SIZE(XRHODREF,3)))
  DO JK = 1, SIZE(XRHODREF,3)
    ZRHODREF(:,:,JK) = XRHODREFZ(JK)
  END DO
    IF ((CPROGRAM == 'REAL  ').AND.(NSV_AER > 1).AND.(IMI==1).AND.(LAERINIT))  &
      CALL CH_AER_REALLFI_n(XSVT(:,:,:,NSV_AERBEG:NSV_AEREND),XSVT(:,:,:,NSV_CHEMBEG-1+JP_CH_CO), ZRHODREF)
...
! flux aerosols 
      ...
      TZFIELD%LTIMEDEP   = .TRUE.
      DO JSV = 1 , NSV_AER
        TZFIELD%CMNHNAME   = 'FLX_'//TRIM(UPCASE(CAERONAMES(JSV)))
        TZFIELD%CLONGNAME  = TRIM(TZFIELD%CMNHNAME)
        WRITE(TZFIELD%CCOMMENT,'(A4,A7,I2)')'X_Y_','FLX_AER',JSV
        CALL IO_WRITE_FIELD(TPFILE,TZFIELD,XFLX_AER(:,:,JSV)) 
      END DO
      TZFIELD%CUNITS     = 'kg/m2'
      DO JSV = 1 , NSV_AER
        TZFIELD%CMNHNAME   = 'FLXT_'//TRIM(UPCASE(CAERONAMES(JSV)))
        TZFIELD%CLONGNAME  = TRIM(TZFIELD%CMNHNAME)
        WRITE(TZFIELD%CCOMMENT,'(A4,A8,I2)')'X_Y_','FLXT_AER',JSV
        CALL IO_WRITE_FIELD(TPFILE,TZFIELD,XFLXT_AER(:,:,JSV)) 
      END DO
    ENDIF
  ...
  END DO
    ...
    IF ((CPROGRAM == 'REAL  ').AND.(NSV_DST > 1).AND.(IMI==1).AND.(LDSTINIT).AND.(.NOT.LDSTMACC)) THEN
      CALL DUSTLFI_n(XSVT(:,:,:,NSV_DSTBEG:NSV_DSTEND), ZRHODREF)
    END IF
...
!
    ...
    !
      ...
      CALL IO_WRITE_FIELD(TPFILE,TZFIELD,ZN0_DST(:,:,:,JJ)*1E-6)
      END DO
      DEALLOCATE(ZSIG_DST)
      DEALLOCATE(ZRG_DST)
      DEALLOCATE(ZN0_DST)
  ...
  DO JK = 1, SIZE(XRHODREF,3)
    ZRHODREF(:,:,JK) = XRHODREFZ(JK)
  END DO
    IF ((CPROGRAM == 'REAL  ').AND.(NSV_SLT > 1).AND.(IMI==1).AND.(LSLTINIT).AND.(.NOT.LSLTMACC))  THEN
      CALL SALTLFI_n(XSVT(:,:,:,NSV_SLTBEG:NSV_SLTEND), ZRHODREF,XZZ)
    END IF
...
!
    ...
    !
      ...
      CALL IO_WRITE_FIELD(TPFILE,TZFIELD,ZN0_SLT(:,:,:,JJ)*1E-6)
    !
      END DO
      DEALLOCATE(ZSIG_SLT)
      DEALLOCATE(ZRG_SLT)
...
! flux aerosols marins
    IF (CPROGRAM == 'MESONH') THEN
      ...
      TZFIELD%LTIMEDEP   = .TRUE.
      DO JSV = 1 , NMODE_SLT
        TZFIELD%CMNHNAME   = 'FLX_'//TRIM(UPCASE(CSALTNAMES(JSV)))
        TZFIELD%CLONGNAME  = TRIM(TZFIELD%CMNHNAME)
        WRITE(TZFIELD%CCOMMENT,'(A4,A7,I1)')'X_Y_','FLX_SLT',JSV
        CALL IO_WRITE_FIELD(TPFILE,TZFIELD,XFLX_SLT(:,:,JSV)) 
      END DO
      TZFIELD%CUNITS     = 'kg/m2'
      DO JSV = 1 , NMODE_SLT
        TZFIELD%CMNHNAME   = 'FLXT_'//TRIM(UPCASE(CSALTNAMES(JSV)))
        TZFIELD%CLONGNAME  = TRIM(TZFIELD%CMNHNAME)
        WRITE(TZFIELD%CCOMMENT,'(A4,A8,I1)')'X_Y_','FLXT_SLT',JSV
        CALL IO_WRITE_FIELD(TPFILE,TZFIELD,XFLXT_SLT(:,:,JSV)) 
      END DO
    ENDIF
  ENDIF  
  !
...
! flux  of so2
      ...
      TZFIELD%NDIMS      = 2
      TZFIELD%LTIMEDEP   = .TRUE.
      TZFIELD%CMNHNAME   = 'FLX_SO2'
      TZFIELD%CLONGNAME  = TRIM(TZFIELD%CMNHNAME)
      WRITE(TZFIELD%CCOMMENT,'(A4,A7)')'X_Y_','FLX_SO2'
      CALL IO_WRITE_FIELD(TPFILE,TZFIELD,XFLX_SO2(:,:)) 
      TZFIELD%CUNITS     = 'kg/m2'
      TZFIELD%CMNHNAME   = 'FLXT_SO2'
      TZFIELD%CLONGNAME  = TRIM(TZFIELD%CMNHNAME)
      WRITE(TZFIELD%CCOMMENT,'(A4,A8)')'X_Y_','FLXT_SO2'
      CALL IO_WRITE_FIELD(TPFILE,TZFIELD,XFLXT_SO2(:,:)) 
