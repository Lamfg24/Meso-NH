...
SUBROUTINE ECMWF_RADIATION_VERS2 ( KLON,KLEV,KRAD_DIAG, KAER, &
     ...
     OORILAM,PPIZA_ORI,PCGA_ORI,PTAUREL_ORI                    )
! -- PIERRE / MARINE SSA DUST - MODIF --
!  
...
INTEGER, DIMENSION (:,:), INTENT (IN)   :: KSHAPE_MAX ! dominant ice crystal shape per mesh
...
REAL, DIMENSION(:,:,:),INTENT(IN)   :: PPIZA_SLT   !Single scattering albedo of salt (wvl dependent)
REAL, DIMENSION(:,:,:),INTENT(IN)   :: PCGA_SLT    !Assymetry factor for salt (wvl dependent)
REAL, DIMENSION(:,:,:),INTENT(IN)   :: PTAUREL_SLT !Optical depth of salt relative to the one at 550nm
REAL, DIMENSION(:,:,:),INTENT(IN)   :: PPIZA_ORI   !Single scattering albedo of orilam (wvl dependent)
REAL, DIMENSION(:,:,:),INTENT(IN)   :: PCGA_ORI    !Assymetry factor for orilam (wvl dependent)
REAL, DIMENSION(:,:,:),INTENT(IN)   :: PTAUREL_ORI !Optical depth of orilam relative to the one at 550nm
...
!         C. Barthe 3/2016 : correction for LIMA
!-----------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
!++cb++22/04/20 : possibilite de prise en compte des formes pour le calcul de r_eff(i)
USE MODD_PARAM_LIMA, ONLY : VRTMIN=>XRTMIN, VCTMIN=>XCTMIN, &
                            VALPHAC=>XALPHAC, VNUC=>XNUC,   &
                            VALPHAR=>XALPHAR, VNUR=>XNUR,   &
                            LCRYSTAL_SHAPE
USE MODD_PARAM_LIMA_WARM, ONLY : VCREC=>XCREC, VCRER=>XCRER, VFREFFR=>XFREFFR, &
                                 VAC=>XAC, VAR=>XAR, VLBEXC=>XLBEXC,           &
                                 VLBEXR=>XLBEXR
USE MODD_PARAM_LIMA_COLD, ONLY : VFREFFI=>XFREFFI, VLBEXI=>XLBEXI, &
                                 XFREFFI_SHAPE, XLBEXI_SHAPE
!--cb--
!
IMPLICIT NONE
!
...
!
REAL :: ZALPHAC, ZNUC, ZAC, ZLBEXC, ZCREC,          &
        ZALPHAR, ZNUR, ZAR, ZLBEXR, ZCRER, ZFREFFR, &
        ZFREFFI, ZLBEXI
REAL, DIMENSION(:), ALLOCATABLE :: ZCTMIN, ZRTMIN
!
REAL(KIND=JPRB) :: ZCCO2_RAD
!--------------------------------------------------------------
ZCCO2_RAD = PCCO2
!
!*         1.     SOME INITIALIZATIONS FOR LIMA
!                 -----------------------------
!
IF ( CCLOUD == "LIMA" ) THEN
  ALLOCATE(ZRTMIN(SIZE(VRTMIN,1)))
  ALLOCATE(ZCTMIN(SIZE(VCTMIN,1)))
  ZRTMIN(:) = VRTMIN(:)
  ZCTMIN(:) = VCTMIN(:)
!
  ZALPHAC = VALPHAC
...
ELSE IF (CCLOUD == "C3R5") THEN
  ALLOCATE(ZRTMIN(SIZE(YRTMIN,1)))
  ALLOCATE(ZCTMIN(SIZE(YCTMIN,1)))
  ZRTMIN(:) = YRTMIN(:)
  ZCTMIN(:) = YCTMIN(:)
!
  ZALPHAC = UALPHAC
...
ELSE IF (CCLOUD == "C2R2" .OR. CCLOUD == "KHKO") THEN
  ALLOCATE(ZRTMIN(SIZE(URTMIN,1)))
  ALLOCATE(ZCTMIN(SIZE(UCTMIN,1)))
  ZRTMIN(:) = URTMIN(:)
  ZCTMIN(:) = UCTMIN(:)
!
  ZALPHAC = UALPHAC
...
!
!print*, 'passage dans rayonnement !'
!print*, 'lcrystal_shape = ', lcrystal_shape
DO JK = 1 , KLEV
  IKL = KLEV + 1 - JK
  DO JSW = 1,NSW
    DO JL = IKIDIA,IKFDIA
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ...
          ZRADRP(JL) = 0.
          ZRADRS(JL) = 0.
          IF (PCCT_C2R2(JL, IKL)>ZCTMIN(2) .AND. PLWC(JL,IKL)>ZRTMIN(2).AND.&
                ZFLWP(JL)/=0.0) THEN
            ZNUM =  PLWC(JL,IKL)/ ZAC
            ZDEN = ZCREC * (PCCT_C2R2(JL,IKL)*(PLWC(JL,IKL))**2)**(ZLBEXC)
            ZRADLS(JL) = MIN( 0.5E6 * sqrt(ZDEN/PCCT_C2R2(JL,IKL)) , 100.)
            ZRADLP(JL) = MIN( 0.5E6 * (ZNUM/ZDEN)                  , 100.)
          ENDIF
           IF (PCRT_C2R2(JL, IKL)>ZCTMIN(3) .AND. PRWC(JL,IKL)>ZRTMIN(3).AND.&
                ZFRWP(JL)/=0.0) THEN
            ZNUM =  PRWC(JL,IKL)/ ZAR
            ZDEN = ZCRER * (PCRT_C2R2(JL,IKL)*(PRWC(JL,IKL))**2)**(ZLBEXR)
...
!        ZRADIP(JL) = 0.0
        ZRADIP(JL) = 40.0
!++cb++ 22/04/20 prise en compte de l'effet des formes
        IF (.NOT. LCRYSTAL_SHAPE) THEN
          IF (PCIT_C1R3(JL, IKL)>ZCTMIN(4) .AND. PIWC(JL,IKL)>ZRTMIN(4)) THEN
            ZMR(JL) = PIWC(JL,IKL)
            ZRADIP(JL) = 1.0E6 * ZFREFFI*(ZMR(JL)/PCIT_C1R3(JL,IKL))**(ZLBEXI)
!++cb++ 05/05/20 blindage du rayon pour eviter plantage
            ZRADIP(JL) = MIN ( MAX( ZRADIP(JL), 20.), 250.)
!--cb--
            ZDESR(JL)  = 2.0 * ZRADIP(JL)
          END IF
        ELSE
! dans un 1er temps : on fait l'hypothese que c'est la forme preponderante
! en nombre qui pilote le rayon effectif
! --> a faire evoluer !
          IF (PCIT_C1R3(JL, IKL)>ZCTMIN(4) .AND. PIWC(JL,IKL)>ZRTMIN(4)) THEN
            ZMR(JL) = PIWC(JL,IKL)
            ZRADIP(JL) = 1.0E6 * XFREFFI_SHAPE(KSHAPE_MAX(JL,IKL)) * &
                         (ZMR(JL) / PCIT_C1R3(JL,IKL))**XLBEXI_SHAPE(KSHAPE_MAX(JL,IKL))
!++cb++ 05/05/20 blindage du rayon pour eviter plantage
            ZRADIP(JL) = MIN ( MAX( ZRADIP(JL), 20.), 250.)
!--cb--
            ZDESR(JL)  = 2.0 * ZRADIP(JL)
