...
!!    -------------
!!      Original    09/05/07
!!      C. Barthe (LACy)   03/2019  ORILAM-LIMA coupling
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
END IF
IF (CCLOUD == 'LIMA') THEN
  ALLOCATE(ZRTMIN(SIZE(XRTMIN_LIMA)))
  ZRTMIN(:) = XRTMIN_LIMA(:)
END IF
!
...
ZZRCT(:,:,:) = PRCT(:,:,:)
ZZRCT(:,:,:) = MAX(ZZRCT(:,:,:), ZRTMIN(2)/2.)
!
WHERE( (ZZRCT(:,:,:)>ZRTMIN(2)) .AND. (PRCS(:,:,:)>0.0 ) ) 
...
ZLBDAR(:,:,:) = 0.0
!
WHERE ( (ZZRCT(:,:,:)>ZRTMIN(2)) .AND. (PRRT(:,:,:)>ZRTMIN(3)) &
                                 .AND. (PRCS(:,:,:)> 0.0 ) )      
