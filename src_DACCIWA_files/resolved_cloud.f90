...
!!      C. Barthe    03/2019 ORILAM-LIMA coupling
...
USE MODD_BUDGET,         ONLY: LBUDGET_TH, LBUDGET_RC, LBUDGET_RG, LBUDGET_RH, LBUDGET_RI, LBUDGET_RR, LBUDGET_RS, LBUDGET_RV, &
                               LBUDGET_SV
USE MODD_CH_AEROSOL,     ONLY: LORILAM, JP_AER_H2O
USE MODD_CH_AERO_n         ! declaration de XN3D
USE MODD_CSTS_DUST
USE MODD_CSTS_SALT
...
INTEGER :: JK,JI,JL
INTEGER :: JSH           ! loop for crystal shape
!
!
!
...
REAL, DIMENSION(:,:,:,:), ALLOCATABLE :: ZSIG_SLT, & ! std dev. for sea salts
                                         ...
                                         ZSIG_AER, & ! std dev. for orilam
                                         ZRG_AER,  & ! nb median diameter for orilam
                                         ZN0_AER     ! nb conc. of orilam (#/m3)
REAL, DIMENSION(:,:,:,:,:), ALLOCATABLE :: ZCTOTA  ! orilam aerosol mass (µg/m3)
REAL, DIMENSION(:,:,:),     ALLOCATABLE :: ZSUM, ZRATH2O
INTEGER :: II ! loop index
REAL :: ZMI, ZRHOI
REAL, DIMENSION(SIZE(PZZ,1),SIZE(PZZ,2),SIZE(PZZ,3)) :: ZAUX
!
!------------------------------------------------------------------------------
!
...
!  PSVT(:,:,:,NSV_LIMA_CCN_FREE)   = ZSVT(:,:,:,NSV_LIMA_CCN_FREE-ISVBEG+1)
  PSVS(:,:,:,NSV_LIMA_CCN_FREE)   = ZSVT(:,:,:,NSV_LIMA_CCN_FREE) * &
                                    PRHODJ(:,:,:) / PTSTEP
!  PSVT(:,:,:,NSV_LIMA_CCN_FREE+1) = ZSVT(:,:,:,NSV_LIMA_CCN_FREE+1-ISVBEG+1) 
  PSVS(:,:,:,NSV_LIMA_CCN_FREE+1) = ZSVT(:,:,:,NSV_LIMA_CCN_FREE+1) * &
                                    PRHODJ(:,:,:) / PTSTEP
!  PSVT(:,:,:,NSV_LIMA_CCN_FREE+2) = ZSVT(:,:,:,NSV_LIMA_CCN_FREE+1-ISVBEG+1) 
  PSVS(:,:,:,NSV_LIMA_CCN_FREE+2) = ZSVT(:,:,:,NSV_LIMA_CCN_FREE+2) * &
                                    PRHODJ(:,:,:) / PTSTEP

!  PSVT(:,:,:,NSV_LIMA_IFN_FREE)   = ZSVT(:,:,:,NSV_LIMA_IFN_FREE-ISVBEG+1) 
  PSVS(:,:,:,NSV_LIMA_IFN_FREE)   = ZSVT(:,:,:,NSV_LIMA_IFN_FREE) * &
                                    PRHODJ(:,:,:) / PTSTEP
!  PSVT(:,:,:,NSV_LIMA_IFN_FREE+1) = ZSVT(:,:,:,NSV_LIMA_IFN_FREE+1-ISVBEG+1)
    ...
    IF (LCOLD) THEN
      ...
      ELSE
        IF (NB_CRYSTAL_SHAPE .EQ. 2) THEN
          ZAUX(:,:,:) = ZSVS(:,:,:,NSV_LIMA_NI) + ZSVS(:,:,:,NSV_LIMA_NI+1)
          WHERE (PRS(:,:,:,4) < YRTMIN(4)/PTSTEP .OR. ZAUX(:,:,:) < YCTMIN(4)/PTSTEP)
            PRS(:,:,:,1) = PRS(:,:,:,1) + PRS(:,:,:,4)
            PTHS(:,:,:) = PTHS(:,:,:) - PRS(:,:,:,4) * ZLS(:,:,:) /  &
                          ZCPH(:,:,:) / ZEXN(:,:,:)
            PRS(:,:,:,4) = 0.0
            ZSVS(:,:,:,NSV_LIMA_NI)   = 0.0
            ZSVS(:,:,:,NSV_LIMA_NI+1) = 0.0
          ELSEWHERE (ZSVS(:,:,:,NSV_LIMA_NI)   < YCTMIN(4)/PTSTEP)
            ZSVS(:,:,:,NSV_LIMA_NI)   = 0.0
          ELSEWHERE (ZSVS(:,:,:,NSV_LIMA_NI+1) < YCTMIN(4)/PTSTEP)
            ZSVS(:,:,:,NSV_LIMA_NI+1) = 0.0
          END WHERE
        ELSE IF (NB_CRYSTAL_SHAPE .EQ. 3) THEN
          ZAUX(:,:,:) = ZSVS(:,:,:,NSV_LIMA_NI) + ZSVS(:,:,:,NSV_LIMA_NI+1) + ZSVS(:,:,:,NSV_LIMA_NI+2)
          WHERE (PRS(:,:,:,4) < YRTMIN(4)/PTSTEP .OR. ZAUX(:,:,:) < YCTMIN(4)/PTSTEP)
            PRS(:,:,:,1) = PRS(:,:,:,1) + PRS(:,:,:,4)
            PTHS(:,:,:) = PTHS(:,:,:) - PRS(:,:,:,4) * ZLS(:,:,:) /  &
                          ZCPH(:,:,:) / ZEXN(:,:,:)
            ...
            ZSVS(:,:,:,NSV_LIMA_NI)   = 0.0
            ZSVS(:,:,:,NSV_LIMA_NI+1) = 0.0
            ZSVS(:,:,:,NSV_LIMA_NI+2) = 0.0
          ELSEWHERE (ZSVS(:,:,:,NSV_LIMA_NI)   < YCTMIN(4)/PTSTEP)
            ZSVS(:,:,:,NSV_LIMA_NI)   = 0.0
          ELSEWHERE (ZSVS(:,:,:,NSV_LIMA_NI+1) < YCTMIN(4)/PTSTEP)
            ZSVS(:,:,:,NSV_LIMA_NI+1) = 0.0
          ELSEWHERE (ZSVS(:,:,:,NSV_LIMA_NI+2) < YCTMIN(4)/PTSTEP)
            ZSVS(:,:,:,NSV_LIMA_NI+2) = 0.0
          END WHERE
        ELSE IF (NB_CRYSTAL_SHAPE .EQ. 4) THEN
          ZAUX(:,:,:) = ZSVS(:,:,:,NSV_LIMA_NI)   + ZSVS(:,:,:,NSV_LIMA_NI+1) + &
                        ZSVS(:,:,:,NSV_LIMA_NI+2) + ZSVS(:,:,:,NSV_LIMA_NI+3)
          WHERE (PRS(:,:,:,4) < YRTMIN(4)/PTSTEP .OR. ZSVS(:,:,:,NSV_LIMA_NI)   < YCTMIN(4)/PTSTEP &
                                                 .OR. ZSVS(:,:,:,NSV_LIMA_NI+1) < YCTMIN(4)/PTSTEP &
                                                 .OR. ZSVS(:,:,:,NSV_LIMA_NI+2) < YCTMIN(4)/PTSTEP &
                                                 .OR. ZSVS(:,:,:,NSV_LIMA_NI+3) < YCTMIN(4)/PTSTEP)
            PRS(:,:,:,1) = PRS(:,:,:,1) + PRS(:,:,:,4)
            PTHS(:,:,:) = PTHS(:,:,:) - PRS(:,:,:,4) * ZLS(:,:,:) /  &
                          ZCPH(:,:,:) / ZEXN(:,:,:)
            PRS(:,:,:,4)  = 0.0
          ...
          ELSEWHERE (ZSVS(:,:,:,NSV_LIMA_NI+2) < YCTMIN(4)/PTSTEP)
            ZSVS(:,:,:,NSV_LIMA_NI+2) = 0.0
          ELSEWHERE (ZSVS(:,:,:,NSV_LIMA_NI+3) < YCTMIN(4)/PTSTEP)
            ZSVS(:,:,:,NSV_LIMA_NI+3) = 0.0
          END WHERE
        END IF
      END IF
  ...
  IF (LCOLD) THEN
    ...
    ELSE
      DO JSH = 1, NB_CRYSTAL_SHAPE
        CALL BUDGET (ZSVS(:,:,:,NSV_LIMA_NI+ JSH-1) * PRHODJ(:,:,:),12+NSV_LIMA_NI+ JSH-1,'NEGA_BU_RSV')
      END DO
