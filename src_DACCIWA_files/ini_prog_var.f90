...
!!                  02/2019  (C. Barthe) ORILAM-LIMA coupling
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
!
REAL, DIMENSION(:,:,:,:), ALLOCATABLE :: ZSIG_X, ZRG_X, ZN0_X
REAL, DIMENSION(:,:,:,:), ALLOCATABLE :: ZSIG_Y, ZRG_Y, ZN0_Y
...
!
  IF (CCLOUD == 'LIMA' .AND. LSALT .AND. LSLTMACC) THEN
    ALLOCATE(ZSIG_X(SIZE(XLBXSVM,1), SIZE(XLBXSVM,2), SIZE(XLBXSVM,3), NMODE_SLT))
    ALLOCATE(ZRG_X(SIZE(XLBXSVM,1), SIZE(XLBXSVM,2), SIZE(XLBXSVM,3), NMODE_SLT))
    ALLOCATE(ZN0_X(SIZE(XLBXSVM,1), SIZE(XLBXSVM,2), SIZE(XLBXSVM,3), NMODE_SLT))
!
    ALLOCATE(ZSIG_Y(SIZE(XLBYSVM,1), SIZE(XLBYSVM,2), SIZE(XLBYSVM,3), NMODE_SLT))
    ALLOCATE(ZRG_Y(SIZE(XLBYSVM,1), SIZE(XLBYSVM,2), SIZE(XLBYSVM,3), NMODE_SLT))
    ALLOCATE(ZN0_Y(SIZE(XLBYSVM,1), SIZE(XLBYSVM,2), SIZE(XLBYSVM,3), NMODE_SLT))
!
...
!
! moments (XSVT;ppp) --> concentration (PN3D;#/m3)
    CALL PPP2SALT(XLBXSVM(:,:,:,NSV_SLTBEG:NSV_SLTEND),ZLBXRHO,&
                  PSIG3D=ZSIG_X,PRG3D=ZRG_X,PN3D=ZN0_X)
    CALL PPP2SALT(XLBYSVM(:,:,:,NSV_SLTBEG:NSV_SLTEND),ZLBYRHO,&
                  PSIG3D=ZSIG_Y,PRG3D=ZRG_Y,PN3D=ZN0_Y)
!
! #/m3 --> #/kg
    ZN0_X(:,:,:,1) = ZN0_X(:,:,:,1) / ZLBXRHO(:,:,:)
    ZN0_X(:,:,:,2) = ZN0_X(:,:,:,2) / ZLBXRHO(:,:,:)
    ZN0_X(:,:,:,3) = ZN0_X(:,:,:,3) / ZLBXRHO(:,:,:)
    ZN0_X(:,:,:,4) = ZN0_X(:,:,:,4) / ZLBXRHO(:,:,:)
    ZN0_X(:,:,:,5) = ZN0_X(:,:,:,5) / ZLBXRHO(:,:,:)
!
    ZN0_Y(:,:,:,1) = ZN0_Y(:,:,:,1) / ZLBYRHO(:,:,:)
    ZN0_Y(:,:,:,2) = ZN0_Y(:,:,:,2) / ZLBYRHO(:,:,:)
    ZN0_Y(:,:,:,3) = ZN0_Y(:,:,:,3) / ZLBYRHO(:,:,:)
    ZN0_Y(:,:,:,4) = ZN0_Y(:,:,:,4) / ZLBYRHO(:,:,:)
    ZN0_Y(:,:,:,5) = ZN0_Y(:,:,:,5) / ZLBYRHO(:,:,:)
!
! orilam --> lima
    XLBXSVM(:,:,:,NSV_LIMA_CCN_FREE) = ZN0_X(:,:,:,2) + &
                                       ZN0_X(:,:,:,3) + &
                                       ZN0_X(:,:,:,4) + &
                                       ZN0_X(:,:,:,5)
    XLBYSVM(:,:,:,NSV_LIMA_CCN_FREE) = ZN0_Y(:,:,:,2) + &
                                       ZN0_Y(:,:,:,3) + &
                                       ZN0_Y(:,:,:,4) + &
                                       ZN0_Y(:,:,:,5)
!
  ...
  END IF
!
  IF (CCLOUD == 'LIMA' .AND. LDUST .AND. LDSTMACC) THEN
    ALLOCATE(ZSIG_X(SIZE(XLBXSVM,1), SIZE(XLBXSVM,2), SIZE(XLBXSVM,3), NMODE_DST))
    ALLOCATE(ZRG_X(SIZE(XLBXSVM,1), SIZE(XLBXSVM,2), SIZE(XLBXSVM,3), NMODE_DST))
    ALLOCATE(ZN0_X(SIZE(XLBXSVM,1), SIZE(XLBXSVM,2), SIZE(XLBXSVM,3), NMODE_DST))
!
    ALLOCATE(ZSIG_Y(SIZE(XLBYSVM,1), SIZE(XLBYSVM,2), SIZE(XLBYSVM,3), NMODE_DST))
    ALLOCATE(ZRG_Y(SIZE(XLBYSVM,1), SIZE(XLBYSVM,2), SIZE(XLBYSVM,3), NMODE_DST))
    ALLOCATE(ZN0_Y(SIZE(XLBYSVM,1), SIZE(XLBYSVM,2), SIZE(XLBYSVM,3), NMODE_DST))
!
    ALLOCATE(ZLBXRHO(SIZE(XLBXSVM,1), SIZE(XLBXSVM,2), SIZE(XLBXSVM,3)))
    ALLOCATE(ZLBYRHO(SIZE(XLBYSVM,1), SIZE(XLBYSVM,2), SIZE(XLBYSVM,3)))
!
...
!
! moments (XSVT;ppp) --> concentration (PN3D;#/m3)
    CALL PPP2DUST(XLBXSVM(:,:,:,NSV_DSTBEG:NSV_DSTEND),ZLBXRHO,&
                  PSIG3D=ZSIG_X,PRG3D=ZRG_X,PN3D=ZN0_X)
    CALL PPP2DUST(XLBYSVM(:,:,:,NSV_DSTBEG:NSV_DSTEND),ZLBYRHO,&
                  PSIG3D=ZSIG_Y,PRG3D=ZRG_Y,PN3D=ZN0_Y)
!
! #/m3 --> #/kg
    ZN0_X(:,:,:,1) = ZN0_X(:,:,:,1) / ZLBXRHO(:,:,:)
...
!
    ...
    ZN0_Y(:,:,:,3) = ZN0_Y(:,:,:,3) / ZLBYRHO(:,:,:)
! orilam --> lima
    XLBXSVM(:,:,:,NSV_LIMA_IFN_FREE) = ZN0_X(:,:,:,1) + &
                                       ZN0_X(:,:,:,2) + &
                                       ZN0_X(:,:,:,3)
    XLBYSVM(:,:,:,NSV_LIMA_IFN_FREE) = ZN0_Y(:,:,:,1) + &
                                       ZN0_Y(:,:,:,2) + &
                                       ZN0_Y(:,:,:,3)
!
    DEALLOCATE(ZSIG_X)
