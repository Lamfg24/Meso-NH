...
REAL, DIMENSION(SIZE(PN0,1))   ::  ztemp
...
INTEGER ::  NSP2 ! Nb of inorganics compounds
...
! Final  Raoult term  
 !   print*,'sum2 =',MINVAL(sum2), maxval(sum2)
 !   print*,'sum3 =',MINVAL(sum3), maxval(sum3)
    WHERE (sum2(:) > XMNH_TINY)
    B(:,i) = XMW*1.E-3*sum1(:)/ (RHOW*sum2(:)) 
    END WHERE
!    print*,'B(:,',i,') =',MINVAL(B(:,i)), MAXVAL(B(:,i))

!  2. Compute Kelvin term of Abdul-Razzak et al. 2004 parameterization
!     ================================================================

! Aerosol salt mass fraction 
    WHERE (sum3(:) > XMNH_TINY)
    ZFRACSEL(:) = sum4(:) / sum3(:)
    END WHERE
...
! Kelvin final term
ztemp (:) = (27.* R0AP(:,i)**3. * SM(:,i)**2. * B(:,i)) / 4.
A(:,i) = ((27.* R0AP(:,i)**3. * SM(:,i)**2. * B(:,i)) / 4.)**(1./3.)  
!print*,'A(:,',i,') =',MINVAL(A(:,i)), MAXVAL(A(:,i))
...
cc0(:) = MAX(cc0(:), XMNH_TINY)

WHERE (B(:,i) .GT. XMNH_TINY) 
...
! number of activated aerosol pp/m3
  WHERE ((PSMAX(:).GT.XMNH_TINY).AND.(SM(:,i).GT.XMNH_TINY))
...
PNCN(:,:) =  MAX(XNNUCS(:,:), 0.)
!print*,'PNCN =',MINVAL(PNCN), MAXVAL(PNCN)
PMCN(:,:) =  MAX(xmapac(:,:), 0.)
