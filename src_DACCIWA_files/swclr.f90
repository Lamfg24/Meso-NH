...
LOGICAL :: OSALT                   ! flag for SALT
REAL_B  :: PPIZA_SLT(KLON,KLEV)    !ssa salt for current wavelength
REAL_B  :: PCGA_SLT(KLON,KLEV)     !assym.fact salt for current wavelength
REAL_B  :: PTAUREL_SLT(KLON,KLEV)  !tau/tau_{550} for current wavelength
LOGICAL :: OORILAM                 ! flag for ORILAM
REAL_B  :: PPIZA_ORI(KLON,KLEV)    !ssa orilam for current wavelength
REAL_B  :: PCGA_ORI(KLON,KLEV)     !assym.fact orilam for current wavelength
REAL_B  :: PTAUREL_ORI(KLON,KLEV)  !tau/tau_{550} for current wavelength
!--MODIF_MESONH
  ...
  ELSE ! MESONH VERSION
     DO JAE=1,6
        DO JL = KIDIA,KFDIA
           ...
           ELSEIF (OSALT.AND.(JAE==2)) THEN
           !Ponderation of aerosol optical properties:first step 
           !ti
             PTAUAZ(JL,JK)=PTAUAZ(JL,JK) + PAER(JL,JAE,IKL) * PTAUREL_SLT(JL,IKL)
           !wi*ti
             PPIZAZ(JL,JK)=PPIZAZ(JL,JK) + PAER(JL,JAE,IKL)  &
                    *PTAUREL_SLT(JL,IKL)*PPIZA_SLT(JL,IKL)
           !wi*ti*gi
             PCGAZ(JL,JK) = PCGAZ(JL,JK) + PAER(JL,JAE,IKL) &
                  *PTAUREL_SLT(JL,IKL)*PPIZA_SLT(JL,IKL)*PCGA_SLT(JL,IKL)
           !wi*ti*(gi**2)
             ZFACOA_NEW(JL,JK)= ZFACOA_NEW(JL,JK)+PAER(JL, JAE, IKL)&
                &*PTAUREL_SLT(JL,IKL) *PPIZA_SLT(JL,IKL)*PCGA_SLT(JL,IKL)*PCGA_SLT(JL,IKL)
           !Special optical properties for orilam aerosols
           ELSEIF (OORILAM.AND.(JAE==4)) THEN
           !Ponderation of aerosol optical properties:first step 
           !ti
             PTAUAZ(JL,JK)=PTAUAZ(JL,JK) + PAER(JL,JAE,IKL) * PTAUREL_ORI(JL,IKL)
           !wi*ti
             PPIZAZ(JL,JK)=PPIZAZ(JL,JK) + PAER(JL,JAE,IKL)  &
                    *PTAUREL_ORI(JL,IKL)*PPIZA_ORI(JL,IKL)
           !wi*ti*gi
             PCGAZ(JL,JK) = PCGAZ(JL,JK) + PAER(JL,JAE,IKL) &
                  *PTAUREL_ORI(JL,IKL)*PPIZA_ORI(JL,IKL)*PCGA_ORI(JL,IKL)
           !wi*ti*(gi**2)
             ZFACOA_NEW(JL,JK)= ZFACOA_NEW(JL,JK)+PAER(JL, JAE, IKL)&
                &*PTAUREL_ORI(JL,IKL) *PPIZA_ORI(JL,IKL)*PCGA_ORI(JL,IKL)*PCGA_ORI(JL,IKL)
           ELSE
           !Ponderation of aerosol optical properties:first step 
           !ti
             PTAUAZ(JL,JK)=PTAUAZ(JL,JK)+PAER(JL, JAE, IKL)*RTAUA(KNU,JAE)
