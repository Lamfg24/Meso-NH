...
!
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PCCS    ! Cloud water C. source
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PCIS    ! Ice crystal C. source (tot)
REAL, DIMENSION(:,:,:,:), INTENT(INOUT) :: PCIS_SHAPE  ! Ice crystal C. source (per shape)
!
REAL, DIMENSION(:,:,:,:), INTENT(INOUT) :: PNAS    ! Cloud  C. nuclei C. source
                                                   !used as Free ice nuclei for
                                                   !IMMERSION freezing
...
INTEGER :: JSH                                        ! Loop index for ice crystral shape
...
REAL, DIMENSION(:,:), ALLOCATABLE :: ZNIS    ! Nucleated Ice nuclei conc. source 
                                             !by Immersion
!
REAL, DIMENSION(:,:), ALLOCATABLE :: ZCIS_SHAPE    ! Pristine ice conc. source (per shape)
!
...
!
  ...
  ALLOCATE(ZCIS(INEGT))
!
  IF (LCRYSTAL_SHAPE) ALLOCATE(ZCIS_SHAPE(INEGT,NB_CRYSTAL_SHAPE))
!
...
!
  ...
  ENDDO
!print*, ' LP1 pcis_tot ni= ', minval(pcis_tot(:,:,:)), maxval(pcis_tot(:,:,:))
!print*, ' LP1 zcis_tot ni= ', minval(zcis_tot(:)), maxval(zcis_tot(:))
!print*, ' LP1 pcis_shape ni= ', minval(pcis_shape(:,:,:,1)), maxval(pcis_shape(:,:,:,1))
!print*, ' LP1 pcis_shape ni+1= ', minval(pcis_shape(:,:,:,2)), maxval(pcis_shape(:,:,:,2))
!print*, ' LP1 zcis_shape ni= ', minval(zcis_shape_v(:,1)), maxval(zcis_shape_v(:,1))
!print*, ' LP1 zcis_shape ni+1= ', minval(zcis_shape_v(:,2)), maxval(zcis_shape_v(:,2))
!
! PACK : done
...
! Table 1 de Phillips et al. (2008)
!
! Dust/Metallic - 2 modes
...
! Black Carbon
  ZSI0(:,3) = 0.0       ! BC
  ZZT_SI0_BC(:) = MAX( 198.0, MIN( 239.0,ZZT(:) ) )  ! 198 K < T < 239 K
  ZSI0(:,3) = (-3.118E-5 * ZZT_SI0_BC(:) + 1.085E-2) * ZZT_SI0_BC(:) + 0.5652 - XDSI0(3)
!
! Organic (Phillips, 2008) ou Primary biological aerosol particles (Phillips, 2013)
  IF (NPHILLIPS == 8) THEN
    ZSI0(:,4) = ZSI0(:,3) ! O = BC
...
!
  DO JMOD_IFN = 1,NMOD_IFN    ! IFN modes
    ...
    DO JSPECIE = 1, NSPECIE  ! Each IFN mode is mixed with DM1, DM2, BC, O
      ZZX(:) = ZZX(:) + XFRAC(JSPECIE,JMOD_IFN) *              &
                       (ZIFS(:,JMOD_IFN) + ZINS(:,JMOD_IFN)) * &
                        Z_FRAC_ACT(:,JSPECIE)
    END DO                   
...
!
! Plates: -1<T<-3, -9<T<-13, -13<T<-18, -18<T<-22 
!      WHERE ( ((ZTCELSIUS(:) .LE. -1.0)  .AND. (ZTCELSIUS(:) .GT. -3.0)) .OR.    &
      WHERE ( (ZTCELSIUS(:) .GT. -3.0) .OR.    &
             ((ZTCELSIUS(:) .LE. -9.0) .AND. (ZTCELSIUS(:) .GT. -22.0)) )
        ZCIS_SHAPE(:,1) = ZCIS_SHAPE(:,1) + ZZX(:) 
      END WHERE
!
! Columns: -3<T<-9, -22<T<-37 if S>Sw
!      WHERE ( ((ZTCELSIUS(:) .LE. -3.0) .AND. (ZTCELSIUS(:) .GT. -9.0)) .OR.  &
      WHERE ( (ZTCELSIUS(:) .GT. -3.0) .OR.    &
               (ZTCELSIUS(:) .LE. -22) )
        ZCIS_SHAPE(:,2) = ZCIS_SHAPE(:,2) + ZZX(:) 
      END WHERE 
!      ZCIS(:) = SUM (ZCIS_SHAPE, DIM=2)
    END IF
  END DO
!print*, ' LP2 zcis_shape ni= ', minval(zcis_shape_v(:,1)), maxval(zcis_shape_v(:,1))
!print*, ' LP2 zcis_shape ni+1= ', minval(zcis_shape_v(:,2)), maxval(zcis_shape_v(:,2))
!print*, ' LP2 zcis_tot ni= ', minval(zcis_tot(:)), maxval(zcis_tot(:))
!
! Budget storage
  IF (NBUMOD==KMI .AND. LBU_ENABLE) THEN
    ...
    IF (LBUDGET_SV) THEN
      ...
      ELSE 
        DO JSH = 1, NB_CRYSTAL_SHAPE
          CALL BUDGET (UNPACK(ZCIS_SHAPE(:,JSH),MASK=GNEGT(:,:,:),&
                              FIELD=PCIS_SHAPE(:,:,:,JSH))*PRHODJ(:,:,:),&
                              12+NSV_LIMA_NI+JSH-1,'HIND_BU_RSV')
        END DO
...
!        ZCIS(:) = SUM (ZCIS_SHAPE, DIM =2)
    ...
    END IF
  END DO
!print*, ' LP3 zcis_shape ni= ', minval(zcis_shape_v(:,1)), maxval(zcis_shape_v(:,1))
!print*, ' LP3 zcis_shape ni+1= ', minval(zcis_shape_v(:,2)), maxval(zcis_shape_v(:,2))
!print*, ' LP3 zcis_tot ni= ', minval(zcis_tot(:)), maxval(zcis_tot(:))
!
! Budget storage
  IF (NBUMOD==KMI .AND. LBU_ENABLE) THEN
    IF (LBUDGET_TH) CALL BUDGET (                                                 &
    ...
    IF (LBUDGET_SV) THEN
      ...
      IF (.NOT. LCRYSTAL_SHAPE) THEN
        CALL BUDGET ( UNPACK(ZCIS(:),MASK=GNEGT(:,:,:), &
                             FIELD=PCIS)*PRHODJ(:,:,:),&
      ...
      ELSE
        DO JSH =1, NB_CRYSTAL_SHAPE
          CALL BUDGET ( UNPACK(ZCIS_SHAPE(:,JSH),MASK=GNEGT(:,:,:),&
                               FIELD=PCIS_SHAPE(:,:,:,JSH))*PRHODJ(:,:,:),&
                               12+NSV_LIMA_NI+JSH-1,'HINC_BU_RSV')
        END DO
      END IF ! LCRYSTAL_SHAPE
    END IF
  END IF
...
!
  ...
  ELSE
    DO JSH = 1, NB_CRYSTAL_SHAPE
      ZW(:,:,:) = PCIS_SHAPE(:,:,:,JSH)
      PCIS_SHAPE(:,:,:,JSH) = UNPACK( ZCIS_SHAPE(:,JSH),MASK=GNEGT(:,:,:),FIELD=ZW(:,:,:) )
    END DO
    ...
    IF (LBUDGET_SV) THEN
      ...
      ELSE 
        DO JSH = 1, NB_CRYSTAL_SHAPE
          ...
          CALL BUDGET (ZW,12+NSV_LIMA_NI+JSH-1,'HIND_BU_RSV')
          CALL BUDGET (ZW,12+NSV_LIMA_NI+JSH-1,'HINC_BU_RSV')
        END DO
      END IF
      IF (NMOD_IFN .GE. 1) THEN
         DO JL = 1, NMOD_IFN
           CALL BUDGET ( PIFS(:,:,:,JL)*PRHODJ(:,:,:),&
                         12+NSV_LIMA_IFN_FREE+JL-1,'HIND_BU_RSV') 
         END DO
      END IF
    END IF
  END IF
