...
!!      T. Hoarau      03/2019   Add budget for CIBU
!!      M. Claeys      06/2019   Add budget for ice crystal habits
!!      JP Pinty       07/2019   Add budget for RDSF
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------ 
...
USE MODD_PARAM_LIMA, ONLY : OWARM=>LWARM, OCOLD=>LCOLD, OSEDI=>LSEDI,   &
                            ...
                            NMOD_CCN, OCIBU=>LCIBU, ORDSF=>LRDSF,        &
                            OCRYSTAL_SHAPE=>LCRYSTAL_SHAPE,              &
                            OHABIT_CHANGE=>LHABIT_CHANGE
!
USE MODE_ll
USE MODE_IO_ll
USE MODE_MSG
...
INTEGER :: JSH     ! loop indice for ice crystal shapes
! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
IF (LBU_RRI) THEN
  ...
  IF (HCLOUD=='LIMA') THEN
     ...
     IPROC=IPROC+1
     IF (OPTSPLIT .OR. (OCOLD .AND. OCIBU .AND. OSNOW))      IPROACTV(9,IPROC) = NCIBURI
     IPROC=IPROC+1
     IF (OPTSPLIT .OR. (OCOLD .AND. OWARM .AND. OSNOW))      IPROACTV(9,IPROC) = NCFRZRI
     IPROC=IPROC+1
     IF (OPTSPLIT .OR. (OCOLD .AND. OWARM .AND. ORDSF))      IPROACTV(9,IPROC) = NRDSFRI
     IPROC=IPROC+1
...
!
  ...
  IF (HCLOUD=='LIMA') THEN
     ...
     YWORK2(9,IPROC) = 'HMS_'
     IPROC=  IPROC+1
     YWORK2(9,IPROC) = 'CIBU_'
     IPROC=  IPROC+1
     YWORK2(9,IPROC) = 'CFRZ_'
     IPROC=  IPROC+1
     YWORK2(9,IPROC) = 'RDSF_'
     IPROC=  IPROC+1
...
IF (HCLOUD=='LIMA') THEN
     ...
     IPROC=IPROC+1
     IF (OPTSPLIT .OR. (OCOLD .AND. OCIBU .AND. OSNOW)) IPROACTV(10,IPROC) = NCIBURS
     IPROC=IPROC+1            
...
IF (HCLOUD=='LIMA') THEN
     ...
     IPROC=IPROC+1
     IF (OPTSPLIT .OR. (OCOLD .AND. OWARM .AND. ORDSF))             IPROACTV(11,IPROC) = NRDSFRG
     IPROC=IPROC+1
...
ELSEIF (JSV >= NSV_LIMA_BEG .AND. JSV <= NSV_LIMA_END) THEN  
   ...
   ELSE IF (JSV == NSV_LIMA_NI .AND. .NOT. LCRYSTAL_SHAPE) THEN
      ...
      IF (OPTSPLIT .OR. (OCOLD .AND. OWARM .AND. OSNOW)) THEN
         ...
         IPROACTV(12+JSV,ILAST_PROC_NBR) = 1
      END IF
      IF (OPTSPLIT .OR. (OCOLD .AND. OCIBU .AND. OSNOW)) THEN
         ILAST_PROC_NBR = ILAST_PROC_NBR + 1
         YWORK2(12+JSV,ILAST_PROC_NBR)= 'CIBU_'
         IPROACTV(12+JSV,ILAST_PROC_NBR) = 1
      END IF
      IF (OPTSPLIT .OR. (OCOLD .AND. OWARM .AND. OSNOW)) THEN
         ILAST_PROC_NBR = ILAST_PROC_NBR + 1
         ...
         IPROACTV(12+JSV,ILAST_PROC_NBR) = 1
      END IF
      IF (OPTSPLIT .OR. (OCOLD .AND. OWARM .AND. ORDSF)) THEN
         ILAST_PROC_NBR = ILAST_PROC_NBR + 1
         YWORK2(12+JSV,ILAST_PROC_NBR)= 'RDSF_'
         IPROACTV(12+JSV,ILAST_PROC_NBR) = 1
      END IF
      IF (OPTSPLIT .OR. (OCOLD .AND. OWARM .AND. OSNOW)) THEN
         ILAST_PROC_NBR = ILAST_PROC_NBR + 1
      ...
      IPROACTV(12+JSV,ILAST_PROC_NBR) = 1
      !
   ELSE IF (JSV .GE. NSV_LIMA_NI .AND. JSV .LT. NSV_LIMA_NI + NB_CRYSTAL_SHAPE .AND. LCRYSTAL_SHAPE) THEN
      ! Pristine ice crystals conc.
       ILAST_PROC_NBR = ILAST_PROC_NBR + 1
       YWORK2(12+JSV,ILAST_PROC_NBR)= 'NEGA_'
       IPROACTV(12+JSV,ILAST_PROC_NBR) = 1
       ...
       IF (.NOT.OPTSPLIT .AND. OHAIL) THEN
         ...
         IPROACTV(12+JSV,ILAST_PROC_NBR) = 1
       END IF
       IF (LHABIT_CHANGE) THEN
         ILAST_PROC_NBR = ILAST_PROC_NBR + 1
         YWORK2(12+JSV,ILAST_PROC_NBR)= 'IHAB_'
         IPROACTV(12+JSV,ILAST_PROC_NBR) = 1
       END IF
       ILAST_PROC_NBR = ILAST_PROC_NBR + 1
       YWORK2(12+JSV,ILAST_PROC_NBR)= 'CEDS_'
