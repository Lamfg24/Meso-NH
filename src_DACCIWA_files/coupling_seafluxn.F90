...
#else
 ZHS(:)=-1.
 WRITE (ILUOUT,*) 'WARNING : no HS values from ECMWF or WW3, then it is initialized to a the wave of wind'
#endif
END IF
!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
!
IF (LEMISSALT2LIMA) THEN 
  IBEG = CHS%SVS%NSV_LIMA_CCN_FREE
  IEND = CHS%SVS%NSV_LIMA_CCN_FREE
  ISLT = 1
