...
REAL, DIMENSION(:,:), ALLOCATABLE :: ZHYDRO_TOT
...
ZTHGR(:,:) = ZTHGR(:,:) * 1000.
!
ALLOCATE(ZHYDRO_TOT(IIU,IJU))      ; ZHYDRO_TOT(:,:)      = 0.0
!
ZHYDRO_TOT(:,:) = ZTHIC(:,:) + ZTHRW(:,:) + ZTHCW(:,:) + &
                  ZTHSN(:,:) + ZTHGR(:,:)
!
LBU_MASK(IIB:IIE,IJB:IJE,1) = ZHYDRO_TOT(IIB:IIE,IJB:IJE) >= 0.1 
!
DEALLOCATE(ZTHIC)
DEALLOCATE(ZTHRW)
DEALLOCATE(ZTHCW)
