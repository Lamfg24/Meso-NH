...
!
!+ marine
FUNCTION COUNTJV_2D(LTAB,I1,I2) RESULT(IC)
  LOGICAL, DIMENSION(:,:),   INTENT(IN)             :: LTAB
  INTEGER, DIMENSION(:),     INTENT(INOUT)          :: I1,I2
  INTEGER                                           :: IC
END FUNCTION COUNTJV_2D
!- marine
!
...
!
FUNCTION FIND_SHAPE(ZT) RESULT(SHAPE_NAME)
  REAL, DIMENSION(:),          INTENT(IN)           :: ZT ! temperature 
  CHARACTER(LEN=3), DIMENSION(SIZE(ZT,1))           :: SHAPE_NAME
END FUNCTION FIND_SHAPE
!
END INTERFACE
!
...
FUNCTION FIND_SHAPE(ZT) RESULT(SHAPE_NAME)
!#########################################
! Find the shape that correspond to the temperature regime
IMPLICIT NONE
!
REAL, DIMENSION(:), INTENT(IN)          :: ZT ! temperature 
CHARACTER(LEN=3), DIMENSION(SIZE(ZT,1)) :: SHAPE_NAME
REAl, DIMENSION(SIZE(ZT,1))             :: ZT_CELSIUS

!Conversion en degres celsius
ZT_CELSIUS(:) = ZT(:) - 273.16
  
!++cb++ 4/02/20 pour eviter tout pb de valeur non initialisee, on enleve la condition
! sur -1C
!WHERE ( ((ZT_CELSIUS(:) .LE. -1.0)  .AND. (ZT_CELSIUS(:) .GT. -3.0)) .OR.    &
WHERE (  (ZT_CELSIUS(:) .GT. -3.0) .OR.    &
        ((ZT_CELSIUS(:) .LE. -9.0) .AND. (ZT_CELSIUS(:) .GT. -22.0)) )
  SHAPE_NAME(:) = 'PLA'
END WHERE

WHERE ( ((ZT_CELSIUS(:) .LE. -3.0) .AND. (ZT_CELSIUS(:) .GT. -9.0)) .OR.  &
         (ZT_CELSIUS(:) .LE. -22) )
