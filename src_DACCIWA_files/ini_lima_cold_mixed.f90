...
!!  Thomas Hoarau      04/2019  : add CIBU
!!  Marine Claeys      06/2019  : add several shapes for ice crystals
!!  Jean-Pierre Pinty  07/2019  : add RDSF
!!
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
...
INTEGER :: JSH                ! Loop for the number of crystal shapes 
!
REAL, DIMENSION(8)  :: ZGAMI  ! parameters involving various moments
REAL, DIMENSION(2,4) :: ZGAMI_SHAPE  ! remplace ZGAMI(6) et ZGAMI(7) 
...
SELECT CASE (CPRISTINE_ICE_LIMA)
  CASE('PLAT')
    ...
    XC1I = 1./XPI   ! Plates
  CASE('COLU')
    XAI = 110.798   !2.14E-3   ! Columns
    XBI = 2.91      !1.7       ! Columns
    XC_I = 1.96E5   ! Columns
    XDI = 1.585     ! Columns
!++cb++ 01/04/20 
!    XC1I = 1.211    !0.8      ! Columns 
    XC1I = 0.24
!--cb--
  CASE('BURO')
...
!
  XAI_SHAPE(1) = 0.82    ! relation masse maxdim
  XAI_SHAPE(2) = 110.798 ! Mitchell 1(d < 100 um) 1996
  XAI_SHAPE(3) = 10.0    ! moyenne entre plates et colonnes (jpp)
  XAI_SHAPE(4) = 0.0141
!
  XBI_SHAPE(1) = 2.5  ! relation masse maxdim
  XBI_SHAPE(2) = 2.91 ! Mitchell 1(d < 100um) 1996
  XBI_SHAPE(3) = 2.7  ! relation masse diamètre des columns
  XBI_SHAPE(4) = 2.18
!
  XC_I_SHAPE(1)  = 747.0   ! vitesse de chute maxdim
  XC_I_SHAPE(2)  = 1.96E5
  XC_I_SHAPE(3)  = 7000.0
  XC_I_SHAPE(4)  = 376.0
!
  XDI_SHAPE(1)  = 1.0   ! vitesse de chute maxdim
  XDI_SHAPE(2)  = 1.585
  XDI_SHAPE(3)  = 1.235
  XDI_SHAPE(4)  = 0.217
!
...
!
IF (LCRYSTAL_SHAPE) THEN
  IF (.NOT. (ALLOCATED(XLBEXI_SHAPE))) ALLOCATE(XLBEXI_SHAPE(NB_CRYSTAL_SHAPE))
  IF (.NOT. (ALLOCATED(XLBI_SHAPE)))   ALLOCATE(XLBI_SHAPE(NB_CRYSTAL_SHAPE))
  DO JSH = 1, NB_CRYSTAL_SHAPE
    XLBEXI_SHAPE(JSH) = 1.0 / XBI_SHAPE(JSH)
    XLBI_SHAPE(JSH)   = XAI_SHAPE(JSH) * MOMG(XALPHAI,XNUI,XBI_SHAPE(JSH))
  END DO
END IF
!
XLBEXS = 1.0/(XCXS-XBS)
...
!
IF (LCRYSTAL_SHAPE) THEN
  IF (.NOT. (ALLOCATED(XFSEDCI_SHAPE)))     ALLOCATE(XFSEDCI_SHAPE(NB_CRYSTAL_SHAPE))
  IF (.NOT. (ALLOCATED(XFSEDRI_SHAPE)))     ALLOCATE(XFSEDRI_SHAPE(NB_CRYSTAL_SHAPE))
  IF (.NOT. (ALLOCATED(XFSEDRI_TOT_SHAPE))) ALLOCATE(XFSEDRI_TOT_SHAPE(NB_CRYSTAL_SHAPE))
!
  DO JSH = 1, NB_CRYSTAL_SHAPE
    XFSEDCI_SHAPE(JSH) = XC_I_SHAPE(JSH) * GAMMA_X0D(XNUI+XDI_SHAPE(JSH)/XALPHAI) / &
                         GAMMA_X0D(XNUI) * (ZRHO00)**XCEXVT
    XFSEDRI_SHAPE(JSH) = XC_I_SHAPE(JSH) * GAMMA_X0D(XNUI+(XDI_SHAPE(JSH)+XBI_SHAPE(JSH))/XALPHAI) / &
                         GAMMA_X0D(XNUI+XBI_SHAPE(JSH)/XALPHAI) * (ZRHO00)**XCEXVT
! marine : correction de l'air ?
    XFSEDRI_TOT_SHAPE(JSH) = XAI_SHAPE(JSH) * GAMMA_X0D(XNUI+XBI_SHAPE(JSH)/XALPHAI) / &
                                              GAMMA_X0D(XNUI)
!    XFSEDRI_TOT_SHAPE(JSH) = XAI_SHAPE(JSH) * GAMMA_X0D(XNUI+XBI_SHAPE(JSH)/XALPHAI) / &
!                                              GAMMA_X0D(XNUI) * (ZRHO00)**XCEXVT
  END DO
...
!
IF (LCRYSTAL_SHAPE) THEN
  IF (.NOT. (ALLOCATED(X0DEPI_SHAPE))) ALLOCATE(X0DEPI_SHAPE(NB_CRYSTAL_SHAPE))
  IF (.NOT. (ALLOCATED(X2DEPI_SHAPE))) ALLOCATE(X2DEPI_SHAPE(NB_CRYSTAL_SHAPE))
  DO JSH = 1, NB_CRYSTAL_SHAPE
    X0DEPI_SHAPE(JSH) = (4.0*XPI)*XC1I_SHAPE(JSH)*XF0I*MOMG(XALPHAI,XNUI,1.)
    X2DEPI_SHAPE(JSH) = (4.0*XPI)*XC1I_SHAPE(JSH)*XF2I*XC_I_SHAPE(JSH)*MOMG(XALPHAI,XNUI,XDI_SHAPE(JSH)+2.0)
  END DO
END IF
!
...
!
IF (LCRYSTAL_SHAPE) THEN
  IF (.NOT. (ALLOCATED(XC0DEPIS_SHAPE))) ALLOCATE(XC0DEPIS_SHAPE(NB_CRYSTAL_SHAPE))
  IF (.NOT. (ALLOCATED(XC1DEPIS_SHAPE))) ALLOCATE(XC1DEPIS_SHAPE(NB_CRYSTAL_SHAPE))
  IF (.NOT. (ALLOCATED(XR0DEPIS_SHAPE))) ALLOCATE(XR0DEPIS_SHAPE(NB_CRYSTAL_SHAPE))
  IF (.NOT. (ALLOCATED(XR1DEPIS_SHAPE))) ALLOCATE(XR1DEPIS_SHAPE(NB_CRYSTAL_SHAPE))
  DO JSH = 1, NB_CRYSTAL_SHAPE
    XC0DEPIS_SHAPE(JSH) = ((4.0 * XPI) / (XAI_SHAPE(JSH) * XBI_SHAPE(JSH))) *     &
                          XC1I_SHAPE(JSH) * XF0IS * (XALPHAI / GAMMA_X0D(XNUI)) * &
                          XDICNVS_LIM**(1.0-XBI_SHAPE(JSH))
    XC1DEPIS_SHAPE(JSH) = ((4.0 * XPI) / (XAI_SHAPE(JSH) * XBI_SHAPE(JSH))) *  &
                          XC1I_SHAPE(JSH) * XF1IS * SQRT(XC_I_SHAPE(JSH)) *    &
                          (XALPHAI / GAMMA_X0D(XNUI)) *                        &
                          XDICNVS_LIM**(1.0-XBI_SHAPE(JSH)+(XDI_SHAPE(JSH)+1.0)/2.0)
    XR0DEPIS_SHAPE(JSH) = XC0DEPIS_SHAPE(JSH) * &
                         (XAI_SHAPE(JSH) * XDICNVS_LIM**XBI_SHAPE(JSH))
    XR1DEPIS_SHAPE(JSH) = XC1DEPIS_SHAPE(JSH) * &
                         (XAI_SHAPE(JSH) * XDICNVS_LIM**XBI_SHAPE(JSH))
  END DO
END IF
!
...
IF (LCRYSTAL_SHAPE) THEN
  DO JSH = 1, NB_CRYSTAL_SHAPE
    ZGAMI_SHAPE(1,JSH) = MOMG(XALPHAI,XNUI,3.+XBI_SHAPE(JSH))
    ZGAMI_SHAPE(2,JSH) = MOMG(XALPHAI,XNUI,XBI_SHAPE(JSH))
  END DO
END IF
!
...
!
IF (LCRYSTAL_SHAPE) THEN
  IF (.NOT. (ALLOCATED(XAGGS_RLARGE1_SHAPE))) ALLOCATE(XAGGS_RLARGE1_SHAPE(NB_CRYSTAL_SHAPE))
  IF (.NOT. (ALLOCATED(XAGGS_RLARGE2_SHAPE))) ALLOCATE(XAGGS_RLARGE2_SHAPE(NB_CRYSTAL_SHAPE))
  DO JSH = 1, NB_CRYSTAL_SHAPE
    XAGGS_RLARGE1_SHAPE(JSH) = XKER_ZRNIC_A2*ZGAMI_SHAPE(1,JSH)*XAI_SHAPE(JSH)
    XAGGS_RLARGE2_SHAPE(JSH) = XKER_ZRNIC_A2*ZGAMI_SHAPE(2,JSH)*ZGAMS(2)*XAI_SHAPE(JSH)
  END DO
END IF
!
GFLAG = .TRUE.
...
IF (LCRYSTAL_SHAPE) THEN
  IF (.NOT.(ALLOCATED(XFREFFI_SHAPE))) ALLOCATE(XFREFFI_SHAPE(NB_CRYSTAL_SHAPE))
  DO JSH = 1, NB_CRYSTAL_SHAPE
    XFREFFI_SHAPE(JSH) = 0.5 * ZGAMI(8) * (1.0/XLBI_SHAPE(JSH))**XLBEXI_SHAPE(JSH)
  END DO
END IF
!  
!-------------------------------------------------------------------------------
...
IF (GFLAG) THEN
  ...
  WRITE(UNIT=ILUOUT0,FMT='("            distribution:AL=",E13.6,"NU=",E13.6)') &
                                                      XALPHAI,XNUI
  IF (LCRYSTAL_SHAPE) THEN
    WRITE(UNIT=ILUOUT0,FMT='(" Summary of the ice particule characteristics for each habit")')
    WRITE(UNIT=ILUOUT0,FMT='("      PLATES")')
    WRITE(UNIT=ILUOUT0,FMT='("                   masse: A=",E13.6," B=",E13.6)') &
                                                      XAI_SHAPE(1),XBI_SHAPE(1)
    WRITE(UNIT=ILUOUT0,FMT='("                 vitesse: C=",E13.6," D=",E13.6)') &
                                                      XC_I_SHAPE(1),XDI_SHAPE(1)
    WRITE(UNIT=ILUOUT0,FMT='("            distribution:AL=",E13.6,"NU=",E13.6)') &
                                                      XALPHAI,XNUI
    WRITE(UNIT=ILUOUT0,FMT='("      COLUMNS")')
    WRITE(UNIT=ILUOUT0,FMT='("                   masse: A=",E13.6," B=",E13.6)') &
                                                      XAI_SHAPE(2),XBI_SHAPE(2)
    WRITE(UNIT=ILUOUT0,FMT='("                 vitesse: C=",E13.6," D=",E13.6)') &
                                                      XC_I_SHAPE(2),XDI_SHAPE(2)
    WRITE(UNIT=ILUOUT0,FMT='("            distribution:AL=",E13.6,"NU=",E13.6)') &
                                                      XALPHAI,XNUI
    WRITE(UNIT=ILUOUT0,FMT='("      IRREGULARS")')
    WRITE(UNIT=ILUOUT0,FMT='("                   masse: A=",E13.6," B=",E13.6)') &
                                                      XAI_SHAPE(3),XBI_SHAPE(3)
    WRITE(UNIT=ILUOUT0,FMT='("                 vitesse: C=",E13.6," D=",E13.6)') &
                                                      XC_I_SHAPE(3),XDI_SHAPE(3)
    WRITE(UNIT=ILUOUT0,FMT='("            distribution:AL=",E13.6,"NU=",E13.6)') &
                                                      XALPHAI,XNUI
    WRITE(UNIT=ILUOUT0,FMT='("      DENDRITES")')
    WRITE(UNIT=ILUOUT0,FMT='("                   masse: A=",E13.6," B=",E13.6)') &
                                                      XAI_SHAPE(4),XBI_SHAPE(4)
    WRITE(UNIT=ILUOUT0,FMT='("                 vitesse: C=",E13.6," D=",E13.6)') &
                                                      XC_I_SHAPE(4),XDI_SHAPE(4)
    WRITE(UNIT=ILUOUT0,FMT='("            distribution:AL=",E13.6,"NU=",E13.6)') &
                                                      XALPHAI,XNUI
  END IF
