...
!
      SUBROUTINE BOUNDARIES (                                       &
            ...
            PRHODJ,                                                 &
            PUT,PVT,PWT,PTHT,PTKET,PRT,PSVT,PSRCT,PRHODREF         )
!
...
!!      Modification    11/2014  (Bovalo)   Change cond. to call ION_BOUNDARY
...
!!      Modification    03/2019  (C. Barthe) LIMA-CELLS coupling
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!         
...
USE MODD_PARAM_LIMA, ONLY : NMOD_CCN, NMOD_IFN, LWARM, LCOLD
!
#ifdef MNH_FOREFIRE
...
LOGICAL, SAVE        :: GFIRSTCALLLV = .TRUE.
LOGICAL, SAVE        :: GFIRSTCALLPYR = .TRUE.
!
...
IF (CCLOUD == 'LIMA' .AND. IMI == 1) THEN
  ...
  ENDIF
!
  IF (NMOD_CCN.GT.0 .AND. LWARM) THEN
  ...
  END IF
!
  IF (NMOD_IFN.GT.0 .AND. LCOLD) THEN
...
! 
  IF (SIZE(PSVT)>0) THEN
    CALL DUST_FILTER(PSVT(:,:,:,NSV_DSTBEG:NSV_DSTEND), PRHODREF)
  END IF
ENDIF
!
...
!
  IF (SIZE(PSVT)>0) THEN
    CALL SALT_FILTER(PSVT(:,:,:,NSV_SLTBEG:NSV_SLTEND), PRHODREF)
  END IF
ENDIF
!
