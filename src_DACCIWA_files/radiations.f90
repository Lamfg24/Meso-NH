...
!!      C. Barthe 03/2019    : coupling LIMA and ORILAM
!!      M. Claeys  07/2018    : add LIMA ice cristals concentration
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
USE MODE_FMWRIT
USE MODE_ll
USE MODE_LIMA_DUSTOPT
USE MODE_LIMA_SALTOPT
...
USE MODI_LIMA_AEROOPT_GET
USE MODI_SUM_ll,      ONLY: GMINLOC_ll, MIN_ll
USE MODD_VAR_ll,      ONLY: IP
!+ marine
USE MODD_DYN_n,       ONLY: XTSTEP
USE MODI_LIMA_FUNCTIONS,  ONLY : COUNTJV_2D
!- marine
!++cb++
...
LOGICAL                         :: GDST     ! .TRUE. when CAOP='EXPL' and LDUST
LOGICAL                         :: GSLT     ! .TRUE. when CAOP='EXPL' and LSALT
LOGICAL                         :: GAER     ! .TRUE. when CAOP='EXPL'and LORILAM
...
!+ marine
INTEGER :: IRAD_SHAPE    ! vectorisation of lima variables
LOGICAL, DIMENSION(SIZE(PTHT,1),SIZE(PTHT,2)) :: GRAD_SHAPE
INTEGER , DIMENSION(SIZE(GRAD_SHAPE)) :: I1,I2,I3 ! Used to replace the COUNT
...
!+ marine
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE :: ZCIT_LIMA_SHAPE     ! Ice water Concentration per shape (LIMA)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE :: ZQIAVE_LIMA_SHAPE   ! Ice water mixing ratio per shape kg/gG
! add of size distribution parameters
INTEGER,         DIMENSION(:,:),   ALLOCATABLE :: ISHAPE_MAX
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE :: ZCSHAPE 
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE :: ZLBDAI_SHAPE
! variables vectorisees
INTEGER,         DIMENSION(:),   ALLOCATABLE :: ISHAPE_MAX_V
REAL(KIND=JPRB), DIMENSION(:),   ALLOCATABLE :: ZQIAVE_V
REAL(KIND=JPRB), DIMENSION(:),   ALLOCATABLE :: ZCIT_LIMA_V
REAL(KIND=JPRB), DIMENSION(:,:), ALLOCATABLE :: ZLBDAI_SHAPE_V 
REAL(KIND=JPRB), DIMENSION(:,:), ALLOCATABLE :: ZCIT_LIMA_SHAPE_V ! Ice water Concentration(LIMA)
REAL(KIND=JPRB), DIMENSION(:,:), ALLOCATABLE :: ZQIAVE_LIMA_SHAPE_V
!
!- marine
...
!REAL, DIMENSION(:,:),   ALLOCATABLE :: ZRADLP
REAL, DIMENSION(:,:,:), ALLOCATABLE :: ZRADIP_SHAPE
!REAL, DIMENSION(:,:,:), ALLOCATABLE :: ZRADIP_SHAPE_1, ZRADIP_SHAPE_2, &
!                                       ZRADIP_SHAPE_3, ZRADIP_SHAPE_4
...
!+ marine
REAL,    DIMENSION(:,:), ALLOCATABLE :: ZWORK1_2D, ZWORK2_2D
!- marine
...
!+ marine : intervention ici si la precedente est gardee
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE  :: ZCIT_LIMA_SHAPE_SPLIT
!
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE  :: ZQIAVE_LIMA_SHAPE_SPLIT
! marine : Variables necessaires pour calculer les mr de chaque ice crystal
! shape

! marine question : besoin de ces variables vectorisees pour le calcul de lambda
! ?
INTEGER,         DIMENSION(:,:),   ALLOCATABLE :: ISHAPE_MAX_SPLIT
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE :: ZCSHAPE_SPLIT
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE :: ZLBDAI_SHAPE_SPLIT
!
!- marine
...
REAL, DIMENSION(:,:,:), ALLOCATABLE :: ZRADIP_SHAPE_SPLIT
!- marine
...
REAL, DIMENSION(:,:,:), ALLOCATABLE :: ZAER_AER      !tau/tau_{550} aerosol from ORILAM               (lon,lat,lev,wvl)
REAL, DIMENSION(:,:,:), ALLOCATABLE :: ZAER_SLT      !tau/tau_{550} sea salt               (lon,lat,lev,wvl)
REAL, DIMENSION(:,:,:), ALLOCATABLE :: ZAER_DST     !tau/tau_{550} dust               (lon,lat,lev,wvl)
REAL, DIMENSION(:,:,:), ALLOCATABLE :: ZTAU550_EQ_TMP      !tau/tau_{550} aerosols               (lon,lat,lev,wvl)
!
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZPIZA_DST            !Single scattering albedo of dust (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZCGA_DST             !Assymetry factor dust            (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZTAUREL_DST          !tau/tau_{550} dust               (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZPIZA_DST_SPLIT      !Single scattering albedo of dust (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZCGA_DST_SPLIT       !Assymetry factor dust            (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZTAUREL_DST_SPLIT    !tau/tau_{550} dust               (points,lev,wvl)
REAL, DIMENSION(KFLEV,KSWB_OLD)           :: ZPIZA_DST_CLEAR      !Single scattering albedo of dust (lev,wvl)
REAL, DIMENSION(KFLEV,KSWB_OLD)           :: ZCGA_DST_CLEAR       !Assymetry factor dust            (lev,wvl)
REAL, DIMENSION(KFLEV,KSWB_OLD)           :: ZTAUREL_DST_CLEAR    !tau/tau_{550} dust               (lev,wvl)
!
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZPIZA_SLT            !Single scattering albedo of salt (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZCGA_SLT             !Assymetry factor salt            (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZTAUREL_SLT          !tau/tau_{550} salt               (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZPIZA_SLT_SPLIT      !Single scattering albedo of salt (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZCGA_SLT_SPLIT       !Assymetry factor salt            (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZTAUREL_SLT_SPLIT    !tau/tau_{550} salt               (points,lev,wvl)
REAL, DIMENSION(KFLEV,KSWB_OLD)           :: ZPIZA_SLT_CLEAR      !Single scattering albedo of salt (lev,wvl)
REAL, DIMENSION(KFLEV,KSWB_OLD)           :: ZCGA_SLT_CLEAR       !Assymetry factor salt            (lev,wvl)
REAL, DIMENSION(KFLEV,KSWB_OLD)           :: ZTAUREL_SLT_CLEAR    !tau/tau_{550} salt               (lev,wvl)
!
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZPIZA_AER            !Single scattering albedo of orilam  (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZCGA_AER             !Assymetry factor orilam             (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZTAUREL_AER          !tau/tau_{550} orilam                (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZPIZA_AER_SPLIT      !Single scattering albedo of orilam  (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZCGA_AER_SPLIT       !Assymetry factor orilam             (points,lev,wvl)
REAL(KIND=JPRB), DIMENSION(:,:,:), ALLOCATABLE   :: ZTAUREL_AER_SPLIT    !tau/tau_{550} orilam                (points,lev,wvl)
REAL, DIMENSION(KFLEV,KSWB_OLD)           :: ZPIZA_AER_CLEAR      !Single scattering albedo of orilam  (lev,wvl)
REAL, DIMENSION(KFLEV,KSWB_OLD)           :: ZCGA_AER_CLEAR       !Assymetry factor orilam             (lev,wvl)
REAL, DIMENSION(KFLEV,KSWB_OLD)           :: ZTAUREL_AER_CLEAR    !tau/tau_{550} orilam                (lev,wvl)
...
!
!++cb++
LOGICAL :: LKEEP_SHAPE ! when true, the different ice crystal shapes are used
                       ! otherwise, total concentration of ice crystals are used 
!--cb--
!-------------------------------------------------------------------------
!-------------------------------------------------------------------------
!-------------------------------------------------------------------------
...
!+ marine
LKEEP_SHAPE = .FALSE.
ALLOCATE(ISHAPE_MAX(KDLON,KFLEV))
ISHAPE_MAX(:,:) = 0
IF (CCLOUD == 'LIMA' .AND. LCRYSTAL_SHAPE .AND. LECRAD_YANG) THEN
  LKEEP_SHAPE = .TRUE.
  ALLOCATE(ZQIAVE_LIMA_SHAPE(KDLON,KFLEV,NB_CRYSTAL_SHAPE))
  ALLOCATE(ZCSHAPE(KDLON,KFLEV,NB_CRYSTAL_SHAPE)) 
  ALLOCATE(ZLBDAI_SHAPE(KDLON,KFLEV,NB_CRYSTAL_SHAPE))
!
  ZQIAVE_LIMA_SHAPE(:,:,:) = 0.0
END IF
!
!++cb++22/04/20
IF (.NOT. LKEEP_SHAPE .AND. (CCLOUD == 'LIMA' .AND. LCRYSTAL_SHAPE)) THEN
  ALLOCATE(ZCSHAPE(KDLON,KFLEV,NB_CRYSTAL_SHAPE))
  ALLOCATE(ZLBDAI_SHAPE(KDLON,KFLEV,NB_CRYSTAL_SHAPE))
END IF
!--cb--
...
!
!
!        2.1*bis pronostic water concentration fields (LIMA coupling) 
!
!+ marine : question : introduire le flag LSHAPE ou s'en affranchir et fixer a zero si necessaire 
IF( CCLOUD == 'LIMA' ) THEN
  ...
  IF (LCRYSTAL_SHAPE) THEN
    ALLOCATE (ZCIT_LIMA_SHAPE(KDLON, KFLEV, NB_CRYSTAL_SHAPE))
    ZCIT_LIMA_SHAPE(:,:,:) = 0.
  END IF
!
!print*, 'nb_crystal_shape = ', nb_crystal_shape
!print*, 'psvt = ', nsv_lima_ni, minval(psvt(:,:,:,nsv_lima_ni)), maxval(psvt(:,:,:,nsv_lima_ni))
!print*, 'psvt = ', nsv_lima_ni+1, minval(psvt(:,:,:,nsv_lima_ni+1)), maxval(psvt(:,:,:,nsv_lima_ni+1))
  DO JK=IKB,IKE
    JKRAD = JK-JPVEXT
    DO JJ=IJB,IJE
      DO JI=IIB,IIE
        ...
        IF (LCOLD .AND. .NOT. LCRYSTAL_SHAPE) THEN
          ZCIT_LIMA(IIJ,JKRAD) = MAX(0.,PSVT(JI,JJ,JK,NSV_LIMA_NI))
        ELSE IF (LCOLD .AND. LCRYSTAL_SHAPE) THEN
          DO JSH = 1, NB_CRYSTAL_SHAPE
            ZCIT_LIMA_SHAPE(IIJ,JKRAD,JSH) = MAX(0.,PSVT(JI,JJ,JK,NSV_LIMA_NI+JSH-1))
          END DO
        END IF
      END DO
    END DO
  ...
  IF (LCOLD .AND. LCRYSTAL_SHAPE) THEN
    ZCIT_LIMA(:,:) = SUM(ZCIT_LIMA_SHAPE, DIM=3) ! Total ice crystal concentration
  END IF
!  IF (.NOT. LKEEP_SHAPE .AND. ALLOCATED(ZCIT_LIMA_SHAPE)) DEALLOCATE(ZCIT_LIMA_SHAPE)
!
!++cb++ 22/04/20 on doit calculer ishape_max meme si on n'active pas ecrad et yang
! compute the mixing ratio of ice crystal per shape
  IF (CCLOUD == 'LIMA' .AND. LCRYSTAL_SHAPE) THEN
    ZCSHAPE(:,:,:) = 0. 
    ZLBDAI_SHAPE(:,:,:) = 1.E10 
! ratio ice conc. per shape / tot. ice conc.
    DO JSH = 1, NB_CRYSTAL_SHAPE ! marine question : nb_crystal_shape ou 4
      WHERE ((ZCIT_LIMA_SHAPE(:,:,JSH) .GT. 0.0) .AND. (ZCIT_LIMA(:,:) .GT. 0.0))
        ZCSHAPE(:,:,JSH) = ZCIT_LIMA_SHAPE(:,:,JSH) / ZCIT_LIMA(:,:)
      END WHERE
    END DO
! find the dominant habit per grid mesh
    WHERE (ZCIT_LIMA(:,:) .GT. 0.0)
      ISHAPE_MAX(:,:) = MAXLOC(ZCSHAPE,DIM=3)
    ELSEWHERE
      ISHAPE_MAX(:,:) = 0
    END WHERE
!print*, 'zcit_lima_shape 1 = ', minval(zcit_lima_shape(:,:,1)), maxval(zcit_lima_shape(:,:,1))
!print*, 'zcit_lima_shape 2 = ', minval(zcit_lima_shape(:,:,2)), maxval(zcit_lima_shape(:,:,2))
!print*, 'rad : count ishape_max = 1 : ', count(ishape_max(:,:) .eq. 1)
!print*, 'rad : count ishape_max = 2 : ', count(ishape_max(:,:) .eq. 2)
!print*, 'rad : count ishape_max =/= : ', count(ishape_max(:,:) .ne. 1 .and. &
!                                               ishape_max(:,:) .ne. 2)
  END IF
!--cb--
!
  IF (LKEEP_SHAPE) THEN
    GRAD_SHAPE(:,:) = .FALSE.
    GRAD_SHAPE(IIB:IIE,IJB:IJE) = ZCIT_LIMA(IIB:IIE,IJB:IJE) > 0.0
    IRAD_SHAPE = COUNTJV_2D( GRAD_SHAPE(:,:), I1(:), I2(:))
!
    IF (IRAD_SHAPE >= 1 ) THEN
      ALLOCATE(ISHAPE_MAX_V(IRAD_SHAPE))
      ALLOCATE(ZQIAVE_V(IRAD_SHAPE))
      ALLOCATE(ZCIT_LIMA_V(IRAD_SHAPE))
      ALLOCATE(ZLBDAI_SHAPE_V(IRAD_SHAPE, NB_CRYSTAL_SHAPE))
      ALLOCATE(ZCIT_LIMA_SHAPE_V(IRAD_SHAPE, NB_CRYSTAL_SHAPE))
      ALLOCATE(ZQIAVE_LIMA_SHAPE_V(IRAD_SHAPE, NB_CRYSTAL_SHAPE))

      DO JL = 1, IRAD_SHAPE
        ISHAPE_MAX_V(JL) = ISHAPE_MAX(I1(JL), I2(JL))
        ZQIAVE_V(JL) = ZQIAVE(I1(JL), I2(JL))
        ZCIT_LIMA_V(JL) = ZCIT_LIMA(I1(JL), I2(JL))
        DO JSH = 1, NB_CRYSTAL_SHAPE
          ZLBDAI_SHAPE_V(JL,JSH) = ZLBDAI_SHAPE(I1(JL), I2(JL),JSH)
          ZCIT_LIMA_SHAPE_V(JL,JSH) = ZCIT_LIMA_SHAPE(I1(JL), I2(JL),JSH)
          ZQIAVE_LIMA_SHAPE_V(JL,JSH) = ZQIAVE_LIMA_SHAPE(I1(JL), I2(JL),JSH)
        END DO
      END DO

! compute the ice mixing ratio per shape
      CALL COMPUTE_LBDA_SHAPE(ISHAPE_MAX_V, IRAD_SHAPE , XTSTEP,     &
                              ZQIAVE_V, ZCIT_LIMA_V, ZLBDAI_SHAPE_V, &
                              ZQIAVE_V, ZCIT_LIMA_SHAPE_V, ZQIAVE_LIMA_SHAPE_V)
!
! Unpack variable (output of compute_lbda_shape)
      ALLOCATE(ZWORK1_2D(KDLON, KFLEV))

      DO JSH = 1, NB_CRYSTAL_SHAPE
        ZWORK1_2D(:,:)= ZQIAVE_LIMA_SHAPE(:,:,JSH)
        ZQIAVE_LIMA_SHAPE(:,:,JSH) = UNPACK( ZQIAVE_LIMA_SHAPE_V(:,JSH),     &
                MASK = GRAD_SHAPE(:,:), FIELD = ZWORK1_2D(:,:))
      END DO

!      DEALLOCATE(ZCSHAPE)
!      DEALLOCATE(ISHAPE_MAX)
      DEALLOCATE(ZWORK1_2D)
      DEALLOCATE(ISHAPE_MAX_V)
      DEALLOCATE(ZQIAVE_V)
      DEALLOCATE(ZCIT_LIMA_V)
      DEALLOCATE(ZLBDAI_SHAPE_V)
      DEALLOCATE(ZCIT_LIMA_SHAPE_V)
      DEALLOCATE(ZQIAVE_LIMA_SHAPE_V)
    ELSE
      ZCIT_LIMA_SHAPE(:,:,:) = 0.
      ZQIAVE_LIMA_SHAPE(:,:,:) = 0.
    END IF ! irad_shape >= 1
  END IF ! lkeep_shape
  IF (ALLOCATED(ZCSHAPE)) DEALLOCATE(ZCSHAPE)
  IF (ALLOCATED(ZLBDAI_SHAPE)) DEALLOCATE(ZLBDAI_SHAPE)
ELSE
  ALLOCATE (ZCCT_LIMA(0,0))
  ALLOCATE (ZCRT_LIMA(0,0))
  ALLOCATE (ZCIT_LIMA(0,0))
END IF
!- marine
!
!-------------------------------------------------------------------------------
...
PRINT *, "LDUST",LDUST
PRINT *, "LSALT",LSALT
!IF (CAOP=='EXPL' .AND. (LDUST .OR. CCLOUD=='LIMA')) THEN !!! LAMBERT !!!
IF (CAOP=='EXPL' .AND. (LDUST .AND. CCLOUD=='LIMA')) THEN
  GDST = .TRUE.
ELSE
  GDST = .FALSE.
ENDIF
!
!IF (CAOP=='EXPL' .AND. (LSALT .OR. CCLOUD=='LIMA')) THEN
IF (CAOP=='EXPL' .AND. (LSALT .AND. CCLOUD=='LIMA')) THEN
  GSLT = .TRUE.
ELSE
  GSLT = .FALSE.
ENDIF
!
IF (CAOP=='EXPL' .AND. (LORILAM .OR. CCLOUD=='LIMA')) THEN
  GAER = .TRUE.
ELSE
  GAER = .FALSE.
ENDIF
...
!
  ...
  ALLOCATE(ZAER_DST(SIZE(PAER,1),SIZE(PAER,2),SIZE(PAER,3)))
  ...
  ALLOCATE(ZAER_AER(SIZE(PAER,1),SIZE(PAER,2),SIZE(PAER,3)))
  ...
  ALLOCATE(ZAER_SLT(SIZE(PAER,1),SIZE(PAER,2),SIZE(PAER,3)))
  ...
  IF (LORILAM) THEN
    CALL AEROOPT_GET(                             &
         ...
         ,ZAER_AER(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT)            &  !O [-] optical depth of aerosols at wvl=550nm
...
!
    ZTAUREL_AER_TMP(:,:,:,:) = MAX(1.E-8,ZTAUREL_AER_TMP(:,:,:,:))
    ZCGA_AER_TMP(:,:,:,:)    = MAX(1.E-8,ZCGA_AER_TMP(:,:,:,:))
    ZPIZA_AER_TMP(:,:,:,:)   = MAX(1.E-8,ZPIZA_AER_TMP(:,:,:,:))
    PAER(:,:,:,4)            = MAX(1.E-8,ZAER_AER(:,:,:))
    ZPIZA_AER_TMP(:,:,:,:)   = MIN(0.99,ZPIZA_AER_TMP(:,:,:,:))
  ENDIF
!
  IF(LDUST) THEN
!
    CALL DUSTOPT_GET(                             &
         ...
         ,ZAER_DST(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT)            &  !O [-] optical depth of dust at wvl=550nm
         ,KSWB_OLD                                    &  !I |nbr] number of shortwave bands 
         )
!
    ...
    ZTAUREL_DST_TMP(:,:,:,:) = MAX(1.E-8,ZTAUREL_DST_TMP(:,:,:,:))
    ZCGA_DST_TMP(:,:,:,:)    = MAX(1.E-8,ZCGA_DST_TMP(:,:,:,:))
    ZPIZA_DST_TMP(:,:,:,:)   = MAX(1.E-8,ZPIZA_DST_TMP(:,:,:,:))
    PAER(:,:,:,3)            = MAX(1.E-8,ZAER_DST(:,:,:))
    ZPIZA_DST_TMP(:,:,:,:)   = MIN(0.99,ZPIZA_DST_TMP(:,:,:,:))
  ENDIF
!
  IF(LSALT) THEN
    CALL SALTOPT_GET(                             &
         ...
         ,ZAER_SLT(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT)            &  !O [-] optical depth of sea salt at wvl=550nm
         ,KSWB_OLD                                    &  !I |nbr] number of shortwave bands
         )
!
    ZTAUREL_SLT_TMP(:,:,:,:) = MAX(1.E-8,ZTAUREL_SLT_TMP(:,:,:,:))
    ZCGA_SLT_TMP(:,:,:,:)    = MAX(1.E-8,ZCGA_SLT_TMP(:,:,:,:))
    ZPIZA_SLT_TMP(:,:,:,:)   = MAX(1.E-8,ZPIZA_SLT_TMP(:,:,:,:))
    PAER(:,:,:,2)            = MAX(1.E-8,ZAER_SLT(:,:,:))
    ZPIZA_SLT_TMP(:,:,:,:)   = MIN(0.99,ZPIZA_SLT_TMP(:,:,:,:))
  ENDIF
!
  IF (CCLOUD == 'LIMA') THEN
!
! Dust and sea salt are treated separately.
! Sulfates, OC and BC are in the LIMA_AEROOPT_GET subroutine
! The default CCN type "M" (maritime) is associated with sea salt
! The default CCN type "C" (continental) is associated with a mix of Su, BC and OC
! If using CCN from MACC analyses, mode 1 is sea salt, mode 2 is sulfates and mode 3 is a mix of BC and OC
! IFN are partitioned in 2 modes of dust, one of BC and one of OC
!
    IF (.NOT. LORILAM) THEN
      IF ( (LWARM .AND. NMOD_CCN.GE.1) .OR. (LCOLD .AND. NMOD_IFN.GE.1) ) THEN
        CALL LIMA_AEROOPT_GET(                                         &
          PSVT(IIB:IIE,IJB:IJE,:,NSV_LIMA_BEG:NSV_LIMA_END)         &  !I [ppp] aerosols concentration
         ,PZZ(IIB:IIE,IJB:IJE,:)                                   &  !I [m] height of layers
         ,PRHODREF(IIB:IIE,IJB:IJE,:)                              &  !I [kg/m3] density of air
         ,ZPIZA_AER_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,:)   &  !O [-] single scattering albedo of aerosols
         ,ZCGA_AER_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,:)    &  !O [-] assymetry factor for aerosols
         ,ZTAUREL_AER_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,:) &  !O [-] opt.depth(wvl=lambda)/opt.depth(wvl=
         ,ZAER_AER(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT)          &  !O [-] optical depth of aerosols at wvl=550
         ,KSWB_OLD                                                 &  !I |nbr] number of shortwave bands
         ,ZIR(IIB:IIE,IJB:IJE,:,:)                                 &  !O [-] opt.depth(wvl=lambda)/opt.depth(wvl=
         ,ZII(IIB:IIE,IJB:IJE,:,:)                                 &  !O [-] opt.depth(wvl=lambda)/opt.depth(wvl=
          )
!
        ZTAUREL_AER_TMP(:,:,:,:) = MAX(1.E-8,ZTAUREL_AER_TMP(:,:,:,:))
        ZCGA_AER_TMP(:,:,:,:)    = MAX(1.E-8,ZCGA_AER_TMP(:,:,:,:))
    ...
    IF (.NOT. LDUST) THEN
      IF ( LCOLD .AND. NMOD_IFN.GE.1 ) THEN
        CALL LIMA_DUSTOPT_GET(                                         &
          PSVT(IIB:IIE,IJB:IJE,:,NSV_LIMA_BEG:NSV_LIMA_END)        &  !I [ppp] Dust scalar concentration
         ,PZZ(IIB:IIE,IJB:IJE,:)                                   &  !I [m] height of layers
         ,PRHODREF(IIB:IIE,IJB:IJE,:)                              &  !I [kg/m3] density of air
         ,ZPIZA_DST_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,:)   &  !O [-] single scattering albedo of dust
         ,ZCGA_DST_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,:)    &  !O [-] assymetry factor for dust
         ,ZTAUREL_DST_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,:) &  !O [-] opt.depth(wvl=lambda)/opt.depth(wvl=
         ,ZAER_DST(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT)          &  !O [-] optical depth of dust at wvl=550nm
         ,KSWB_OLD                                                     &  !I |nbr] number of shortwave bands
         )
!
    ...
    IF (.NOT. LSALT) THEN
      IF ( LWARM .AND. NMOD_CCN.GE.1) THEN
        CALL LIMA_SALTOPT_GET(                                         &
          PSVT(IIB:IIE,IJB:IJE,:,NSV_LIMA_BEG:NSV_LIMA_END)        &  !I [ppp] sea salt scalar concentration
         ,PZZ(IIB:IIE,IJB:IJE,:)                                   &  !I [m] height of layers
         ,PRHODREF(IIB:IIE,IJB:IJE,:)                              &  !I [kg/m3] density of air
         ,ZPIZA_SLT_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,:)   &  !O [-] single scattering albedo of sea salt
         ,ZCGA_SLT_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,:)    &  !O [-] assymetry factor for sea salt
         ,ZTAUREL_SLT_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,:) &  !O [-] opt.depth(wvl=lambda)/opt.depth(wvl=
         ,ZAER_SLT(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT)          &  !O [-] optical depth of sea salt at wvl=550
         ,KSWB_OLD                                                     &  !I |nbr] number of shortwave bands
         )
!
        ZTAUREL_SLT_TMP(:,:,:,:) = MAX(1.E-8,ZTAUREL_SLT_TMP(:,:,:,:))
  ...
  DO JAE = 1, KAER
    ...
    IF (GDST.AND.(JAE==3)) THEN
      ...
      ZTAUAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX)= ZTAUAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) + &
                                     ...
                                     ZTAUREL_DST_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX) 
      !wi*ti
      ZPIZAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX)= ZPIZAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) + &
                               PAER(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,JAE)                * &
                               ZTAUREL_DST_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX) * &
                               ZPIZA_DST_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX)
      !wi*ti*gi
      ZCGAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) = ZCGAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) + &
                               PAER(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,JAE)                * &
                               ZTAUREL_DST_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX) * &
                               ZPIZA_DST_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX)   * &
                               ZCGA_DST_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX)

    !Special optical properties for salt
    ELSEIF (GSLT.AND.(JAE==2)) THEN
      !Ponderation of aerosol optical in case of explicit optical factor
      !ti
      ZTAUAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX)= ZTAUAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) + &
                                               PAER(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,JAE) * &
                                     ZTAUREL_SLT_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX) 
      !wi*ti
      ZPIZAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX)= ZPIZAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) + &
                                               PAER(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,JAE)                * &
                                     ZTAUREL_SLT_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX) * &
                                     ZPIZA_SLT_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX)
      !wi*ti*gi
      ZCGAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) = ZCGAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) + &
                       PAER(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,JAE)                * &
                       ZTAUREL_SLT_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX) * &
                       ZPIZA_SLT_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX)   * &
                       ZCGA_SLT_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX)

    !Special optical properties for orilam
    ELSEIF (GAER .AND.(JAE==4)) THEN
      !Ponderation of aerosol optical in case of explicit optical factor
      !ti
      ZTAUAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX)= ZTAUAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) + &
                                       PAER(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,JAE) * &
                             ZTAUREL_AER_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX) 
      !wi*ti
      ZPIZAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX)= ZPIZAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) + &
                        PAER(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,JAE)                * &
                        ZTAUREL_AER_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX) * &
                        ZPIZA_AER_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX)
      !wi*ti*gi
      ZCGAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) = ZCGAZ(IIB:IIE,IJB:IJE,IKB:IKE,WVL_IDX) + &
                       PAER(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,JAE)                * &
                       ZTAUREL_AER_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX) * &
                       ZPIZA_AER_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX)   * &
                       ZCGA_AER_TMP(IIB:IIE,IJB:IJE,IKB-JPVEXT:IKE-JPVEXT,WVL_IDX)
    ELSE
...
END IF
!
ALLOCATE(ZPIZA_DST(KDLON,KFLEV,KSWB_OLD))
ALLOCATE(ZCGA_DST(KDLON,KFLEV,KSWB_OLD)) 
ALLOCATE(ZTAUREL_DST(KDLON,KFLEV,KSWB_OLD)) 
ALLOCATE(ZPIZA_SLT(KDLON,KFLEV,KSWB_OLD)) 
ALLOCATE(ZCGA_SLT(KDLON,KFLEV,KSWB_OLD)) 
ALLOCATE(ZTAUREL_SLT(KDLON,KFLEV,KSWB_OLD)) 
ALLOCATE(ZPIZA_AER(KDLON,KFLEV,KSWB_OLD)) 
ALLOCATE(ZCGA_AER(KDLON,KFLEV,KSWB_OLD)) 
ALLOCATE(ZTAUREL_AER(KDLON,KFLEV,KSWB_OLD)) 
!
ZPIZA_DST(:,:,:)   = 0.
ZCGA_DST(:,:,:)    = 0.
ZTAUREL_DST(:,:,:) = 0.
...
!to vectors of type #points, #levs, #wavelengths
  IF (LDUST .OR. CCLOUD=='LIMA') THEN
    DO JJ=IJB,IJE
      DO JI=IIB,IIE
        IIJ = 1 + (JI-IIB) + (IIE-IIB+1)*(JJ-IJB)
        ZPIZA_DST(IIJ,:,:)   = ZPIZA_DST_TMP(JI,JJ,:,:)
        ZCGA_DST(IIJ,:,:)    = ZCGA_DST_TMP(JI,JJ,:,:)
        ZTAUREL_DST(IIJ,:,:) = ZTAUREL_DST_TMP(JI,JJ,:,:)
      END DO
    END DO
  END IF
  ...
  DEALLOCATE(ZAER_DST)
!
  IF (LSALT .OR. CCLOUD=='LIMA') THEN
    DO JJ=IJB,IJE
      DO JI=IIB,IIE
        IIJ = 1 + (JI-IIB) + (IIE-IIB+1)*(JJ-IJB)
        ZPIZA_SLT(IIJ,:,:)   = ZPIZA_SLT_TMP(JI,JJ,:,:)
        ZCGA_SLT(IIJ,:,:)    = ZCGA_SLT_TMP(JI,JJ,:,:)
        ZTAUREL_SLT(IIJ,:,:) = ZTAUREL_SLT_TMP(JI,JJ,:,:)
      END DO
    END DO
  END IF
  ...
  DEALLOCATE(ZAER_SLT)
!
  IF (LORILAM .OR. CCLOUD=='LIMA') THEN
    DO JJ=IJB,IJE
      DO JI=IIB,IIE
        IIJ = 1 + (JI-IIB) + (IIE-IIB+1)*(JJ-IJB)
        ZPIZA_AER(IIJ,:,:)   = ZPIZA_AER_TMP(JI,JJ,:,:)
        ZCGA_AER(IIJ,:,:)    = ZCGA_AER_TMP(JI,JJ,:,:)
        ZTAUREL_AER(IIJ,:,:) = ZTAUREL_AER_TMP(JI,JJ,:,:)
      END DO
    END DO
  END IF
...
!!$  IF( ICLEAR_COL /=0 ) THEN ! at least one clear-sky column exists -> average profiles on clear columns
  IF( ZCLEAR_COL_ll /= 0.0  ) THEN ! at least one clear-sky column exists -> average profiles on clear columns
    ...
    IF(CAOP=='EXPL')THEN
      IF(LDUST .OR. CCLOUD=='LIMA')THEN
        DO WVL_IDX=1,KSWB_OLD
          ZPIZA_DST_CLEAR(:,WVL_IDX)   = SUM_DD_R2_R1_ll(REAL(ZPIZA_DST(INDEX_ICLEAR_COL(:),:,WVL_IDX))) / ZCLEAR_COL_ll
          ZCGA_DST_CLEAR(:,WVL_IDX)    = SUM_DD_R2_R1_ll(REAL(ZCGA_DST(INDEX_ICLEAR_COL(:),:,WVL_IDX))) / ZCLEAR_COL_ll
          ZTAUREL_DST_CLEAR(:,WVL_IDX) = SUM_DD_R2_R1_ll(REAL(ZTAUREL_DST(INDEX_ICLEAR_COL(:),:,WVL_IDX))) / ZCLEAR_COL_ll
        ENDDO
      ENDIF
      IF(LSALT .OR. CCLOUD=='LIMA')THEN
        DO WVL_IDX=1,KSWB_OLD
          ZPIZA_SLT_CLEAR(:,WVL_IDX)   = SUM_DD_R2_R1_ll(REAL(ZPIZA_SLT(INDEX_ICLEAR_COL(:),:,WVL_IDX))) / ZCLEAR_COL_ll
          ZCGA_SLT_CLEAR(:,WVL_IDX)    = SUM_DD_R2_R1_ll(REAL(ZCGA_SLT(INDEX_ICLEAR_COL(:),:,WVL_IDX))) / ZCLEAR_COL_ll
          ZTAUREL_SLT_CLEAR(:,WVL_IDX) = SUM_DD_R2_R1_ll(REAL(ZTAUREL_SLT(INDEX_ICLEAR_COL(:),:,WVL_IDX))) / ZCLEAR_COL_ll
        ENDDO
      ENDIF
      IF(LORILAM .OR. CCLOUD=='LIMA')THEN
        DO WVL_IDX=1,KSWB_OLD
          ZPIZA_AER_CLEAR(:,WVL_IDX)   = SUM_DD_R2_R1_ll(REAL(ZPIZA_AER(INDEX_ICLEAR_COL(:),:,WVL_IDX))) / ZCLEAR_COL_ll
          ZCGA_AER_CLEAR(:,WVL_IDX)    = SUM_DD_R2_R1_ll(REAL(ZCGA_AER(INDEX_ICLEAR_COL(:),:,WVL_IDX))) / ZCLEAR_COL_ll
          ZTAUREL_AER_CLEAR(:,WVL_IDX) = SUM_DD_R2_R1_ll(REAL(ZTAUREL_AER(INDEX_ICLEAR_COL(:),:,WVL_IDX))) / ZCLEAR_COL_ll
        ENDDO
      ENDIF
    END IF
    !
...
!
  ELSE ! no clear columns -> the first column is chosen, without physical meaning: it will not be
    ...
    IF(CAOP=='EXPL')THEN
      IF(LDUST .OR. CCLOUD=='LIMA')THEN
        ZPIZA_DST_CLEAR(:,:)=ZPIZA_DST(1,:,:)
        ZCGA_DST_CLEAR(:,:)=ZCGA_DST(1,:,:)
        ZTAUREL_DST_CLEAR(:,:)=ZTAUREL_DST(1,:,:)
      ENDIF
      IF(LSALT .OR. CCLOUD=='LIMA')THEN
        ZPIZA_SLT_CLEAR(:,:)=ZPIZA_SLT(1,:,:)
        ZCGA_SLT_CLEAR(:,:)=ZCGA_SLT(1,:,:)
        ZTAUREL_SLT_CLEAR(:,:)=ZTAUREL_SLT(1,:,:)
      ENDIF
      IF(LORILAM .OR. CCLOUD=='LIMA')THEN
        ZPIZA_AER_CLEAR(:,:)=ZPIZA_AER(1,:,:)
        ZCGA_AER_CLEAR(:,:)=ZCGA_AER(1,:,:)
        ZTAUREL_AER_CLEAR(:,:)=ZTAUREL_AER(1,:,:)
      ENDIF
    END IF
!
...
!+ marine : for ice crystal shapes
  IF (LKEEP_SHAPE) THEN
    ALLOCATE(ZWORK1_2D(ICLOUD, NB_CRYSTAL_SHAPE))
    ALLOCATE(ZWORK2_2D(ICLOUD+KFLEV, NB_CRYSTAL_SHAPE)) !  allocation for the KFLEV levels of
                                   !  the ICLOUD cloudy columns
                                   !  and of the KFLEV levels of the clear sky one
  END IF
!- marine
...
!+ marine
  IF (LKEEP_SHAPE) THEN
    DO JSH = 1, NB_CRYSTAL_SHAPE
      ZWORK1(:) = PACK( TRANSPOSE(ZQIAVE_LIMA_SHAPE(:,:,JSH)),MASK=GCLOUDT(:,:))
      ZWORK2_2D(1:ICLOUD,JSH) = ZWORK1(1:ICLOUD) ! fills the ICLOUD_COL cloudy columns
      ZWORK2_2D(ICLOUD+1:,JSH)= 0.0      !   and the single clear_sky one
    END DO
    DEALLOCATE(ZQIAVE_LIMA_SHAPE)
    ALLOCATE(ZQIAVE_LIMA_SHAPE(ICLOUD_COL+1,KFLEV, NB_CRYSTAL_SHAPE))
    DO JSH = 1, NB_CRYSTAL_SHAPE
      ZQIAVE_LIMA_SHAPE(:,:,JSH) = TRANSPOSE( RESHAPE( ZWORK2_2D(:,JSH),(/KFLEV,ICLOUD_COL+1/) ) )
    END DO
  END IF ! lkeep_shape 
  !
  ! liquid water mixing ratio profiles
  ...
  ! 
  ! ice water mixing ratio profiles
! marine : question : quoi faire ici ?
  !
...
! + marine
    IF (LKEEP_SHAPE) THEN
      DO JSH = 1, NB_CRYSTAL_SHAPE
        ZWORK1(:) = PACK( TRANSPOSE(ZCIT_LIMA_SHAPE(:,:,JSH)),MASK=GCLOUDT(:,:) )
        ZWORK2_2D(1:ICLOUD,JSH) = ZWORK1(1:ICLOUD) ! fills the ICLOUD_COL cloudy columns
        ZWORK2_2D(ICLOUD+1:,JSH)= 0.0      !   and the single clear_sky one
      END DO
      DEALLOCATE(ZCIT_LIMA_SHAPE)
      ALLOCATE(ZCIT_LIMA_SHAPE(ICLOUD_COL+1,KFLEV, NB_CRYSTAL_SHAPE))
      DO JSH = 1, NB_CRYSTAL_SHAPE
        ZCIT_LIMA_SHAPE (:,:,JSH) = TRANSPOSE(RESHAPE(ZWORK2_2D(:,JSH),(/KFLEV,ICLOUD_COL+1/) ) )
      END DO
    END IF
!- marine
  ENDIF
!++cb++ 22/04/20
  IF ( SIZE(ISHAPE_MAX) > 0)  THEN
    ZWORK1(:) = PACK( TRANSPOSE(ISHAPE_MAX(:,:)),MASK=GCLOUDT(:,:) )
    ZWORK2(1:ICLOUD) = ZWORK1(1:ICLOUD) ! fills the ICLOUD_COL cloudy columns
    ZWORK2(ICLOUD+1:)= 0.0      !   and the single clear_sky one
    DEALLOCATE(ISHAPE_MAX)
    ALLOCATE(ISHAPE_MAX(ICLOUD_COL+1,KFLEV))
    ISHAPE_MAX (:,:) = TRANSPOSE( RESHAPE( ZWORK2(:),(/KFLEV,ICLOUD_COL+1/) ) )
  END IF
!--cb--
  !
  ...
  IF(CAOP=='EXPL')THEN
    ...
    IF (LDUST .OR. CCLOUD=='LIMA') THEN
      ...
      DO WVL_IDX = 1, KSWB_OLD
        ZWORK1AER(:,WVL_IDX) = PACK( TRANSPOSE(ZPIZA_DST(:,:,WVL_IDX)),MASK=GCLOUDT(:,:) )
        ZWORK2AER(1:ICLOUD,WVL_IDX) = ZWORK1AER(:,WVL_IDX)
        ZWORK2AER(ICLOUD+1:,WVL_IDX) = ZPIZA_DST_CLEAR(:,WVL_IDX)
      ENDDO
      DEALLOCATE(ZPIZA_DST)
      ALLOCATE(ZPIZA_DST(ICLOUD_COL+1,KFLEV,KSWB_OLD))
      DO WVL_IDX = 1, KSWB_OLD
       ZPIZA_DST(:,:,WVL_IDX) = TRANSPOSE( RESHAPE( ZWORK2AER(:,WVL_IDX),(/KFLEV,ICLOUD_COL+1/) ) )
      ENDDO
      !Assymetry factor
      DO WVL_IDX = 1, KSWB_OLD
        ZWORK1AER(:,WVL_IDX) = PACK(TRANSPOSE(ZCGA_DST(:,:,WVL_IDX)), MASK=GCLOUDT(:,:))
        ZWORK2AER(1:ICLOUD,WVL_IDX) = ZWORK1AER(:,WVL_IDX)
        ZWORK2AER(ICLOUD+1:,WVL_IDX) = ZCGA_DST_CLEAR(:,WVL_IDX)
      ENDDO
      DEALLOCATE(ZCGA_DST)
      ALLOCATE(ZCGA_DST(ICLOUD_COL+1,KFLEV,KSWB_OLD))
      DO WVL_IDX = 1, KSWB_OLD
        ZCGA_DST(:,:,WVL_IDX) = TRANSPOSE(RESHAPE(ZWORK2AER(:,WVL_IDX),(/KFLEV,ICLOUD_COL+1/)))
      ENDDO
      !Relative wavelength-distributed optical depth
      DO WVL_IDX = 1, KSWB_OLD
        ZWORK1AER(:,WVL_IDX) =  PACK(TRANSPOSE(ZTAUREL_DST(:,:,WVL_IDX)), MASK=GCLOUDT(:,:))
        ZWORK2AER(1:ICLOUD,WVL_IDX) = ZWORK1AER(:,WVL_IDX)
        ZWORK2AER(ICLOUD+1:,WVL_IDX) = ZTAUREL_DST_CLEAR(:,WVL_IDX)
      ENDDO
      DEALLOCATE(ZTAUREL_DST)
      ALLOCATE(ZTAUREL_DST(ICLOUD_COL+1,KFLEV,KSWB_OLD))
      DO WVL_IDX = 1, KSWB_OLD
        ZTAUREL_DST(:,:,WVL_IDX) = TRANSPOSE(RESHAPE(ZWORK2AER(:,WVL_IDX),(/KFLEV,ICLOUD_COL+1/)))
      ENDDO
    ELSE
      DEALLOCATE(ZPIZA_DST)
      ALLOCATE(ZPIZA_DST(ICLOUD_COL+1,KFLEV,KSWB_OLD))
...
!
    IF(LSALT .OR. CCLOUD=='LIMA')THEN
      !Single scattering albedo
      DO WVL_IDX = 1, KSWB_OLD
        ZWORK1AER(:,WVL_IDX) = PACK( TRANSPOSE(ZPIZA_SLT(:,:,WVL_IDX)),MASK=GCLOUDT(:,:) )
        ZWORK2AER(1:ICLOUD,WVL_IDX) = ZWORK1AER(:,WVL_IDX)
        ZWORK2AER(ICLOUD+1:,WVL_IDX) = ZPIZA_SLT_CLEAR(:,WVL_IDX)
      ENDDO
      DEALLOCATE(ZPIZA_SLT)
      ALLOCATE(ZPIZA_SLT(ICLOUD_COL+1,KFLEV,KSWB_OLD))
      DO WVL_IDX = 1, KSWB_OLD
        ZPIZA_SLT(:,:,WVL_IDX) = TRANSPOSE( RESHAPE( ZWORK2AER(:,WVL_IDX),(/KFLEV,ICLOUD_COL+1/) ) )
      ENDDO
      !Assymetry factor
      DO WVL_IDX = 1, KSWB_OLD
        ZWORK1AER(:,WVL_IDX) = PACK(TRANSPOSE(ZCGA_SLT(:,:,WVL_IDX)), MASK=GCLOUDT(:,:))
        ZWORK2AER(1:ICLOUD,WVL_IDX) = ZWORK1AER(:,WVL_IDX)
        ZWORK2AER(ICLOUD+1:,WVL_IDX) = ZCGA_SLT_CLEAR(:,WVL_IDX)
      ENDDO
      DEALLOCATE(ZCGA_SLT)
      ALLOCATE(ZCGA_SLT(ICLOUD_COL+1,KFLEV,KSWB_OLD))
      DO WVL_IDX = 1, KSWB_OLD
        ZCGA_SLT(:,:,WVL_IDX) = TRANSPOSE(RESHAPE(ZWORK2AER(:,WVL_IDX),(/KFLEV,ICLOUD_COL+1/)))
      ENDDO
      !Relative wavelength-distributed optical depth
      DO WVL_IDX = 1, KSWB_OLD
        ZWORK1AER(:,WVL_IDX) =  PACK(TRANSPOSE(ZTAUREL_SLT(:,:,WVL_IDX)), MASK=GCLOUDT(:,:))
        ZWORK2AER(1:ICLOUD,WVL_IDX) = ZWORK1AER(:,WVL_IDX)
        ZWORK2AER(ICLOUD+1:,WVL_IDX) = ZTAUREL_SLT_CLEAR(:,WVL_IDX)
      ENDDO
      DEALLOCATE(ZTAUREL_SLT)
      ALLOCATE(ZTAUREL_SLT(ICLOUD_COL+1,KFLEV,KSWB_OLD))
      DO WVL_IDX=1,KSWB_OLD
        ZTAUREL_SLT(:,:,WVL_IDX) = TRANSPOSE(RESHAPE(ZWORK2AER(:,WVL_IDX),(/KFLEV,ICLOUD_COL+1/)))
      ENDDO
    ELSE
      DEALLOCATE(ZPIZA_SLT)
      ALLOCATE(ZPIZA_SLT(ICLOUD_COL+1,KFLEV,KSWB_OLD))
...
!
    IF(LORILAM .OR. CCLOUD=='LIMA')THEN
      !Single scattering albedo
      DO WVL_IDX = 1, KSWB_OLD
        ZWORK1AER(:,WVL_IDX) = PACK( TRANSPOSE(ZPIZA_AER(:,:,WVL_IDX)),MASK=GCLOUDT(:,:) )
        ZWORK2AER(1:ICLOUD,WVL_IDX) = ZWORK1AER(:,WVL_IDX)
        ZWORK2AER(ICLOUD+1:,WVL_IDX) = ZPIZA_AER_CLEAR(:,WVL_IDX)
      ENDDO
      DEALLOCATE(ZPIZA_AER)
      ALLOCATE(ZPIZA_AER(ICLOUD_COL+1,KFLEV,KSWB_OLD))
      DO WVL_IDX = 1, KSWB_OLD
        ZPIZA_AER(:,:,WVL_IDX) = TRANSPOSE( RESHAPE( ZWORK2AER(:,WVL_IDX),(/KFLEV,ICLOUD_COL+1/) ) )
      ENDDO
      !Assymetry factor
      DO WVL_IDX=1,KSWB_OLD
        ZWORK1AER(:,WVL_IDX) = PACK(TRANSPOSE(ZCGA_AER(:,:,WVL_IDX)), MASK=GCLOUDT(:,:))
        ZWORK2AER(1:ICLOUD,WVL_IDX) = ZWORK1AER(:,WVL_IDX)
        ZWORK2AER(ICLOUD+1:,WVL_IDX) = ZCGA_AER_CLEAR(:,WVL_IDX)
      ENDDO
      DEALLOCATE(ZCGA_AER)
      ALLOCATE(ZCGA_AER(ICLOUD_COL+1,KFLEV,KSWB_OLD))
      DO WVL_IDX = 1, KSWB_OLD
        ZCGA_AER(:,:,WVL_IDX) = TRANSPOSE(RESHAPE(ZWORK2AER(:,WVL_IDX),(/KFLEV,ICLOUD_COL+1/)))
      ENDDO
      !Relative wavelength-distributed optical depth
      DO WVL_IDX = 1, KSWB_OLD
        ZWORK1AER(:,WVL_IDX) =  PACK(TRANSPOSE(ZTAUREL_AER(:,:,WVL_IDX)), MASK=GCLOUDT(:,:))
        ZWORK2AER(1:ICLOUD,WVL_IDX) = ZWORK1AER(:,WVL_IDX)
        ZWORK2AER(ICLOUD+1:,WVL_IDX) = ZTAUREL_AER_CLEAR(:,WVL_IDX)
      ENDDO
      DEALLOCATE(ZTAUREL_AER)
      ALLOCATE(ZTAUREL_AER(ICLOUD_COL+1,KFLEV,KSWB_OLD))
      DO WVL_IDX = 1, KSWB_OLD
        ZTAUREL_AER(:,:,WVL_IDX) = TRANSPOSE(RESHAPE(ZWORK2AER(:,WVL_IDX),(/KFLEV,ICLOUD_COL+1/)))
      ENDDO
    ELSE
      DEALLOCATE(ZPIZA_AER)
      ALLOCATE(ZPIZA_AER(ICLOUD_COL+1,KFLEV,KSWB_OLD))
...
!
  ...
  DEALLOCATE(ZWORK)
!+ marine
  IF (ALLOCATED(ZWORK1_2D))DEALLOCATE(ZWORK1_2D)
  IF (ALLOCATED(ZWORK2_2D)) DEALLOCATE(ZWORK2_2D)
!- marine
  !  
...
!
!ice
!+ marine : question : intervenir ici aussi pr le mr shape
ZWORK_GRID(:,:)=ZQIAVE(:,:)
DO JKRAD=1, KFLEV
  JK1=KFLEV+1-JKRAD
...
IF (LKEEP_SHAPE) THEN
  DO JSH = 1, NB_CRYSTAl_SHAPE
    ZWORK_GRID(:,:)=ZQIAVE_LIMA_SHAPE(:,:,JSH)
    DO JKRAD=1, KFLEV
      JK1=KFLEV+1-JKRAD
      ZQIAVE_LIMA_SHAPE(:,JKRAD,JSH)=ZWORK_GRID(:,JK1)
    END DO
  END DO
END IF
!- marine
...
!+ marine
   IF (LKEEP_SHAPE) THEN
     DO JSH = 1, NB_CRYSTAL_SHAPE
       ZWORK_GRID(:,:)=ZCIT_LIMA_SHAPE(:,:,JSH)
       DO JKRAD=1, KFLEV
         JK1=KFLEV+1-JKRAD
         ZCIT_LIMA_SHAPE(:,JKRAD,JSH)=ZWORK_GRID(:,JK1)
       END DO
     END DO
   END IF
!- marine
...
!
!++cb++
ZWORK_GRID(:,:)=ISHAPE_MAX(:,:)
DO JKRAD=1, KFLEV
   JK1=KFLEV+1-JKRAD
   ISHAPE_MAX(:,JKRAD)=ZWORK_GRID(:,JK1)
END DO
!--cb--
!
!ozone content 
...
!Dust single scattering albedo
  IF (LDUST .OR. CCLOUD=='LIMA') THEN
    !Dust single scattering albedo
    DO JI=1,KSWB_OLD
      ZWORK_GRID(:,:)=ZPIZA_DST(:,:,JI)
      DO JKRAD=1,KFLEV
        JK1=KFLEV+1-JKRAD
        ZPIZA_DST(:,JKRAD,JI)=ZWORK_GRID(:,JK1)
      ENDDO
    ENDDO
    !Dust asymmetry factor
    DO JI=1,KSWB_OLD
      ZWORK_GRID(:,:)=ZCGA_DST(:,:,JI)
      DO JKRAD=1,KFLEV
        JK1=KFLEV+1-JKRAD
        ZCGA_DST(:,JKRAD,JI)=ZWORK_GRID(:,JK1)
      ENDDO
    ENDDO
    DO JI=1,KSWB_OLD
      ZWORK_GRID(:,:)=ZTAUREL_DST(:,:,JI)
      DO JKRAD=1,KFLEV
        JK1=KFLEV+1-JKRAD
        ZTAUREL_DST(:,JKRAD,JI)=ZWORK_GRID(:,JK1)
      ENDDO
    ENDDO 
  END IF
!
  IF (LSALT .OR. CCLOUD=='LIMA') THEN
    !Salt single scattering albedo
    DO JI=1,KSWB_OLD
      ZWORK_GRID(:,:)=ZPIZA_SLT(:,:,JI)
      DO JKRAD=1,KFLEV
        JK1=KFLEV+1-JKRAD
        ZPIZA_SLT(:,JKRAD,JI)=ZWORK_GRID(:,JK1)
      ENDDO
    ENDDO
    !Salt asymmetry factor
    DO JI=1,KSWB_OLD
      ZWORK_GRID(:,:)=ZCGA_SLT(:,:,JI)
      DO JKRAD=1,KFLEV
        JK1=KFLEV+1-JKRAD
        ZCGA_SLT(:,JKRAD,JI)=ZWORK_GRID(:,JK1)
      ENDDO
    ENDDO
    DO JI=1,KSWB_OLD
      ZWORK_GRID(:,:)=ZTAUREL_SLT(:,:,JI)
      DO JKRAD=1,KFLEV
        JK1=KFLEV+1-JKRAD
        ZTAUREL_SLT(:,JKRAD,JI)=ZWORK_GRID(:,JK1)
      ENDDO
    ENDDO 
  END IF
!
  IF (LORILAM .OR. CCLOUD=='LIMA') THEN
    !Orilam single scattering albedo
    DO JI=1,KSWB_OLD
      ZWORK_GRID(:,:)=ZPIZA_AER(:,:,JI)
      DO JKRAD=1,KFLEV
        JK1=KFLEV+1-JKRAD
        ZPIZA_AER(:,JKRAD,JI)=ZWORK_GRID(:,JK1)
      ENDDO
    ENDDO
    !Orilam asymmetry factor
    DO JI=1,KSWB_OLD
      ZWORK_GRID(:,:)=ZCGA_AER(:,:,JI)
      DO JKRAD=1,KFLEV
        JK1=KFLEV+1-JKRAD
        ZCGA_AER(:,JKRAD,JI)=ZWORK_GRID(:,JK1)
      ENDDO
    ENDDO
    DO JI=1,KSWB_OLD
      ZWORK_GRID(:,:)=ZTAUREL_AER(:,:,JI)
      DO JKRAD=1,KFLEV
        JK1=KFLEV+1-JKRAD
        ZTAUREL_AER(:,JKRAD,JI)=ZWORK_GRID(:,JK1)
      ENDDO
    ENDDO 
  END IF
END IF
...
!+ marine
  ...
  IF (LKEEP_SHAPE) ALLOCATE(ZRADIP_SHAPE(IDIM,KFLEV,NB_CRYSTAL_SHAPE))
!- marine
ELSE
...
!+ marine
  ALLOCATE(ZRADIP(0,0))
  IF (LKEEP_SHAPE) ALLOCATE(ZRADIP_SHAPE(0,0,0))
!- marine
END IF
!
  ...
  IF (CCLOUD == 'LIMA') THEN
    IF (CRAD == "ECMW") THEN
      ...
      CALL ECMWF_RADIATION_VERS2  ( IDIM ,KFLEV, KRAD_DIAG, KAER,     &      
        ...
        GDST,ZPIZA_DST,ZCGA_DST,ZTAUREL_DST,                       &
        GSLT,ZPIZA_SLT,ZCGA_SLT,ZTAUREL_SLT,                       &
        GAER,ZPIZA_AER,ZCGA_AER,ZTAUREL_AER                        )
    ELSE IF (CRAD == "ECRA") THEN  
      IF (.NOT. LKEEP_SHAPE) THEN
      ...
      ELSE
        ...
        !  GAER,ZPIZA_AER,ZCGA_AER,ZTAUREL_AER,                       &
        !  ZLAT,ZLON,                                                 &
!+ marine : inserer les arguments optionnels des formes de critaux 
        !  NB_CRYSTAL_SHAPE, ZQIAVE_LIMA_SHAPE,                       &
        !  ZCIT_LIMA_SHAPE, ZRADIP_SHAPE                )
        END IF
!- marine
    ENDIF
...
!+ marine
      IF (LKEEP_SHAPE) &
        ALLOCATE(  ZQIAVE_LIMA_SHAPE_SPLIT(IDIM_EFF,KFLEV,NB_CRYSTAL_SHAPE))
!- marine
      ...
      ALLOCATE(  ZPIZA_DST_SPLIT(IDIM_EFF,KFLEV,KSWB_OLD))
      ALLOCATE(  ZCGA_DST_SPLIT(IDIM_EFF,KFLEV,KSWB_OLD))
      ALLOCATE(  ZTAUREL_DST_SPLIT(IDIM_EFF,KFLEV,KSWB_OLD))
      ALLOCATE(  ZPIZA_SLT_SPLIT(IDIM_EFF,KFLEV,KSWB_OLD))
      ALLOCATE(  ZCGA_SLT_SPLIT(IDIM_EFF,KFLEV,KSWB_OLD))
      ALLOCATE(  ZTAUREL_SLT_SPLIT(IDIM_EFF,KFLEV,KSWB_OLD))
      ALLOCATE(  ZPIZA_AER_SPLIT(IDIM_EFF,KFLEV,KSWB_OLD))
      ALLOCATE(  ZCGA_AER_SPLIT(IDIM_EFF,KFLEV,KSWB_OLD))
      ALLOCATE(  ZTAUREL_AER_SPLIT(IDIM_EFF,KFLEV,KSWB_OLD))
...
!+ marine
        ...
        IF (LKEEP_SHAPE) &
          ALLOCATE(  ZRADIP_SHAPE_SPLIT(IDIM_EFF,KFLEV,NB_CRYSTAL_SHAPE))
!- marine
      ELSE
...
!+ marine
        ...
        IF (LKEEP_SHAPE) & 
          ALLOCATE(  ZRADIP_SHAPE_SPLIT(0,0,0))
!- marine
      END IF
!
! C2R2 coupling
...
!
      ...
      ELSE
        ALLOCATE (ZCCT_LIMA_SPLIT(0,0))
      END IF
!
      IF (SIZE (ZCRT_LIMA) > 0)  THEN
        ALLOCATE (ZCRT_LIMA_SPLIT(IDIM_EFF,KFLEV))
      ELSE
        ALLOCATE (ZCRT_LIMA_SPLIT(0,0))
      END IF
!
      IF (SIZE (ZCIT_LIMA) > 0)  THEN
...
!+ marine
        IF (LKEEP_SHAPE) &
          ALLOCATE (ZCIT_LIMA_SHAPE_SPLIT(IDIM_EFF,KFLEV, NB_CRYSTAL_SHAPE))
!- marine
      ELSE
        ALLOCATE (ZCIT_LIMA_SPLIT(0,0))
!+ marine
        IF (LKEEP_SHAPE) &
          ALLOCATE (ZCIT_LIMA_SHAPE_SPLIT(0,0,0))
!- marine
      END IF
!
!++cb++ 22/04/20
      IF (SIZE (ISHAPE_MAX) > 0)  THEN
        ALLOCATE (ISHAPE_MAX_SPLIT(IDIM_EFF,KFLEV))
      ELSE
        ALLOCATE (ISHAPE_MAX_SPLIT(0,0))
      END IF
!--cb--
!
    END IF
...
!+ marine
    IF (LKEEP_SHAPE) &
      ZQIAVE_LIMA_SHAPE_SPLIT(:,:,:) = ZQIAVE_LIMA_SHAPE( IBEG:IEND ,:,:)
!- marine
    ...
    IF(GDST)THEN
       ZPIZA_DST_SPLIT(:,:,:)=ZPIZA_DST(IBEG:IEND,:,:)
       ZCGA_DST_SPLIT(:,:,:)=ZCGA_DST(IBEG:IEND,:,:)
       ZTAUREL_DST_SPLIT(:,:,:)=ZTAUREL_DST(IBEG:IEND,:,:)
    ENDIF
    IF(GSLT)THEN
      ZPIZA_SLT_SPLIT(:,:,:)=ZPIZA_SLT(IBEG:IEND,:,:)
      ZCGA_SLT_SPLIT(:,:,:)=ZCGA_SLT(IBEG:IEND,:,:)
      ZTAUREL_SLT_SPLIT(:,:,:)=ZTAUREL_SLT(IBEG:IEND,:,:)
    ENDIF
    IF(GAER)THEN
      ZPIZA_AER_SPLIT(:,:,:)=ZPIZA_AER(IBEG:IEND,:,:)
      ZCGA_AER_SPLIT(:,:,:)=ZCGA_AER(IBEG:IEND,:,:)
      ZTAUREL_AER_SPLIT(:,:,:)=ZTAUREL_AER(IBEG:IEND,:,:)
    ENDIF
...
!
    IF (CCLOUD == 'LIMA') THEN
! LIMA concentrations
      IF (SIZE (ZCCT_LIMA) > 0)  ZCCT_LIMA_SPLIT(:,:) = ZCCT_LIMA (IBEG:IEND ,:)
      IF (SIZE (ZCRT_LIMA) > 0)  ZCRT_LIMA_SPLIT(:,:) = ZCRT_LIMA (IBEG:IEND ,:)
!+ marine
      IF (SIZE (ZCIT_LIMA) > 0) THEN
        ZCIT_LIMA_SPLIT(:,:) = ZCIT_LIMA (IBEG:IEND ,:)
        IF (LKEEP_SHAPE) &
          ZCIT_LIMA_SHAPE_SPLIT(:,:,:) = ZCIT_LIMA_SHAPE (IBEG:IEND ,:,:)
      END IF
!- marine
!++cb++ 22/04/20
      IF (SIZE (ISHAPE_MAX) > 0)  ISHAPE_MAX_SPLIT(:,:) = ISHAPE_MAX (IBEG:IEND ,:)
!--cb--
...
!
        CALL ECMWF_RADIATION_VERS2  ( IDIM_EFF , KFLEV, KRAD_DIAG, KAER,                 &
                ...
                GDST,ZPIZA_DST_SPLIT,ZCGA_DST_SPLIT,ZTAUREL_DST_SPLIT,                   &
                GSLT,ZPIZA_SLT_SPLIT,ZCGA_SLT_SPLIT,ZTAUREL_SLT_SPLIT,                   &
                GAER,ZPIZA_AER_SPLIT,ZCGA_AER_SPLIT,ZTAUREL_AER_SPLIT                    )
...
!
        ...
        ELSE
          ...
          !    ZLAT_SPLIT,ZLON_SPLIT,                                                       &
!+ marine
           !   NB_CRYSTAL_SHAPE, ZQIAVE_LIMA_SHAPE_SPLIT, ZCIT_LIMA_SHAPE_SPLIT,            &
           !   ZRADIP_SHAPE_SPLIT )
        END IF
      END IF              
    ELSE
...
!+ marine
              ...
              IF (LKEEP_SHAPE) ZRADIP_SHAPE( IBEG:IEND ,:,:) = ZRADIP_SHAPE_SPLIT(:,:,:)
!- marine
...
!+ marine
      IF ( ALLOCATED( ZQIAVE_LIMA_SHAPE_SPLIT ) ) DEALLOCATE(  ZQIAVE_LIMA_SHAPE_SPLIT  )
!- marine
...
!+ marine
      IF ( ALLOCATED( ZCIT_LIMA_SHAPE_SPLIT ) ) DEALLOCATE( ZCIT_LIMA_SHAPE_SPLIT )
!- marine
!++cb++ 22/04/20
      IF ( ALLOCATED( ISHAPE_MAX_SPLIT ) ) DEALLOCATE( ISHAPE_MAX_SPLIT )
!--cb--
...
!+ marine
      ...
      IF ( ALLOCATED ( ZRADIP_SHAPE_SPLIT ) ) DEALLOCATE( ZRADIP_SHAPE_SPLIT )
!- marine
      ...
      DEALLOCATE(ZPIZA_DST_SPLIT)
      DEALLOCATE(ZCGA_DST_SPLIT)
      DEALLOCATE(ZTAUREL_DST_SPLIT)
      DEALLOCATE(ZPIZA_SLT_SPLIT)
      DEALLOCATE(ZCGA_SLT_SPLIT)
      DEALLOCATE(ZTAUREL_SLT_SPLIT)
      DEALLOCATE(ZPIZA_AER_SPLIT)
      DEALLOCATE(ZCGA_AER_SPLIT)
      DEALLOCATE(ZTAUREL_AER_SPLIT)
    END IF
  END DO
END IF
...
DEALLOCATE(ZQIAVE)
!+ marine
IF ( ALLOCATED( ZQIAVE_LIMA_SHAPE ) ) DEALLOCATE( ZQIAVE_LIMA_SHAPE )
!- marine
DEALLOCATE(ZCFAVE)
DEALLOCATE(ZPRES_HL)
DEALLOCATE(ZT_HL)
...
DEALLOCATE(ZCIT_LIMA)
!+ marine
IF ( ALLOCATED( ZCIT_LIMA_SHAPE ) ) DEALLOCATE( ZCIT_LIMA_SHAPE )
!- marine
DEALLOCATE(ISHAPE_MAX) !++cb--
!
DEALLOCATE(ZTS)
DEALLOCATE(ZALBP)
DEALLOCATE(ZALBD)
      ...
      DO JSH = 1, NB_CRYSTAL_SHAPE
        DO JK=IKB,IKE
          ...
          DO JJ=IJB,IJE
            DO JI=IIB,IIE
              IIJ = 1 + (JI-IIB) + (IIE-IIB+1)*(JJ-IJB)
              ZSTORE_3D(JI,JJ,JK) = ZRADIP_SHAPE(IIJ,JKRAD,JSH)
            END DO
          END DO
        END DO
        IF (JSH == 1) THEN
          TZFIELD%CMNHNAME   = 'EFRADI_1'
          TZFIELD%CLONGNAME  = 'EFRADI_SHAPE_1'
        ELSE IF (JSH == 2) THEN
          TZFIELD%CMNHNAME   = 'EFRADI_2'
          TZFIELD%CLONGNAME  = 'EFRADI_SHAPE_2'
        ELSE IF (JSH == 3) THEN
          TZFIELD%CMNHNAME   = 'EFRADI_3'
          TZFIELD%CLONGNAME  = 'EFRADI_SHAPE_3'
        ELSE IF (JSH == 4) THEN
          TZFIELD%CMNHNAME   = 'EFRADI_4'
          TZFIELD%CLONGNAME  = 'EFRADI_SHAPE_4'
        END IF
        TZFIELD%CSTDNAME   = ''
        TZFIELD%CUNITS     = ''
        TZFIELD%CDIR       = 'XY'
...
!+ marine
DEALLOCATE(ZRADIP)
IF (ALLOCATED(ZRADIP_SHAPE)) DEALLOCATE(ZRADIP_SHAPE)
!- marine
