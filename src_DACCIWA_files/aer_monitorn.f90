...
!!    C. Barthe et al. (LACy) 03/2019  - ORILAM-LIMA coupling
!!    EXTERNAL
!!    --------
!
USE MODD_LUNIT_n
...
!
!                           
!     3.6 Return to lognormal distribution (compute M0 and M6 using RG, SIG and
!     new mass from M3)
...
!
  ...
  ENDDO
  DO JSV=1,NSV_DSTDEP
    XRSVS(IIB:IIE,IJB:IJE,IKB:IKE,NSV_DSTDEPBEG-1+JSV) =          &
                  ZSVDST(IIB:IIE,IJB:IJE,IKB:IKE,NMODE_DST+JSV) * &
                  XRHODJ(IIB:IIE,IJB:IJE,IKB:IKE) / PTSTEP
  ENDDO
...
!
!                          
!     4.6 Return to lognormal distribution (compute M0 and M6 using RG, SIG and
!     new mass from M3)
!
...
!
  ...
  ENDDO
  DO JSV=1,NSV_SLTDEP
    XRSVS(IIB:IIE,IJB:IJE,IKB:IKE,NSV_SLTDEPBEG-1+JSV) =  &
          ZSVSLT(IIB:IIE,IJB:IJE,IKB:IKE,NMODE_SLT+JSV) * &
          XRHODJ(IIB:IIE,IJB:IJE,IKB:IKE) / PTSTEP
  ENDDO
