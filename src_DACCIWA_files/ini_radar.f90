...
!!      M. Claeys   06/2019  Add different shapes for ice crystals
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
USE MODD_PARAM_LIMA, ONLY : LCRYSTAL_SHAPE, NB_CRYSTAL_SHAPE
USE MODD_PARAM_LIMA_COLD, ONLY : XAI_SHAPE,  XBI_SHAPE, &
                                 XC_I_SHAPE, XDI_SHAPE, &
                                 XLBEXI_SHAPE, XLBI_SHAPE
!
IMPLICIT NONE
!
...
INTEGER :: JSH ! loop for ice crystal shape
!
!-------------------------------------------------------------------------------
!
...
! capacitance : choisir un rapport d'aspect pour plates et columns from Westbrook et al
!
  XAI_SHAPE(1)  = 0.82   ! relation masse maxdim
  XAI_SHAPE(2)  = 110.798   ! Mitchell 1(d < 100 um) 1996
  XAI_SHAPE(3)  = (XPI/6.0) * ZRHOIW
  XAI_SHAPE(4)  = 0.0141
!
