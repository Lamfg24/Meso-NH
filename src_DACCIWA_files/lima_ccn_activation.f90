...
!
! Work arrays
REAL, DIMENSION(:), ALLOCATABLE    :: ZZW1, ZZW2, ZZW3, ZZW4, ZZW5, ZZW6,ZMCN, ZNATOLD, &
                                      ZZTDT,          & ! dT/dt
...
!
     ...
     ZNATOLD(:) = ZNAT(:,1)  ! save activated number at t-dt
!     ZNFTOLD(:) = ZNFT(:,1)  ! save activated mass at t-dt
   !
     ...
     ZZW1(:) = MAX(ZNAT(:,1)- ZNATOLD(:) , 0.0 ) 
!     ZZW4(:) = MAX(ZNFT(:,1)- ZNFTOLD(:) , 0.0 ) 
!
     ! New particles number activated
     ZW(:,:,:) = UNPACK( ZZW1(:),MASK=GNUCT(:,:,:),FIELD=0.0 )
     ! New particles mass activated
!     ZW4(:,:,:) = UNPACK( ZZW4(:),MASK=GNUCT(:,:,:),FIELD=0.0 )
   ...
   !
     ZZW6(:) = ZZW1(:) ! j'ai l'impression que c'est la somme sur ses modes de ccn actives
     DEALLOCATE(ZMCN) 
     DEALLOCATE(ZNATOLD)
!
ELSE
...
!
   ZZW1(:)=0.
   WHERE (ZZT(:)>0.0 .AND. ZSMAX(:)>0.0) ! ZZW1 is computed with ZSMAX [NO UNIT]
