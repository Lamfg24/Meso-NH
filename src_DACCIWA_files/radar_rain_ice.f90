...
USE MODD_PARAM_LIMA_COLD, ONLY: XDI_L=>XDI,XLBEXI_L=>XLBEXI,XLBI_L=>XLBI,XAI_L=>XAI,XBI_L=>XBI,XC_I_L=>XC_I,&
                                ...
                                XDI_S=>XDI_SHAPE, XLBEXI_S=>XLBEXI_SHAPE, &
                                XLBI_S=>XLBI_SHAPE, XAI_S=>XAI_SHAPE,&
                                XBI_S=>XBI_SHAPE, XC_I_S=>XC_I_SHAPE
...
!
INTEGER :: JSH   ! loop for ice crystal shapes
...
IF (SIZE(PRT,4) >= 4) THEN
  ...
  IF (CCLOUD == 'LIMA') THEN
    IF (.NOT. LCRYSTAL_SHAPE) THEN
      ...
      ZEXP = 2.0*XBI_L
      WHERE( PRT(:,:,:,4).GT.XRTMIN_L(4) .AND. PCIT(:,:,:,1).GT.0.0 )
        ZLBDA(:,:,:) = XLBI_L**(XLBEXI_L)* (PRT(:,:,:,4)/PCIT(:,:,:,1))**(-XLBEXI_L)
        ZW(:,:,:) = ZEQICE*ZDMELT_FACT *1.E18*PRHODREF(:,:,:)*PCIT(:,:,:,1)*(ZLBDA(:,:,:)**(-ZEXP))*MOMG(XALPHAI_L,XNUI_L,ZEXP)
        PVDOP(:,:,:) = PVDOP(:,:,:)+ZEQICE*ZDMELT_FACT*MOMG(XALPHAI_L,XNUI_L,ZEXP+XDI_L) &
                       *1.E18*PRHODREF(:,:,:)*PCIT(:,:,:,1)*XC_I_L*(ZLBDA(:,:,:)**(-ZEXP-XDI_L))
        PRARE(:,:,:) = PRARE(:,:,:) + ZW(:,:,:)
      END WHERE
    ELSE
      DO JSH = 1, NB_CRYSTAL_SHAPE
        ZDMELT_FACT = ( (6.0*XAI_S(JSH))/(XPI*XRHOLW) )**(2.0)    
        ZEXP = 2.0 * XBI_S(JSH)
        WHERE( PRT(:,:,:,4).GT.XRTMIN_L(4) .AND. PCIT(:,:,:,JSH).GT.0.0 )
          ZLBDA(:,:,:) = XLBI_S(JSH)**(XLBEXI_S(JSH)) * (PRT(:,:,:,4) / &
                         PCIT(:,:,:,JSH))**(-XLBEXI_S(JSH))
          ZW(:,:,:)    = ZEQICE * ZDMELT_FACT * 1.E18 * PRHODREF(:,:,:) * &
                         PCIT(:,:,:,JSH) *                                &
                         (ZLBDA(:,:,:)**(-ZEXP)) * MOMG(XALPHAI_L,XNUI_L,ZEXP)
          PVDOP(:,:,:) = PVDOP(:,:,:) +                                                  &
                         ZEQICE * ZDMELT_FACT * MOMG(XALPHAI_L,XNUI_L,ZEXP+XDI_S(JSH)) * &
                         1.E18 * PRHODREF(:,:,:) * PCIT(:,:,:,JSH) * XC_I_S(JSH) *       &
                         (ZLBDA(:,:,:)**(-ZEXP-XDI_S(JSH)))
          PRARE(:,:,:) = PRARE(:,:,:) + ZW(:,:,:)
        END WHERE
      END DO
    END IF
  ELSE
    ...
    ZEXP = 2.0*XBI_I
    WHERE( PRT(:,:,:,4).GT.XRTMIN_I(4) .AND. PCIT(:,:,:,1).GT.0.0 )
      ZLBDA(:,:,:) = XLBI_I*( PRHODREF(:,:,:)*PRT(:,:,:,4)/PCIT(:,:,:,1) )**XLBEXI_I
      ZW(:,:,:) = ZEQICE*ZDMELT_FACT*1.E18*PCIT(:,:,:,1)*(ZLBDA(:,:,:)**(-ZEXP))*MOMG(XALPHAI_I,XNUI_I,ZEXP)
      PVDOP(:,:,:) = PVDOP(:,:,:)+ZEQICE*ZDMELT_FACT*MOMG(XALPHAI_I,XNUI_I,ZEXP+XDI_I) &
                     *1.E18*PCIT(:,:,:,1)*XC_I_I*(ZLBDA(:,:,:)**(-ZEXP-XDI_I))
      PRARE(:,:,:) = PRARE(:,:,:) + ZW(:,:,:)
    END WHERE
  END IF
END IF
