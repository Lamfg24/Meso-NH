...
!!      C. Barthe   03/2019    make this source coherent with ini_rain_ice
!!                             + LIMA-CELLS coupling
!!
!-------------------------------------------------------------------------------
!
...
USE MODD_PARAM_LIMA, ONLY : LHAIL
!
USE MODI_MOMG
USE MODI_GAMMA
USE MODI_GAMMA_INC
...
IF (CSEDIM == 'SPLI' .AND. .NOT. LRED) THEN
  IF (HCLOUD == 'ICE4' .OR. (HCLOUD == 'LIMA' .AND. LHAIL)) THEN
    ZVTRMAX = 40.
  ELSE IF (HCLOUD == 'ICE3' .OR. (HCLOUD == 'LIMA' .AND. (.NOT. LHAIL))) THEN
    ZVTRMAX = 10.
  END IF 
END IF
!
...
IF (HCLOUD == 'ICE4' .OR. (HCLOUD == 'LIMA' .AND. LHAIL)) THEN
  ALLOCATE( XRTMIN(7) )
ELSE IF (HCLOUD == 'ICE3' .OR. HCLOUD == 'LIMA') THEN
  ALLOCATE( XRTMIN(6) )
END IF
!
IF (HCLOUD(1:3) == 'ICE' .OR. HCLOUD == 'LIMA') THEN
  ...
  XRTMIN(6) = 1.0E-15
END IF
IF (HCLOUD == 'ICE4' .OR. (HCLOUD == 'LIMA' .AND. LHAIL)) XRTMIN(7) = 1.0E-15
!
