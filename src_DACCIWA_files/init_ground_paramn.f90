...
!!  02/2020     (P. Tulet)   direct coupling between LIMA and salt/dust emission
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
USE MODD_NSV
USE MODD_DUST,       ONLY : CDUSTNAMES, LDUST
...
IF (CCLOUD == 'LIMA' .AND. LSALT2LIMA .AND. .NOT.(LSALT)) THEN
  HSV(NSV_LIMA_CCN_FREE) = 'SLTLIM'
END IF
IF (CCLOUD == 'LIMA' .AND. LDUST2LIMA .AND. .NOT.(LDUST)) THEN
  HSV(NSV_LIMA_IFN_FREE) = 'DSTLIM'
END IF
