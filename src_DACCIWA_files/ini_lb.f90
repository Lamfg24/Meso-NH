...
!!      C. Barthe       03/2019     Add sea salt deposition
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!
        ...
        IF (KSIZELBYSV_ll  /= 0 ) THEN
          ...
          IF ( SIZE(PLBYSVM,1) /= 0 ) THEN
            IF (IRESP/=0) THEN
              IF (PRESENT(PLBYSVMM)) THEN
                PLBYSVM(:,:,:,JSV)=PLBYSVMM(:,:,:,JSV)
                WRITE(ILUOUT,*) 'Dust Deposition  PLBYSVM   will be initialized to 0'
              ELSE
...
IF (NSV_SLTDEPEND>=NSV_SLTDEPBEG) THEN
  ...
  DO JSV = NSV_SLTDEPBEG, NSV_SLTDEPEND
    SELECT CASE(HGETSVM(JSV))
      CASE ('READ')
        IF ( KSIZELBXSV_ll /= 0 ) THEN
          TZFIELD%CMNHNAME   = 'LBX_'//TRIM(UPCASE(CDESLTNAMES(JSV-NSV_SLTDEPBEG+1)))
          TZFIELD%CLONGNAME  = TRIM(TZFIELD%CMNHNAME)
          TZFIELD%CLBTYPE    = 'LBX'
          WRITE(TZFIELD%CCOMMENT,'(A6,A6,I3.3)')'2_Y_Z_','LBXSVM',JSV
          CALL IO_READ_FIELD_LB(TPINIFILE,TZFIELD,IL3DX,IRIMX,PLBXSVM(:,:,:,JSV),IRESP)
          IF ( SIZE(PLBXSVM,1) /= 0 ) THEN
            IF (IRESP/=0) THEN
              IF (PRESENT(PLBXSVMM)) THEN
                PLBXSVM(:,:,:,JSV)=PLBXSVMM(:,:,:,JSV)
                WRITE(ILUOUT,*) 'Sea Salt Desposition PLBXSVM   will be initialized to 0'
              ELSE
!callabortstop
                CALL PRINT_MSG(NVERB_FATAL,'GEN','INI_LB','problem to initialize sea salt PLBXSVM')
              ENDIF
        ...
        !
        IF (KSIZELBYSV_ll  /= 0 ) THEN
          TZFIELD%CMNHNAME   = 'LBY_'//TRIM(UPCASE(CDESLTNAMES(JSV-NSV_SLTDEPBEG+1)))
          TZFIELD%CLONGNAME  = TRIM(TZFIELD%CMNHNAME)
          TZFIELD%CLBTYPE    = 'LBY'
          WRITE(TZFIELD%CCOMMENT,'(A6,A6,I3.3)')'X_2_Z_','LBYSVM',JSV
          CALL IO_READ_FIELD_LB(TPINIFILE,TZFIELD,IL3DY,IRIMY,PLBYSVM(:,:,:,JSV),IRESP)
          IF ( SIZE(PLBYSVM,1) /= 0 ) THEN
            IF (IRESP/=0) THEN
              IF (PRESENT(PLBYSVMM)) THEN
                PLBYSVM(:,:,:,JSV)=PLBYSVMM(:,:,:,JSV)
                WRITE(ILUOUT,*) 'Sea Salt Deposition  PLBYSVM   will be initialized to 0'
              ELSE
