...
!!     C. Barthe 07/2019  change the size of PCIT in RADAR_RAIN_ICE / ice crystal shapes
!!
!! --------------------------------------------------------------------------
!       
!*      0. DECLARATIONS
...
REAL  :: ZRDSRV                 ! XRD/XRV
REAL, DIMENSION(:,:,:,:), ALLOCATABLE :: ZAUX ! used to transform 3D array into 4D array (pcit)
!----------------------------------------------------------------------------
!
!*      2.   PRELIMINARIES
!            -------------
...
ALLOCATE(ZAUX(SIZE(PCIT,1),SIZE(PCIT,2),SIZE(PCIT,3),1))
ZAUX(:,:,:,1) = PCIT(:,:,:)
IF (CCLOUD(1:3)=="ICE") CALL RADAR_RAIN_ICE (PR, ZAUX, PRHODREF, ZTEMP, ZRARE, ZWORK32, &
                                                         ZWORK33, ZWORK34 )
DEALLOCATE(ZAUX)
! Theta_v
