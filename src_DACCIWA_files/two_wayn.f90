...
END DO
END IF
! Volc scalar variables
IF (NSV_PYR_A(IMI) > 0) THEN
DO JVAR=1,NSV_PYR_A(KMI)
  ZTSVM(:,:,:,JVAR-1+NSV_PYRBEG_A(KMI)) = 0.
  DO JX=1,IDXRATIO
    DO JY=1,IDYRATIO
      II1 = IIB+JX-1
      II2 = IIE+JX-IDXRATIO
      IJ1 = IJB+JY-1
      IJ2 = IJE+JY-IDYRATIO
      ZTSVM(IIBC:IIEC,IJBC:IJEC,:,JVAR-1+NSV_PYRBEG_A(KMI)) = &
           &ZTSVM(IIBC:IIEC,IJBC:IJEC,:,JVAR-1+NSV_PYRBEG_A(KMI))+&
           &XRHODJ(II1:II2:IDXRATIO,IJ1:IJ2:IDYRATIO,:)*&
           &XSVT(II1:II2:IDXRATIO,IJ1:IJ2:IDYRATIO,:,JVAR-1+NSV_PYRBEG_A(IMI))
    END DO
  END DO
END DO
END IF
! Lava scalar variables
IF (NSV_LV_A(IMI) > 0) THEN
DO JVAR=1,NSV_LV_A(KMI)
  ZTSVM(:,:,:,JVAR-1+NSV_LVBEG_A(KMI)) = 0.
  DO JX=1,IDXRATIO
    DO JY=1,IDYRATIO
      II1 = IIB+JX-1
      II2 = IIE+JX-IDXRATIO
      IJ1 = IJB+JY-1
      IJ2 = IJE+JY-IDYRATIO
      ZTSVM(IIBC:IIEC,IJBC:IJEC,:,JVAR-1+NSV_LVBEG_A(KMI)) = &
           &ZTSVM(IIBC:IIEC,IJBC:IJEC,:,JVAR-1+NSV_LVBEG_A(KMI))+&
           &XRHODJ(II1:II2:IDXRATIO,IJ1:IJ2:IDYRATIO,:)*&
           &XSVT(II1:II2:IDXRATIO,IJ1:IJ2:IDYRATIO,:,JVAR-1+NSV_LVBEG_A(IMI))
    END DO
  END DO
END DO
END IF
...
ENDDO
! Volc variables
DO JVAR=NSV_PYRBEG_A(KMI),NSV_PYREND_A(KMI)
  PRSVS(IXOR:IXEND,IYOR:IYEND,:,JVAR) = PRSVS(IXOR:IXEND,IYOR:IYEND,:,JVAR) &
     -  ZK2W * PRHODJ(IXOR:IXEND,IYOR:IYEND,:) * (PSVM(IXOR:IXEND,IYOR:IYEND,:,JVAR) &
                 -ZSVM(IXOR:IXEND,IYOR:IYEND,:,JVAR)/ZRHODJ(IXOR:IXEND,IYOR:IYEND,:) )
ENDDO
! Lava variables
DO JVAR=NSV_LVBEG_A(KMI),NSV_LVEND_A(KMI)
  PRSVS(IXOR:IXEND,IYOR:IYEND,:,JVAR) = PRSVS(IXOR:IXEND,IYOR:IYEND,:,JVAR) &
     -  ZK2W * PRHODJ(IXOR:IXEND,IYOR:IYEND,:) * (PSVM(IXOR:IXEND,IYOR:IYEND,:,JVAR) &
                 -ZSVM(IXOR:IXEND,IYOR:IYEND,:,JVAR)/ZRHODJ(IXOR:IXEND,IYOR:IYEND,:) )
ENDDO
