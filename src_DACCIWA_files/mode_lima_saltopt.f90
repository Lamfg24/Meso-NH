...
!  
! *********************************************************************
  SUBROUTINE LIMA_SALTOPT_GET(PSVT,PZZ,PRHODREF,                      &
...
! Arguments
!
REAL,    DIMENSION(:,:,:,:), INTENT(IN)    :: PSVT        ! [moments/molec_{air}] transported moments 
                                                          ! aerosol
REAL,    DIMENSION(:,:,:),   INTENT(IN)    :: PZZ         ! [m] height of layers
REAL,    DIMENSION(:,:,:),   INTENT(IN)    :: PRHODREF    ! [kg/m3] density of air
REAL,    DIMENSION(:,:,:,:), INTENT(INOUT) :: PPIZA_WVL   ! [-] single scattering albedo aerosol layer 
                                                          ! for all SW wavelengths
REAL,    DIMENSION(:,:,:,:), INTENT(INOUT) :: PCGA_WVL    ! [-] assymetry factor faerosol layer for all 
                                                          ! SW wavelengths
REAL,    DIMENSION(:,:,:,:), INTENT(INOUT) :: PTAUREL_WVL ! [-] opt.depth/opt.depth(550) faerosol layer 
                                                          ! for all SW wvl 
...
!     If CCN type "C", -> we should not get in here
!     IF CCN from MACC analyses, mode1=sea-salt, mode2=Su, mode3=BC/OC
!
!     Eventually, 1 mode is considered for MACC CCN, NMOD_CCN modes are for CCN type "M"
!
IF ( CCCN_MODES=='MACC' ) THEN
  NMODE_AER=1
ELSE
  NMODE_AER=NMOD_CCN
END IF
...
ZSVT(:,:,:,:) = 0.
DO JMDE=1, NMODE_AER
  ZSVT(:,:,:,JMDE) = PSVT(:,:,:,NSV_LIMA_CCN_FREE+JMDE-1)
END DO
!
...
!
DO JMDE = 1, NMODE_AER
  ZMASS(:,:,:,JMDE) = 3.1415/6. * XRHO_CCN(JMDE) * ZSVT(:,:,:,JMDE) &
...
DEALLOCATE(ZSVT)
!
  END SUBROUTINE LIMA_SALTOPT_GET
!
!----------------------------------------------------------------------
!
! **********************************************************************************
