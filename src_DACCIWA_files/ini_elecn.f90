...
!!      C. Barthe    03/2019   LIMA-CELLS coupling and update (/ini_micron.f90)
!!
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
...
USE MODD_PRECIP_n, ONLY : XINPRR, XACPRR, XINPRS, XACPRS, XINPRG, XACPRG, &
                          ...
                          XINDEP,XACDEP
USE MODD_REF
USE MODD_REF_n, ONLY : XRHODJ, XTHVREF, XRHODREF
USE MODD_TIME
USE MODD_CLOUDPAR_n, ONLY : NSPLITR, NSPLITG
USE MODD_FIELD_n, ONLY : XRT, XSVT, XTHT, XPABST, XTHM, XRCM
USE MODD_PARAM_LIMA, ONLY : LSCAV, MSEDC=>LSEDC, MACTIT=>LACTIT, LHAIL
USE MODD_LIMA_PRECIP_SCAVENGING_n
USE MODD_NSV, ONLY : NSV, NSV_LIMA_BEG, NSV_LIMA_END
!
...
USE MODI_INI_LIMA
USE MODI_INI_LIMA_WARM
USE MODI_INI_LIMA_COLD_MIXED
...
!              -----------------------------
!
IF (HCLOUD(1:3) == 'ICE' .OR. HCLOUD == 'LIMA') THEN
...
END IF
!
IF ((HCLOUD(1:3) == 'ICE' .AND. LSEDIC) .OR. (HCLOUD == 'LIMA' .AND. MSEDC)) THEN
...
!               --------------------------------
!
IF (HCLOUD == 'LIMA' .AND.  MACTIT) THEN
...
!
!
!*       2t.   ALLOCATE  Module MODD_PRECIP_SCAVENGING_n
!              -----------------------------------------
!
IF ( (HCLOUD=='LIMA') .AND. LSCAV ) THEN
...
DEALLOCATE(ZDZ)
!
IF (HELEC(1:3) == 'ELE' .AND. HCLOUD == 'LIMA') THEN
  CALL INIT_AEROSOL_CONCENTRATION(XRHODREF,                              &
                                  XSVT(:,:,:,NSV_LIMA_BEG:NSV_LIMA_END), &
                                  PZZ(:,:,:))
  CALL INI_LIMA(PTSTEP,ZDZMIN,NSPLITR, NSPLITG)   ! 1/2 spectral warm cloud
END IF
!
!
IF (HELEC(1:3) == 'ELE'.AND. (HCLOUD(1:3) == 'ICE' .OR. HCLOUD == 'LIMA')) THEN
!
!
