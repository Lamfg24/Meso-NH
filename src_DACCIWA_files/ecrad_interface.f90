...
SUBROUTINE ECRAD_INTERFACE ( KLON,KLEV,KRAD_DIAG, KAER, &
     ...
     OSALT,PPIZA_SLT,PCGA_SLT,PTAUREL_SLT,                    &
     OORILAM,PPIZA_AER,PCGA_AER,PTAUREL_AER,                  &
     PLAT,PLON,                                               &
!+ marine
     KNB_CRYSTAL_SHAPE, PQIWC_LIMA_SHAPE, PCIT_LIMA_SHAPE,    &
     PRADIP_SHAPE )
!- marine)
!##############################################################
!  
...
!       M. Claeys  07/2018 : introduction of ice crystal shapes from LIMA
!-----------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
REAL(KIND=JPRB), DIMENSION(:,:,:),INTENT(IN)    :: PPIZA_SLT   !Single scattering albedo of salt (wvl dependent)
REAL(KIND=JPRB), DIMENSION(:,:,:),INTENT(IN)    :: PCGA_SLT    !Assymetry factor for salt (wvl dependent)
REAL(KIND=JPRB), DIMENSION(:,:,:),INTENT(IN)    :: PTAUREL_SLT !Optical depth of salt relative to the one at 550nm
REAL(KIND=JPRB), DIMENSION(:,:,:),INTENT(IN)    :: PPIZA_AER   !Single scattering albedo of orilam (wvl dependent)
REAL(KIND=JPRB), DIMENSION(:,:,:),INTENT(IN)    :: PCGA_AER    !Assymetry factor for orilam (wvl dependent)
REAL(KIND=JPRB), DIMENSION(:,:,:),INTENT(IN)    :: PTAUREL_AER !Optical depth of orilam relative to the one at 550nm
LOGICAL, INTENT (IN)                 :: ODUST  ! flag for dust
LOGICAL, INTENT (IN)                 :: OSALT  ! flag for salt
LOGICAL, INTENT (IN)                 :: OORILAM  ! flag for orilam
!
...
!
!+ marine
! optional arguments when several ice crystal shapes (CCLOUD == LIMA .AND. LCRYSTAL_SHAPE .AND. NICEOPT == 7)
INTEGER,                            INTENT (IN),  OPTIONAL :: KNB_CRYSTAL_SHAPE
REAL,            DIMENSION (:,:,:), INTENT (OUT), OPTIONAL :: PRADIP_SHAPE  ! Cloud ice effective radius
REAL(KIND=JPRB), DIMENSION (:,:,:), INTENT (IN),  OPTIONAL :: PQIWC_LIMA_SHAPE ! mean layer ice specicifc water content (kg/kg)
REAL(KIND=JPRB), DIMENSION (:,:,:), INTENT (IN),  OPTIONAL :: PCIT_LIMA_SHAPE ! ice crystal concentration
!- marine
!  
!
...
!+ marine
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZCIT_LIMA_SHAPE_1
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZCIT_LIMA_SHAPE_2
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZCIT_LIMA_SHAPE_3
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZCIT_LIMA_SHAPE_4
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZQIWC_LIMA_SHAPE_1
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZQIWC_LIMA_SHAPE_2
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZQIWC_LIMA_SHAPE_3
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZQIWC_LIMA_SHAPE_4
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZRE_ICE_UM_SHAPE_1
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZRE_ICE_UM_SHAPE_2
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZRE_ICE_UM_SHAPE_3
REAL(KIND=JPRB),DIMENSION(KLON,KLEV) :: ZRE_ICE_UM_SHAPE_4
!- marine
...
!++cb++ passage du monde MNH au monde ECRAD : de NB_CRYSTAL_SHAPE a 4
  ZQIWC_LIMA_SHAPE_1(:,:) = PQIWC_LIMA_SHAPE(:,:,1)
  ZCIT_LIMA_SHAPE_1(:,:)  = PCIT_LIMA_SHAPE(:,:,1)
  ZQIWC_LIMA_SHAPE_2(:,:) = PQIWC_LIMA_SHAPE(:,:,2)
  ZCIT_LIMA_SHAPE_2(:,:)  = PCIT_LIMA_SHAPE(:,:,2)
  IF (KNB_CRYSTAL_SHAPE .GE. 3) THEN
    ZQIWC_LIMA_SHAPE_3(:,:) = PQIWC_LIMA_SHAPE(:,:,3)
    ZCIT_LIMA_SHAPE_3(:,:)  = PCIT_LIMA_SHAPE(:,:,3)
  END IF
  IF (KNB_CRYSTAL_SHAPE .EQ. 4) THEN
    ZQIWC_LIMA_SHAPE_4(:,:) = PQIWC_LIMA_SHAPE(:,:,4)
    ZCIT_LIMA_SHAPE_4(:,:)  = PCIT_LIMA_SHAPE(:,:,4)
  END IF
!--cb--
...
!+ marine
  !        KNB_CRYSTAL_SHAPE, &
  !        ZQIWC_LIMA_SHAPE_1, ZQIWC_LIMA_SHAPE_2, ZQIWC_LIMA_SHAPE_3, ZQIWC_LIMA_SHAPE_4, &
  !        ZCIT_LIMA_SHAPE_1, ZCIT_LIMA_SHAPE_2, ZCIT_LIMA_SHAPE_3, ZCIT_LIMA_SHAPE_4, &
  !        ZRE_ICE_UM_SHAPE_1, ZRE_ICE_UM_SHAPE_2, ZRE_ICE_UM_SHAPE_3, ZRE_ICE_UM_SHAPE_4 )
!- marine
ELSE
  CALL RADIATION_SCHEME  (IKIDIA, IKFDIA,KLON, KLEV, IAER, &
          PRII0,PMU0, PTS, PALBD, PALBP, &
...
!+ marine
    PRADIP(:,:) = ZRE_ICE_UM(:,:)
    IF (PRESENT(KNB_CRYSTAL_SHAPE)) THEN
        PRADIP_SHAPE(:,:,1) = ZRE_ICE_UM_SHAPE_1(:,:)
        PRADIP_SHAPE(:,:,2) = ZRE_ICE_UM_SHAPE_2(:,:)
        IF (KNB_CRYSTAL_SHAPE .GE. 3) & 
            PRADIP_SHAPE(:,:,3) = ZRE_ICE_UM_SHAPE_3(:,:)
        IF (KNB_CRYSTAL_SHAPE .GE. 4) &
            PRADIP_SHAPE(:,:,4) = ZRE_ICE_UM_SHAPE_4(:,:)
    END IF
END IF
