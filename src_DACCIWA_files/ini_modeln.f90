...
!  C. Barthe 03/2019 : add sea salt flux
!---------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
  ...
  IF ( CLBCX(1) /= 'CYCL' )  THEN
     IF ( NRIMX .GT. IDIMX )   THEN
        WRITE(*,'(A,I8,A/A,2I8,/A)') "Processor=", IP-1, &
             ...
             " Local domain too small for relaxation NRIMX,IDIMX ", &
             NRIMX,IDIMX ,&
  ...
  IF ( CLBCY(1) /= 'CYCL' ) THEN
     IF ( NRIMY .GT. IDIMY )  THEN
        WRITE(*,'(A,I8,A/A,2I8,/A)') "Processor=", IP-1, &
             ...
             " Local domain too small for relaxation NRIMY,IDIMY ", &
             NRIMY,IDIMY ,&
             " change relaxation parameters or number of processors "
        !callabortstop
        CALL ABORT
...
ALLOCATE(XFLX_SO2(IIU,IJU))
ALLOCATE(XFLXT_SO2(IIU,IJU))
XFLX_SLT(:,:,:) = 0.
XFLXT_SLT(:,:,:) = 0.
XFLX_AER(:,:,:) = 0.
XFLXT_AER(:,:,:) = 0.
