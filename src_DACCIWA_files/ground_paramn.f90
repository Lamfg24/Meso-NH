...
!!     (C. Barthe) 03/2019  Sea salt : add the flux
!!     (P. Tulet)  03/2020  direct coupling between LIMA and salt/dust emission
!-------------------------------------------------------------------------------
!
!*       0.     DECLARATIONS
!               ------------
...
USE MODD_FIELD_n,    ONLY : XUT, XVT, XWT, XTHT, XRT, XPABST, XSVT, XTKET, XZWS, &
                            XFLX_SLT, XFLXT_SLT, XFLX_AER, XFLXT_AER, XFLX_SO2, XFLXT_SO2
...
USE MODD_PARAM_LIMA, ONLY : LSALT2LIMA, LDUST2LIMA
...
INTEGER :: JSV_SLT   ! index for sea salt flux
...
IF (LUSECHEM) THEN
  ...
  END DO
    ...
    XFLX_SO2(:,:) = ZSFTS(:,:,NSV_CHEMBEG-1+JP_SO2) *64E-3 / XAVOGADRO
    ! total net flux of so2 in kg/m2
    XFLXT_SO2(:,:) = XFLXT_SO2(:,:)+ ZSFTS(:,:,NSV_CHEMBEG-1+JP_SO2)*XTSTEP
ELSE
...
IF (CCLOUD == 'LIMA' .AND. LSALT2LIMA .AND. .NOT.(LSALT)) THEN
  ! convert part.m-2.s-1 into part.kg-1.m.s-1
  PSFSV(:,:,NSV_LIMA_CCN_FREE) = PSFSV(:,:,NSV_LIMA_CCN_FREE) / XRHODREF(:,:,IKB)
END IF
IF (CCLOUD == 'LIMA' .AND. LDUST2LIMA .AND. .NOT.(LDUST)) THEN
  ! convert part.m-2.s-1 into part.kg-1.m.s-1
  PSFSV(:,:,NSV_LIMA_IFN_FREE) = PSFSV(:,:,NSV_LIMA_IFN_FREE) / XRHODREF(:,:,IKB)
END IF
!
IF (LSALT) THEN
...
! Instantaneous net flux
  DO JSV = 1, NMODE_SLT
    JSV_SLT = NSV_SLTBEG + (JSV-1) * IMOMENTS
...
!
IF ((LORILAM).AND.(CPROGRAM/='DIAG  ')) THEN
  DO JSV=NSV_AERBEG,NSV_AEREND
    PSFSV(:,:,JSV) = ZSFTS(:,:,JSV) * XMD / ( XAVOGADRO * XRHODREF(:,:,IKB))
! Instantaneous net flux (kg/m2/s)
    IDX=INT(0.5+(JSV-NSV_AERBEG+1)/2.)
    IF (IDX .LE. (NSP+NCARB+NSOA)) THEN
    XFLX_AER(:,:,JSV-NSV_AERBEG+1) = ZSFTS(:,:,JSV) * XMI(:,:,IKB,IDX) &
                                     *1E-3  / XAVOGADRO
! Total net flux
