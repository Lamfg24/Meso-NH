...
REAL, DIMENSION(:,:,:,:), INTENT(INOUT) :: PCIS_SHAPE  ! rapport de concentration des formes
...
!!      M. Claeys   06/2019  add several crystal shapes
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
USE MODD_PARAM_LIMA,      ONLY : NMOD_CCN, NMOD_IMM, XRTMIN, XCTMIN, XNUC, LWARM, LRAIN, &
                                 NB_CRYSTAL_SHAPE, LCRYSTAL_SHAPE
...
!
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PCIS    ! Ice crystal C. source
REAL, DIMENSION(:,:,:,:), INTENT(INOUT) :: PCIS_SHAPE  ! Rapport de concentration par forme 
REAL, DIMENSION(:,:,:,:), INTENT(INOUT) :: PNIS    ! Activated ice nuclei C. source
                                                   !for IMMERSION
REAL, DIMENSION(:,:,:),   INTENT(INOUT) :: PNHS    ! haze homogeneous freezing
!
...
REAL, DIMENSION(SIZE(PRHODREF,1),SIZE(PRHODREF,2),SIZE(PRHODREF,3))   &
                                  :: ZW, ZT ! work arrays
REAL, DIMENSION(:,:,:,:), ALLOCATABLE :: ZW_4D ! work arrays
!
...
IF (INEGT.GT.0) THEN
  ...
  IF (LCRYSTAL_SHAPE) THEN
    ALLOCATE(ZCIS_SHAPE(INEGT,NB_CRYSTAL_SHAPE ))
  END IF
  !
...
!
    ...
    IF (.NOT. LCRYSTAL_SHAPE) THEN
      ZCIS(:) = ZCIS(:) + ZZX(:)
    ELSE
! Pour SHAPE_02 : Formation de colonnes (JSH=2)  uniquement : a changer plus tard
...
! Budget storage
  IF (NBUMOD==KMI .AND. LBU_ENABLE .AND. OHHONI .AND. NMOD_CCN.GT.0 ) THEN
    ...
    IF (LBUDGET_SV) THEN
      ...
      ELSE
        DO JSH = 1, NB_CRYSTAL_SHAPE
          CALL BUDGET ( UNPACK(ZCIS_SHAPE(:,JSH),MASK=GNEGT(:,:,:),FIELD=PCIS_SHAPE(:,:,:,JSH))*PRHODJ(:,:,:),&
                                                           12+NSV_LIMA_NI+JSH-1,'HONH_BU_RSV') ! RCI
        END DO
      END IF
...
!                                                                       
      ...
      END WHERE
!
    ELSE IF (LCRYSTAL_SHAPE .AND. NB_CRYSTAL_SHAPE .GE. 3) THEN
!
      WHERE((ZZT(:)<XTT-35.0) .AND. (ZCCT(:)>XCTMIN(2)) .AND. (ZRCT(:)>XRTMIN(2)) )
        ZLBDAC(:) = XLBC * ZCCT(:) / (ZRCT(:)) ! Lambda_c**3
        ZZX(:) = 1.0 / ( 1.0 + (XC_HONC/ZLBDAC(:))*PTSTEP*                      &
...
!                                                                       
        ZCCS(:) = ZCCS(:) - ZZW(:)
        ZCIS_SHAPE(:,3) = ZCIS_SHAPE(:,3) + ZZW(:) ! irreguliers
        ZCIS(:) = SUM(ZCIS_SHAPE, DIM=2)
        ZZW(:) = ZRCS(:) * (1.0 - ZZX(:))                                  ! RCHONI
!                                                                       
        ZRCS(:) = ZRCS(:) - ZZW(:)
      ...
      END WHERE
!
    ELSE IF (LCRYSTAL_SHAPE .AND. NB_CRYSTAL_SHAPE .EQ. 2) THEN
!
      WHERE( (ZZT(:)<XTT-35.0) .AND. (ZCCT(:)>XCTMIN(2)) .AND. (ZRCT(:)>XRTMIN(2)) )
        ZLBDAC(:) = XLBC*ZCCT(:) / (ZRCT(:)) ! Lambda_c**3
        ZZX(:) = 1.0 / ( 1.0 + (XC_HONC/ZLBDAC(:))*PTSTEP*                      &
...
!                                                                       
        ZCCS(:) = ZCCS(:) - ZZW(:)
! Pour SHAPE_02 : Formation de colonnes (JSH=2)  uniquement : a changer plus tard
        ZCIS_SHAPE(:,2) = ZCIS_SHAPE(:,2) + ZZW(:)
        ZCIS(:) = SUM(ZCIS_SHAPE, DIM=2)
! ZZW -> conversion en mass mixing ratio : probleme d'unité ????
        ZZW(:) = ZRCS(:) * (1.0 - ZZX(:))                                  ! RCHONI
!                                                                       
        ZRCS(:) = ZRCS(:) - ZZW(:)
        ZRIS(:) = ZRIS(:) + ZZW(:)
    ...
    IF (NBUMOD==KMI .AND. LBU_ENABLE) THEN
      ...
      IF (LBUDGET_SV) THEN
        ...
        ELSE
          DO JSH = 1, NB_CRYSTAL_SHAPE
            CALL BUDGET ( UNPACK(ZCIS_SHAPE(:,JSH),MASK=GNEGT(:,:,:),FIELD=PCIS_SHAPE(:,:,:,JSH))*PRHODJ(:,:,:),&
                                                            12+NSV_LIMA_NI+JSH-1,'HONC_BU_RSV')
          END DO
        END IF ! lcrystal_shape
      END IF ! lbudget_sv
    END IF ! lbu_enable
...
!
  ...
  ELSE
    ...
    DO JSH = 1, NB_CRYSTAL_SHAPE
      ...
      PCIS_SHAPE(:,:,:,JSH) = UNPACK(ZCIS_SHAPE(:,JSH),MASK=GNEGT(:,:,:),FIELD=ZW_4D(:,:,:,JSH) )
    END DO
    ...
    IF (LBUDGET_SV) THEN
      ...
      ELSE
        DO JSH = 1, NB_CRYSTAL_SHAPE
          ...
          IF( OHHONI .AND. NMOD_CCN.GT.0 ) CALL BUDGET (ZW,12+NSV_LIMA_NI+JSH-1,'HONH_BU_RSV') 
          IF (LWARM) CALL BUDGET (ZW,12+NSV_LIMA_NI+JSH-1,'HONC_BU_RSV')
        END DO
      END IF
      IF( OHHONI .AND. NMOD_CCN.GT.0 ) THEN
        DO JL=1, NMOD_CCN
