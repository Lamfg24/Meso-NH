...
!   ZM(:,:,:,4)=   MAX(ZSV(:,:,:,JP_CH_M0j) * 1E+6, XMNH_TINY) ! molec_{aer}/m3_{air}
   ZM(:,:,:,1)=   ZSV(:,:,:,JP_CH_M0i) * 1E+6 ! molec_{aer}/m3_{air}
   ZM(:,:,:,4)=   ZSV(:,:,:,JP_CH_M0j) * 1E+6 ! molec_{aer}/m3_{air}
!
!-------------------------------------------------------------------------------
!
...
DO JN=1,JPMODE
  IF(PRESENT(PN3D)) THEN
    PN3D(:,:,:,JN) = ZM(:,:,:,NM0(JN))
    PRINT *, "MODE = ", JN, "PN3D =", MINVAL(PN3D(:,:,:,JN)/1000000),MAXVAL(PN3D(:,:,:,JN)/1000000)
  ENDIF

  IF(PRESENT(PRG3D)) THEN
    PRG3D(:,:,:,JN)=(ZM(:,:,:,NM3(JN))**4. &
              / (ZM(:,:,:,NM6(JN))*ZM(:,:,:,NM0(JN))**3.))**(1./6.)
    PRINT *, "MODE = ", JN, "PRG3D =", MINVAL(PRG3D(:,:,:,JN)),MAXVAL(PRG3D(:,:,:,JN))
  ENDIF
