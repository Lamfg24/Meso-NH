...
!  
   ...
   IF (DACCIWA) THEN
    ...
    !
            ZZW1(:)= 1350.0 * ZRCT(:)**(2.47) * (ZCCT(:)* ZRHODREF(:)/1.0E6)**(-1.79) ! ZCCT in cm-3         
            ZZW1(:) = min (ZRCS(:), ZZW1(:))
            ZRCS(:) = ZRCS(:) - ZZW1(:)
            ZRRS(:) = ZRRS(:) + ZZW1(:)
        !
            ZCRS(:) = ZCRS(:) + ZZW1(:) * 3./(4.*XPI*XRHOLW*(XR0)**(3.))
        !
    ...
    !
        ...
        IF (LBUDGET_SV) CALL BUDGET (PCCS(:,:,:)*PRHODJ(:,:,:),14+(NSV_LIMA_NC-1),&
                                    &'SELF_BU_RSV') ! RCC
        IF (LBUDGET_RC) CALL BUDGET (PRCS*PRHODJ(:,:,:),7 ,'AUTO_BU_RRC')
        IF (LBUDGET_SV) CALL BUDGET (PCRS*PRHODJ(:,:,:),15+(NSV_LIMA_NR-1),'AUTO_BU_RSV')
