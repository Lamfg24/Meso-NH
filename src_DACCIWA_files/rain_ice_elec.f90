...
!
INTERFACE
      SUBROUTINE RAIN_ICE_ELEC (OSEDIC, HSEDIM, HSUBG_AUCV, OWARM, KKA, KKU, KKL,     &
                                KSPLITR, PTSTEP, KRR,                                 &
                                ...
                                PRAINFR, PSEA, PTOWN,                                 &
                                PRHT, PRHS, PINPRH, PQHT, PQHS, PFPR                  )
!
!
...
!!         C. Barthe : 04/2019  Update with rain_ice.f90 in 5-4-1
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
! according to autoconversion option
  ...
  ELSEIF (HSUBG_AUCV == 'PDF ') THEN
    ...
    ELSE
      ...
      CALL PRINT_MSG(NVERB_FATAL,'GEN','RAIN_ICE_ELEC_n','')      
    ENDIF
  ELSE
...
!  CALL RAIN_ICE_SEDIMENTATION_STAT
  WRITE(*,*) ' STOP'
  WRITE(*,*) ' ELEC - NO SEDIMENTATION SCHEME FOR HSEDIM=',HSEDIM
!callabortstop
  CALL PRINT_MSG(NVERB_FATAL,'GEN','RAIN_ICE_ELEC_n','')
ELSE
...
IF (KRR == 7) ZPQHS(:,:,:) = 0.0
!
ZPQRS(:,:,:) = PQRS(:,:,:) - PQRT(:,:,:) * ZINVTSTEP
ZPQSS(:,:,:) = PQSS(:,:,:) - PQST(:,:,:) * ZINVTSTEP
ZPQGS(:,:,:) = PQGS(:,:,:) - PQGT(:,:,:) * ZINVTSTEP
IF (KRR == 7) ZPQHS(:,:,:) = PQHS(:,:,:) - PQHT(:,:,:) * ZINVTSTEP
PQRS(:,:,:)  = PQRT(:,:,:) * ZINVTSTEP
PQSS(:,:,:)  = PQST(:,:,:) * ZINVTSTEP
PQGS(:,:,:)  = PQGT(:,:,:) * ZINVTSTEP
IF (KRR == 7) PQHS(:,:,:) = PQHT(:,:,:) * ZINVTSTEP
!
...
!
  ...
  DO JK = IKTB, IKTE
    ...
    PQRS(:,:,JK) = PQRS(:,:,JK) + ZW(:,:,JK) * (ZWSEDQ(:,:,JK+KKL) - ZWSEDQ(:,:,JK))
  END DO
  IF (PRESENT(PFPR)) THEN
    DO JK = IKTB, IKTE
      PFPR(:,:,JK,3) = ZWSED(:,:,JK)
...
!
  ...
  DO JK = IKTB, IKTE
    ...
    PQIS(:,:,JK) = PQIS(:,:,JK) + ZW(:,:,JK) * (ZWSEDQ(:,:,JK+KKL) - ZWSEDQ(:,:,JK))
  END DO
  IF (PRESENT(PFPR)) THEN
    DO JK = IKTB, IKTE
      PFPR(:,:,JK,4) = ZWSED(:,:,JK)
...
!
  ...
  DO JK = IKTB, IKTE
    ...
    PQSS(:,:,JK) = PQSS(:,:,JK) + ZW(:,:,JK) * (ZWSEDQ(:,:,JK+KKL) - ZWSEDQ(:,:,JK))
  END DO
  IF (PRESENT(PFPR)) THEN
    DO JK = IKTB, IKTE
      PFPR(:,:,JK,5) = ZWSED(:,:,JK)
...
!
  ...
  DO JK = IKTB, IKTE
    ...
    PQGS(:,:,JK) = PQGS(:,:,JK) + ZW(:,:,JK) * (ZWSEDQ(:,:,JK+KKL) - ZWSEDQ(:,:,JK))
  END DO
  IF (PRESENT(PFPR)) THEN
    DO JK = IKTB, IKTE
      PFPR(:,:,JK,6) = ZWSED(:,:,JK)
...
!
    ...
    DO JK = IKTB, IKTE
      ...
      PQHS(:,:,JK) = PQHS(:,:,JK) + ZW(:,:,JK) * (ZWSEDQ(:,:,JK+KKL) - ZWSEDQ(:,:,JK))
    END DO
    IF (PRESENT(PFPR)) THEN
      DO JK = IKTB, IKTE
        PFPR(:,:,JK,7) = ZWSED(:,:,JK)
...
!
    IF (CSUBG_RC_RR_ACCR=='NONE') THEN
      ...
      WHERE (ZRCT(:) > XRTMIN(2) .AND. ZRRT(:) > XRTMIN(3) .AND. ZRCS(:) > 0.0)
        ...
        ZRCS(:) = ZRCS(:) - ZZW(:)
        ZRRS(:) = ZRRS(:) + ZZW(:)
        ZWQ1(:,2) = XCOEF_RQ_C * ZQCT(:) * ZZW(:) / ZRCT(:)
      END WHERE
!
...
!
    ELSEIF (CSUBG_RC_RR_ACCR=='PRFR') THEN
      ...
      ZZW(:) = 0.
      WHERE (ZHLC_HRC(:) > XRTMIN(2) .AND. ZRRT(:) > XRTMIN(3) .AND. &
             ZRCS(:) > 0.0           .AND. ZHLC_HCF(:) > 0 )
        ...
        ZZW(:) = XFCACCR * ( ZHLC_HRC(:)/ZHLC_HCF(:) )     &
               ...
               * ZHLC_HCF
      END WHERE
      WHERE (ZHLC_LRC(:) > XRTMIN(2) .AND. ZRRT(:) > XRTMIN(3) .AND. &
             ZRCS(:) > 0.0           .AND. ZHLC_LCF(:)>0)
...
!
    ...
    ELSEIF (CSUBG_RR_EVAP=='CLFR' .OR. CSUBG_RR_EVAP=='PRFR') THEN
      ...
      WHERE(  (ZRRT(:)>XRTMIN(3)) .AND. ( ZZW4(:) > ZCF(:) ) )
        ...
        ZRVS(:) = ZRVS(:) + ZZW(:)
        ZTHS(:) = ZTHS(:) - ZZW(:)*ZLVFACT(:)
        ZWQ1(:,3) = XCOEF_RQ_R * ZQRT(:) * ZZW(:) / ZRRT(:)
      END WHERE
