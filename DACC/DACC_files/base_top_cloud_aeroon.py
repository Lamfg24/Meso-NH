...
for i in range(1,5):
    ...
    MRC.append(mMRc)
    stdMRc = np.std(np.std(MRc,axis=1),axis=1)
    MRC_std.append(stdMRc)
    print(len(MRC))
    ZHAt = MESO_products.ZHAT.values
    np.savez("zhat",name = ZHAt)
...
np.savez("BCBackground_XXL_MRCstd_aeroon",name = MRC_std)
   	
np.savez("BCBackground_XXL_MRC_aeroon",name = MRC)
